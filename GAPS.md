...
| **~~`settings.showEnergyAxis` and `settings.theme` reach nothing~~ — re-measured 2026-08-05 ([§ D250](DECISIONS.md)), and ~~`showEnergyAxis` reaches a *function* and no pixel~~ — **both halves now CLOSED** (2026-09-02); the row is kept for what its staleness cost** | `theme` is wired and visible — measured in a browser, `data-theme="light"` on the root and the whole shell recoloured ([§ D235](DECISIONS.md) built the second palette). `showEnergyAxis` left `SINK_MISSING` because it acquired a sink, `render/runSummary.ts#summaryFigureIds` — and **that function's only shipped caller is `mode/disclosure.ts#disclosureItems`, whose only shipped caller is `dev/main.ts#drawParity`, which turns it into `parityRefusal`: a string that is empty whenever parity holds.** So the register was emptied by a sink nothing renders. Measured: with a run on screen at 1280×720, the whole shell's text is **byte-identical** with the switch on and off (GitHub issue #70, whose reporter is right about this one and wrong about persistence). The two energy cells a player reads are `shift/report.ts#energyFigures`, emitted unconditionally, and `DayReportInput` has no field for the preference — so the fix is one required field plus one caller, and the deeper one is that `scope.test.ts`'s *a presentation control reaches a sink* is satisfiable by a pure function no surface calls. **`SINK_MISSING` being empty is not the same claim as "all four are visible", and it was read as one.** **CLOSED, and this row went on describing the defect for weeks after** (re-verified 2026-09-02, `c3953bb`): the prescribed fix — *one required field plus one caller* — is built. `menu/screens.ts:1422` declares the toggle, `dev/main.ts:5679` passes the player's own value into `dayReportOf`, and `shift/report.ts:1236` is the sink; `shift/report.test.ts:613-625` moves the switch and requires the sheet to change, with companion cases pinning both and neither. Left in place rather than deleted, because this row is the evidence for [`RISKS.md`](RISKS.md) **R44**: it is one of the two stale rows that became the headline items of GitHub issue **#178**, and its item there was already built **eleven minutes before that issue was filed**. #325 owns the guard that would have caught it. |
...
| **~~A live weight editor makes overfitting the tuning seeds the dominant strategy~~ — CLOSED (2026-09-06, GitHub issue #325's mutation fixture)** | The finding, kept: a stage cleared on an edited vector was **beaten on three measures on that stage's own declared holdout set**, and the sweep was sharp — three neighbouring values clear, the fourth does not. Both halves of the row's claim were refuted four ways by 2026-09-05 and the row was not struck, which `RISKS.md` R44 recorded on that date and which is why this row was the fixture for `packages/viz/src/gapsRegister.test.ts`: `campaign/judge.ts` gates on the holdout, checks the second batch's seed rather than trusting it, `campaign/parse.ts` enforces disjointness at parse time, and `campaign/brief.ts` names the holdout set and its seed in the shipped brief, so the campaign no longer judges on tuning seeds only and the shipped surface says so. The guard's first run failed on this row with the row's own claim as its oracle; the row is struck on the same commit and the oracle now asserts the opposite. This closes GitHub issue #178's item 4. |
...
| **The 41 statically swept DOM entry points are not driven** | The generated honesty search reaches them only by scanning source for probability words — R10, and no other of the ten properties. A sentence assembled at runtime there is invisible to the rest. Weaker than driving, and stated as a limitation rather than presented as coverage. **The figure is derived rather than transcribed** ([§ D421](DECISIONS.md), `packages/viz/src/honesty/derive.test.ts`): **20** mounts and **21** screen-registry rows, every one excluded on DOM ground (the eighteenth mount is GitHub issue #348’s as-built stage on the fix-it screen, the nineteenth is issue #380’s worked answer, the one component § D529 clause 2 forbids building twice, and the twentieth is [§ D1000](DECISIONS.md)’s five-families editor, which both fix-it surfaces mount; the seventeenth row is issue #220’s rush, the eighteenth is issue #364’s Scenario hub — § D525’s first tile — the nineteenth and twentieth are § D529’s two tutorial screens, and the twenty-first is GitHub issue #244’s landing page). This row published *three* from wave 12 until 2026-09-01, and *three* was never a measurement — the narrowest reading of it, the mounts `derive.test.ts`'s own docstring enumerated, had been four since `mountRightRail` joined that list. **The screen rows' pure halves are driven** — each exclusion names the adapter that renders that screen's words — so what goes unswept, in both groups alike, is only what the entry point authors inline. |
...
| **~~The structural-refusal reason is prose keyed on an id the leg record does not carry~~ — CLOSED (2026-09-06, wave V, [§ D511](DECISIONS.md))** | `core` joins the reason to the leg at reconcile time — `UndeliveredJourney.structuralRefusal`, matched from the leg's own landing, direction and destination to the active call `#unservable` is keyed on — and `viz` carries it as `VizLeg.structuralRefusal` (version 12), absent on every leg that boarded or whose call was merely late. No surface draws it yet, and none is named. The row as it stood: So it cannot be joined to a leg. **This was in the wave plan and I never briefed it** — an orchestration miss, recorded rather than dropped. |
...
| **~~A dispatcher card's words are derived, and the better ones are authored where nothing may read them~~ — CLOSED (2026-09-06, wave V, [§ D508](DECISIONS.md))** | The ruling GitHub issue #178 item 5 asked for. `DispatcherProfile.blurb` is authored on every shipped profile (1–160 characters, schema-bounded, `loader.test.ts` holds the file to it), and `dev/rightRail.ts#authoredBlurbOf` draws it on the Casual face **only beside the exact shipped vector it was written for** — same id on the list, same blurb, same decision fingerprint — so a saved copy, an edited working copy or a searched vector gets the derived line and never a sentence about a different vector. That guard is what the withdrawal at `rightRail.ts` was about, and it is now a property `rightRail.test.ts` runs both ways. Both registers still carry every string (§ D299 § 2). `$comment` still reaches nothing. The row as it stood: `$comment` is maintainer documentation and no longer reaches a card ([§ D186](DECISIONS.md)) — the longest rendered card went from **5 133 characters to 164**. The derived replacement is honest and bounded and **reads as configuration, not as a sentence a building manager would say**, which `docs/12` § 2.2 makes a requirement rather than a flavour. The handoff already writes the right copy per dispatcher; `data/dispatcher-profiles.json` has no field to put it in, and `$comment` must not be made to serve. **The authored-field half of that wish is withdrawn in code rather than open, and this row does not say so** (re-verified 2026-09-02, `c3953bb`): `dev/rightRail.ts:293-306` withdraws it with an argument — a weight vector is the one object a **search** writes, so authored prose beside it is stale on the first round that improves it. **Not struck through**, because what survives the withdrawal is the other half: the derived card still reads as configuration rather than as a sentence a building manager would say, which `docs/12` § 2.2 makes a requirement. That residual needs a ruling and is GitHub issue **#178** item 5. |
...
| **`selection.*` has no control in the dispatcher editor, and the ground is a second writer rather than a dead field** — [`docs/21`](docs/21-engineer-reimagined-contract.md) § 3.6's one refusal | The other **six** blocks `dispatcherEditor.ts#unauthorableBlocksOf` named are authorable as of this wave (`dev/familyControls.ts`), each leaving the register on the commit that gave it controls. `selection` stays, and its ground is worth reading because it is **not** § D219's usual shape: `resolveDispatchConfig` resolves all seven fields and a run honours them, so the field is alive. What refuses a control here is that `dev/state.ts#drivingProfileOf` ends in `profileWithSelector(…, state.selectorSpec)`, which **rebuilds `profile.selection` from the Selector panel's own document and deletes what that document does not carry** — so a value written from this panel would be overwritten before the shift started. The refusal names the panel that does own the field, which the old one-clause register entry did not. Closing it properly means one document for `selection`, not two: either the Selector panel's spec is seeded from the dispatcher being edited, or the dispatcher editor writes through that spec. Neither is a control this lane could add without deciding which. The ground is pinned by a test that reads `drivingProfileOf` back out of the source, so if that call ever leaves, the refusal goes red rather than quietly stale (§ D227's *worse than a dead seam*). |
