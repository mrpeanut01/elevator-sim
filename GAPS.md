...
| **Basic mode cannot *shorten* a suppression reason — NARROWED, not closed** | `core`'s half is done: the ground now travels beside the prose as `RunSummary.awtInvalidGround`, derived from the table where a ground *is* its entry, and `viz/src/mode/` has the per-ground wording ([§ D183](DECISIONS.md)). **What remains is the transport**: `VizSummary` does not carry the field, so on a recording this build produces the ground is absent and Basic renders exactly what it rendered before — asserted byte-identical, so nothing regressed while it waits. Closing it is a `VIZ_SCHEMA_VERSION` bump with its history row, which `contract/types.ts`'s convention makes deliberate rather than incidental. **The consumer landed before the transport, which is the ordering this repository prefers**: a field that arrives before anything reads it is the dead seam, not the reverse. |
