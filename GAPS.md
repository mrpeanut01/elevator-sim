...
| **Sandbox is a string with no feature behind it** | `Sandbox` appears in the coach ribbon when a building has no contract, and no mode sits behind it — a label describing a feature. **Endless is closed:** `c5`'s reward now names something that exists (`shift/week.ts`'s `openEndless`, `menu/enterEndless.ts`, the **Keep going** row on the campaign screen), and the day report has a sentence of its own rather than telling an endless player they are on their own building. Sandbox was left open because it is not the same shape: endless was a missing *mode* and this is a missing *meaning* — nobody had decided what Sandbox is, and building the wrong thing is worse than the label. **That has been decided and enforced, and this row went on saying otherwise** (re-verified 2026-09-02, `c3953bb`): `shift/week.ts:104` carries `SANDBOX_CONTRACT_ID`, `dev/state.ts:846` is its non-test writer, `shift/weekLabel.ts:116` is the label case, and `week.ts:431` is the scoring gate. `e0f87d5` (GitHub issue #290) made `closeDay` sandbox-aware, after which `bestMinutePct`, `streak` and `cleanRun` stopped moving on a week belonging to no assignment — and `weekLabel.ts:20` says this row is stale in terms, which nobody read. The second of the two rows that fed **#178**; see [`RISKS.md`](RISKS.md) **R44** and #325. |
