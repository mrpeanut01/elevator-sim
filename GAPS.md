...
| ~~**`DayReportInput.event` is a dead input**~~ **— closed, and the larger half was not the dead input** | Destructured and read by nothing; the forecast card names *tomorrow's* event, derived independently through `eventFor(day + 1, …)`. So a sheet for a move-in day described the figures and never mentioned the derated car that shaped them: every line true, and the account missing its subject. It is now a meta line, because it is identity rather than a reading — and absent on a single run, where there is no week to book anything against. |
