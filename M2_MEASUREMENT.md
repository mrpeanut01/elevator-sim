...
```
grep -rl "chromium.launch" packages --include="*.browser.test.ts" | wc -l   # → 65
find packages -name "*.browser.test.ts" | wc -l                            # → 65
grep -rn "firefox\|webkit\|Firefox\|WebKit\|Gecko" packages --include="*.ts"  # → no output
```

**65 of 65** browser-tier files call `chromium.launch()` as a literal, and the strings `firefox`,
...
1. **The product's own tier is single-engine** (§ 3). Even a machine with all three browsers
   ...
   **65 of 65** browser-tier files name `chromium`. Until a browser-tier file takes its engine as a
...
2. **Take the engine as a parameter.** `browserTier.test-helper.ts` already owns the gate in one
   ...
   Firefox row #203 § 4 already recommends**: installing Gecko buys nothing while **65 of 65**
