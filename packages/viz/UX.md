...
> **Wave 12 (2026-07-30):** `SG-15` was confirmed inert by driving and then wired
> ([§ D187](../../DECISIONS.md)); `SH-09` was built ([§ D189](../../DECISIONS.md)); `RX-03` remains
> the one `🔲` row, deferred to the drive phase. The paragraph above stands as the record of how
> this pass marked them.
>
> **Drive phase (2026-07-30, later the same day): the browser arrived.** § 26 was executed top to
> bottom in Chromium (Playwright against the dev server), so *"`✅ run` appears on exactly zero
> rows"* above is now history: 81 rows carry `✅ run` with one-line evidence each, `RX-03` was
> reproduced, fixed (`5d4b782`) and re-driven, and **four new `🔲` rows were found by driving and
> filed rather than fixed** — `TP-08`, `TP-10`, `DR-13`, `RR-11`. § 25 holds the re-derived count;
> the defect list travels with the drive report.
>
> **Defect-fix lane (2026-07-30, later still): the filed defects are closed.** All four driven-red
> rows and § D198's cosmetics were fixed the same day, each red before green, and re-driven in
> Chromium ([§ D199](../../DECISIONS.md)) — the rows in §§ 13, 14 and 16 carry the closures with
> their history kept, `SH-21` (the favicon) was added because § D198 recorded that *no row covered
> the favicon; the absence is the finding*, and § 25 is re-derived. No `🔲` remains.
...
|---|---|---|---|---|
| SH-01 | happy | Open the page cold | Every element the viewer resolves is present, and an opening shift runs unprompted | ✅ test — `elementMap.test.ts` checks the manifest against `index.html` **in both directions**, has no id twice, and lists exactly which of the page's ids the viewer never resolves (the document editor's) · ✅ run (2026-07-30 drive, Chromium 1500×950: the page opened cold and the opening shift ran unprompted — the status filled with AWT/WT95) |
| SH-02 | happy | Read the tab strip at rest | Three primary tabs and three secondary; the four editors absent, not merely disabled | ✅ test — `surfaces.test.ts` *"hides every contextual editor nobody has opened"* · ✅ run (drive: at rest the strip showed exactly run/report/scenarios plus the three instruments; the four editors absent) |
| SH-03 | happy | Click a tab | Its panel shows, every other hides, `aria-selected` follows | ✅ test (`surfaceStateFor` shows exactly one panel) · ✅ run (drive: all ten tabs clicked; exactly one surface panel shown each time, `aria-selected` following) |
| SH-04 | alternate | <kbd>←</kbd>/<kbd>→</kbd>/<kbd>Home</kbd>/<kbd>End</kbd> on the strip | Walks the **visible** ring, wrapping; a hidden editor is skipped, never focused | ✅ test (`tabAfterKey`, including the skip and the wrap) · ✅ run (drive: the arrows walked and wrapped the visible ring, skipping unrevealed editors; Home/End hit the ends) |
| SH-05 | alternate | Open an editor from the right rail | Its tab appears and **stays** appeared for the session | ✅ test (`surfaceStateFor` with a revealed set) · ✅ run (drive: all four `Open … editor →` buttons revealed, opened and focused their tab, and the tab stayed for the session across a close and reopen) |
...
| SH-07 | edge | The roving tabindex | Exactly one **visible** tab is `0`; exactly one panel is shown | ✅ test · ✅ run (drive: exactly one visible tab at tabindex 0, no hidden tab focusable) |
| SH-08 | alternate | Deep link `?building&dispatcher&seed&duration&tab&rail&mode` | Opens there; a value the page does not have is refused rather than silently opening the first tab | ✅ test — the seven-key patch, the `duration` clamp and the reader refusals are tested directly since wave 12 via the pure `deepLinkStateOf` ([§ D189](../../DECISIONS.md), `main.test.ts`) · ✅ run (drive: a `?building&seed&tab&rail` reload restored building, tab, rail and a bit-identical status; `?mode=advanced` opened in engineer) · History: ~~⚠️ `applyDeepLink` is exported from `main.ts` for exactly this and no test imports it~~ — true until § D189 |
| SH-09 | alternate | Change the building, the seed or the tab, then copy the address bar | The URL follows the run, so a second person pasting it sees the same page | ✅ test — **built in wave 12** ([§ D189](../../DECISIONS.md)): `history.replaceState` — never `pushState` — at the `renderAll` chokepoint; deterministic defaults omitted, the seed always written, nothing written before boot; the round trip `deepLinkStateOf(deepLinkSearchOf(state))` asserted directly, and `applyDeepLink` has its first tests via the pure `deepLinkStateOf` it was refactored around · ✅ run (drive: building/seed/tab/rail all reached the address bar and a reload reproduced the run bit-identically) · ✅ **residual closed in the defect-fix lane** ([§ D199](../../DECISIONS.md), `4a62387`): one `syncUrl()` immediately after the `urlWritable` flip, so boot writes exactly `?seed=…` the moment it completes — § D189's three intents (`replaceState` only, deterministic defaults omitted, nothing written *before* boot completes) survive untouched, and `main.test.ts` pins the flip-then-write ordering at the source; re-driven · *History:* ~~⚠️ **defect filed (drive): the opening run’s seed never reaches the address bar** — `urlWritable` flips true only after boot’s `runShift()` has already rendered, so the URL is bare until the first interaction and a link copied at that instant is a different run wearing the same address~~ · ~~🔲 **not built, and this is a regression against the retired `RV-03`/`RV-T2`.**~~ There was no `history.pushState` or `replaceState` anywhere in `packages/viz`; the link was read on boot and never written |
| SH-10 | alternate | Narrow the window past 1340 px | The right rail becomes an overlay drawer, closed, with a toggle in the tab strip | ✅ test — `drawerStateFor`, **and** `surfaces.test.ts` reads `index.html` and asserts `DRAWER_BREAKPOINT_PX` matches the `@media (max-width: 1339px)` rule that does the layout · ✅ run (drive at 1339/1340: the rail became a closed overlay drawer with the toggle in the strip; one press opened it; the `matchMedia` crossing kept state coherent both directions) |
| SH-11 | alternate | Widen it again | The column comes back without the reader pressing anything, and their choice is remembered for the next narrowing | ✅ test · ✅ run (drive: widening restored the column unprompted; narrowing again remembered the reader’s open drawer) |
| SH-12 | recovery | <kbd>Escape</kbd> with the drawer open | The drawer closes | ✅ test — **built in wave 12** ([§ D188](../../DECISIONS.md)): a pure `escapeClosesDrawer(viewportPx, openedByReader)` in `dev/surfaces.ts`, wired into `wireKeyboard`, focus returned to `#drawer-toggle`; **deliberately inert in column mode** — at ≥ 1340 px the rail is a remembered layout choice, not a modal · ✅ run (drive at 1200 px: Escape closed the reader-opened drawer and focus returned to `#drawer-toggle`; at 1500 px Escape left the column alone) · *History:* ~~🔲 **not built — a real gap, found by driving in an earlier pass and confirmed here by reading.**~~ There was no `Escape` handler in `packages/viz/src` or in `index.html`; the toggle was the only way out |
| SH-13 | happy | Read the header | Building name, spec line, clock, phase pill, day and weekday, tenant count | ✅ run for `drawHeader` (drive: name, day pill and clock drawn and moving on every run change) · the parts are ✅ test elsewhere: `statLineOf` (`contracts.test.ts`, on all five buildings), `clockAt`/`phaseAt` (`timeline.test.ts`, including the wrap past midnight) |
| SH-14 | happy | Read the footer | Status line, seed line, **one** `copy run` | ✅ test — `provenanceBlock.test.ts` asserts `#copy-provenance` is gone from the page, the manifest **and** `docs/12`, so exactly one control copies provenance · ✅ run (drive: status line, seed line and exactly one `copy run`) |
...
| SH-16 | failure | `data/` fetch fails | The **failing path** is named in every failure mode — not only on `!response.ok`, which is the branch a file missing from this dev server does not take — focus moves to the `role="alert"` region, and a Retry chip appears beside it | ✅ test (`bootstrap.test.ts` fail → Retry → succeed; `dev/data.ts` names the path in all three modes) · ✅ run (drive with the `data/` fetches blocked: the failing path named — *could not fetch /elevator-specs.json* — focus landed on the `role="alert"` region, the Retry chip appeared, all six transport controls really disabled; ~~noted while there: the phase pill and the footer status line are simply blank in this state~~ — the side-finding is closed in the defect-fix lane ([§ D199](../../DECISIONS.md), `2a2cdc0`): `showLoadFailure` now authors both slots with the empty state's own strings, *no run yet* and *no shift run yet — data/ did not load*, and the state was re-driven) |
| SH-17 | recovery | Press Retry | Refetches with no page reload, starts **at most once** however many attempts succeed, and a throw from `start` rejects rather than vanishing into a floating promise | ✅ test — the four `bootstrap.test.ts` cases are the `RV-21` regression, which was **false when driven** · ✅ run (drive: Retry after unblocking refetched with no reload and started exactly one run) |
...
| SH-19 | empty | Nothing has run yet | The empty state is explained rather than blank | ✅ run — **corrected and closed in the defect-fix lane** ([§ D199](../../DECISIONS.md), `2a2cdc0`): this row's claim was **false as written** — `#clock`'s `06:00` and the canvas's `No run yet.` are markup literals, but the pill's *no run yet* and the footer's *no shift run yet* were `drawHeader`/`drawFooter` strings that never run on a failed boot, so in the one state this row is reachable in both slots were simply **blank** (the drive's `SH-16` side-finding). `showLoadFailure` now authors both — *no run yet* and *no shift run yet — data/ did not load* — and the failed-boot state was re-driven with all four slots filled. Still **barely reachable**: `boot()` calls `runShift()` unconditionally, so the empty state is seen only when the opening run **fails** |
| SH-20 | failure | `data/campaign.json` fails while the other nine surfaces load | The failure is reported in the Campaign surface's own alert slot; the other nine come up | ✅ run (drive with only `campaign.json` blocked: the failure landed in `#campaign-error` and the other nine surfaces came up, run included) |
| SH-21 | edge | The browser asks for `/favicon.ico` | The page ships an icon, so the console carries no 404 on any load — the one standing error that trains a reader to ignore the console | ✅ run — **added in the defect-fix lane** ([§ D199](../../DECISIONS.md), `2a2cdc0`; [§ D198](../../DECISIONS.md) finding 7 recorded that *no ledger row covered the favicon; the absence is the finding*, and this row closes the absence): an inline SVG `data:` icon in `index.html`'s head — two shafts and a car, in the page's own tokens — so there is no new request and no new file; re-driven with a clean console. The fix is markup-only, so no test asserts the link; `elementMap` and the provenance guards are untouched |
...
| LR-15 | happy | *Show me the maths* | It is **a toggle that toggles** — the prototype's own rule made it inert | ✅ test (`mathsDisclosureOf`) · ✅ run (drive: the disclosure opened and closed across two clicks, `aria-expanded` following) |
| LR-16 | happy | Engineer maths | Quotes the run's own refusal **verbatim** when the means are suppressed; says which gates passed, over what window and over what `n`, when they do; never reproduces the prototype's invented figure | ✅ test (`live/honesty.test.ts`) · ✅ run (drive, on a vertical-city run refused on the abandonment-horizon ground: the maths quoted the run’s own reason verbatim) |
...
| LR-20 | empty | A recording with no decision yet | One *standing by* row, drawn as a state with no clock time it did not have | ✅ test · ✅ run (drive: one *standing by* row before any call registered) |
| LR-21 | failure | A run whose mean is refused | **No mean anywhere in the left rail** — not as a number, not inside a sentence — and every row still fills, which is why the rail is counts | ✅ test (`leftRail.test.ts`, plus `live/noMeans.test.ts`, which sweeps every module in `live/` for a suppressible figure and carries a positive control) · ✅ run (drive: on a suppressed run no digit-bearing mean appeared anywhere in the rail) |
...
| Id | Class | Scenario | Expected | Mark |
|---|---|---|---|---|
| CO-01 | happy | Pick a different building | The run re-runs on it, and the week follows the **scenario that building belongs to** rather than staying on the old one | ✅ test (`state.test.ts` `withBuilding`: takes the scenario, leaves the week alone for a building the reader drew, re-seeds the editor's working copy only while it is untouched, and is pure) · ✅ run (drive: changing the building re-ran the shift on it at the same seed) |
| CO-02 | happy | Pick a different arrival pattern | The run really moves off the building's own profile | ✅ test — **positive half closed in wave 12, lane T** ([§ D191](../../DECISIONS.md)): a shipped non-default arrival pattern changes both the handed config **and the legs**, beside the pre-existing negative half (`shiftRunConfigOf` hands the run **no** demand override under the building's own demand) · ✅ run (drive: building → hotel moved the figures at the same seed, and returning to the building’s own demand reproduced the original bit-identically) · *History:* ~~⚠️ — **this is the § D177 gap on the busiest control in the ribbon**~~ — the select had no legs test and nothing asserted the positive half |
| CO-03 | happy | Pick a different shift length | The run re-runs at the new duration and the timeline re-rules | ✅ run (drive: 1800 → 900 re-ran at the new duration, re-ruled the timeline ticks and wrote `duration=900` to the URL) — *was unverified with no test; the drive is its first establishment* |
| CO-04 | happy | Press **Run** | Runs what the three selects above it describe | ✅ test — `provenanceBlock.test.ts` asserts `#run` is in the coach ribbon, **is off the transport card**, and that the manifest agrees which surface owns it · ✅ run (drive: Run re-adopted the run the selects describe; unchanged selects and seed reproduced the same figures) |
| CO-05 | alternate | Press **See all scenarios** | The Scenarios tab opens | ✅ run (drive: the Scenarios tab opened and was selected) |
...
| SG-06 | happy | The out-of-service badge | One badge per column, its state **in words**, a hit rectangle that contains the pill it drew, and the same fact in the text alternative | ✅ test (`stageRender.test.ts`) · ✅ run (drive: the badge answered a pointer click at its drawn rectangle and the state reached the text alternative in words — *1 car held out of service: main-B*) |
| SG-07 | alternate | Click the badge | That car leaves service and the shift re-runs without it | ✅ test for the **mechanism** — `events.test.ts` *"move-in genuinely takes a car out of service"* drives the same `outOfServiceCarIds` through `shiftRunConfigOf` into a real run · ✅ run for the **click** (drive: one click under the shaft took `main-B` out of service and re-ran the shift without it — AWT 22.8 → 37.2 s; a second click restored the original figures bit-identically) |
...
| SG-10 | failure | A run whose mean is refused | No mean on the canvas, in the `aria-label`, or in the bitmap `Export PNG` writes — on **both** suppression grounds | ✅ test (`canvas.test.ts`, both grounds, with the gate copied from the summary and never recomputed) · ✅ run (drive: a refused run’s canvas `aria-label` carried the suppression sentence and no mean) |
...
| SG-15 | alternate | Choose a bank in the bank filter | The picture narrows to that bank | ✅ test — **wired in wave 12** ([§ D187](../../DECISIONS.md)): a pure `shaftsForBank` threaded through `drawStage` → `buildLayout`, the § D177 test moves the filter and requires the column set to change, `''` asserted unchanged **by identity**, the unknown-bank fallback pinned, and the no-silent-truncation caption *"bank X — showing N of M shafts"* carried into the exported PNG · ✅ run (**re-driven 2026-07-30, drive phase**: vertical-city, playback paused, sha-256 over the canvas bitmap — all eight bank choices produced distinct bitmaps and returning to `(all)` was byte-identical to the original) · *History, kept because the finding is the row's value:* ~~🔲 **INERT — found by reading and CONFIRMED BY DRIVING 2026-07-30.**~~ `#bank-filter`'s `change` handler wrote a `bankFilter` binding that `drawStage` never read; `buildLayout` and `drawScene` were handed `recording.shafts` whole and took no filter. The only other reader was `fillBankSelect`, which passed it back as the *selected option*. **Driven:** `vertical-city`, twelve cars over seven banks, playback **paused** so the clock could not confound the measurement — a hash over the canvas bitmap was **byte-identical** across `(all)`, `shuttle` and `zone-6-local`, and identical again on returning to `(all)`. The first attempt at this measurement *was* confounded — the run was playing and the pixels changed because the clock moved — which is why the paused control matters and is recorded. The retired `RS-05`'s claim that *"the bank filter narrows to a bank"* was **false of this viewer** until § D187 |
| SG-16 | alternate | Choose a landing in the landing selector | The assigned car's shaft is outlined and captioned with how long until it arrives; *unassigned* when the record never answered; *nobody waiting* and *nobody answered* are different sentences; under destination dispatch the **promised** shaft is outlined, not the one that happened to answer | ✅ test (`overlayRender.test.ts`) · ✅ run (drive: picking a landing changed the paused bitmap; clearing it restored the view) |
...
| SG-18 | empty | No recording | The canvas is left at its markup `aria-label` of `No run yet.` and nothing is drawn | ✅ run (drive: with the boot blocked the canvas stayed at its markup `No run yet.` and nothing was drawn) |
...
| Id | Class | Scenario | Expected | Mark |
|---|---|---|---|---|
| TP-01 | happy | Play / pause | The glyph and the `aria-label` follow the state | ✅ run (drive: ❚❚/Pause ↔ ▶/Play followed every press mid-run) — `Playback` itself is ✅ test (`playback.test.ts`, `mapping`) |
| TP-02 | happy | The speed chips | ×1 / ×10 / ×60 / ×240 / ×900, exactly one pressed, each captioned *N simulated seconds per real second* | ✅ run (drive: five chips, exactly one pressed, each captioned *N simulated seconds per real second*; `[`/`]` walked and clamped the ladder) — `SPEEDS` is now read by a test — `chromeLabels.test.ts` imports it and pins one `aria-label` per rung, so a chip added to the ladder without a name fails there; the ×900 rung exists because a 1 800 s shift at ×60 is thirty seconds of watching |
| TP-03 | happy | The timeline | Phase segments contiguous over `[startedAt, endedAt]` with no gap and no overhang, the drain banded rather than left as a hole, ticks ruled at the run's own times both ends inclusive, and the playhead where the clock is | ✅ test (`timeline.test.ts`, including the empty schedule and the drain tail) · ✅ run (drive: the ticks re-ruled on a duration change and `#playhead` kept moving across many re-runs and fills) |
| TP-04 | happy | Click the timeline to scrub | The playhead lands where dropped and the picture is correct for that instant | ✅ run (drive: a click at 50 % of the timeline landed the playhead at 49.87 % with the clock at the matching instant) — `Playback.seekToProgress` is also ✅ test |
| TP-05 | alternate | <kbd>←</kbd>/<kbd>→</kbd> with the timeline focused | Seeks | ✅ run (drive: ArrowRight on the focused timeline sought one real second × speed) — and note what it seeks: `step(±60)` is ±60 display frames, i.e. **±1 real second × the current speed**, so ±60 simulated seconds at ×60 and ±900 at ×900. That is not the retired `KB-04`'s ∓5 s / ∓60 s; see `KX-10` |
| TP-06 | alternate | Step back / forward | Pauses first, then advances exactly one display frame at the current speed | ✅ run (drive: each button paused and moved exactly one display frame at the current speed) — shares one handler with `KX-05`’s <kbd>,</kbd>/<kbd>.</kbd> |
| TP-07 | alternate | The loop chip | A `.chip[aria-pressed]` beside the speed chips, not a checkbox; toggling re-adopts the run at the current instant, because `Playback` takes `loop` at construction | ✅ test for the markup (`provenanceBlock.test.ts`) · ✅ run for the re-adopt (drive: toggling re-adopted at the current instant — the clock held) |
| TP-08 | invalid | Type into the seed field | A blank draws one and **shows the drawn seed**; a whole number re-runs at that seed; anything else is **refused by name** rather than coerced, and only the literal `0` ever runs seed 0 | ✅ test — **fixed in the defect-fix lane** ([§ D199](../../DECISIONS.md), `815e314`): the parse is the exported `seedEntryOf`, holding `menu/menu.ts#isSeedText` — **1–20 digits or a refusal**, the bound the menu's own Seed field has always taken and this one adopted in GitHub issue #111(c), because a run started here can be posted to a board and an over-long seed would have been refused at post time by a rule this screen never mentioned (it read `/^\d+$/`, unbounded, until then): a whole number runs as itself, a blank asks for a fresh draw and the drawn seed is written back into the field (a blank field over a drawn seed is the same hazard one keystroke earlier), and anything else refuses on `#status` **naming what was typed** with the field restored to the seed still running — `main.test.ts` pins all three arms, including that only the literal `0` may run seed 0 · ✅ run (re-driven: `banana` refused by name with the run untouched; a blank drew and showed the drawn seed) · *History:* ~~🔲 **driven 2026-07-30 and the hazard is real — defect filed.** Typing `banana` silently re-ran at **seed 0**: the field still read `banana` while the footer read *seed 0*, so a mistyped seed reproduced a different run with no refusal anywhere. The parse was `BigInt(raw.replace(/\D/g, '') \|\| '0')`, so a non-numeric entry became **seed 0** rather than being refused~~ |
| TP-09 | alternate | Verify replay | A match names the fingerprint; a **mismatch reports both fingerprints and leaves the stored recording on screen** | ✅ test (`record/document.test.ts`, `replay/replay.test.ts`, including a fingerprint that moves when a car's start position moves) · ✅ run for the button (drive: Verify named the fingerprint — *seed … reproduces this recording (fingerprint 3693e426)*) |
| TP-10 | alternate | Save recording | A JSON document that **is** the recording — the same document Load reads, frames re-derived on load — named by building and seed | ✅ test — **fixed in the defect-fix lane** ([§ D199](../../DECISIONS.md), `a2a95b4`): the reader is the contract and is untouched; the writer moved to it — `writeRecordingDocument` now sits beside `readRecordingDocument` in `record/document.ts`, and **the document is the recording itself**, whose top level already carries `schemaVersion` and every required field. Frames are deliberately not persisted — a pure derivation persisted beside its source is drift waiting to happen — and `document.test.ts` asserts the round trip with the re-derived frames **bit-identical**, plus the old `{recording, frames}` wrapper reproduced byte-for-byte and refused; that removal took the frame serializer's last shipped caller, *which was the defect*, so the pair is reclassified deliberate-API with the C24 precedent · the happy-path browser adopt through the file input is still owed a drive, and is marked unverified on `TP-11`, whose row carries that clause · *History:* ~~🔲 **driven 2026-07-30 — defect filed.** Save downloaded `{recording, frames}` with no top-level `schemaVersion`, and the shipped Load refused exactly that document — *“the document has no numeric schemaVersion, so it is not a viz recording”* — so the product could not reload the file it saved. `record/document.ts` had a `readRecordingDocument` and no writer beside it~~ |
| TP-11 | failure | Load a recording | The schema version is checked **first**; a newer one is refused by name; an **older** one is refused too, because a v2 recording has no `legs` and the overlay would silently report an empty window on it; a truncated file reports its parse position and the previous run keeps playing | ✅ test (`record/document.test.ts`) · ✅ run for the refusals (drive: schema 999 refused by name against the shipped 8; a truncated file named its parse position; the previous run kept playing through both) · ⚠️ (the happy adopt through the file input — still owed a drive; since [§ D199](../../DECISIONS.md) `TP-10`'s Save writes a document Load accepts, so the artifact to adopt now exists) |
| TP-12 | alternate | Export PNG | The current frame, with the seed and the clock in the header the canvas already draws | ✅ run (drive: Export downloaded a real PNG named by building and seed) — the header's content is ✅ test (`canvas.test.ts`, *"names the reporting window in the footer, so the exported PNG says what it covers"*); the export is not |
| TP-13 | failure | Press `copy run` | One control copies `building, dispatcher, traffic, seed, duration` in a form the CLI accepts | ✅ test — **met in wave 12** ([§ D190](../../DECISIONS.md), `dbfc22e`): the line now emits `--traffic`/`--template`, and the viewer route was **driven bit-identical to the CLI route at 10 of 10 cells** (sha256 over `legs`/`boardedLegs`/`waiting`) rather than assumed equivalent, with one cell re-pinned leg-for-leg as the tripwire; what no flag can express — a saved pattern, any day but the first, held cars, moved group levers, saved buildings or dispatchers — **refuses with named reasons** on `#status` and copies nothing · ✅ run (**re-driven, drive phase**: on the hotel pattern the clipboard carried `--traffic hotel --template lunch-two-way`; on a later day the copy **refused** — *no CLI line reproduces this run … the CLI has no --day* — and the clipboard was untouched) · *History:* ~~🔲 **unmet, and carried forward verbatim from the retired `RV-T7` and from [`GAPS.md`](../../GAPS.md).**~~ The line read `--building … --dispatcher … --seed … --duration …` and named **no traffic and no day**, so on any non-default pattern, or any day past the first, it reproduced a **different** run — a provenance claim the CLI would honour and the reader could not check |
| TP-14 | happy | How many controls copy provenance | Exactly one | ✅ test — `provenanceBlock.test.ts` asserts `#copy-provenance` is gone from `index.html`, from `elementMap.ts` **and** from `docs/12`, in both directions. The design refactor had left two controls calling the same function with the same state · ✅ run (drive: `#copy-run` count 1, `#copy-provenance` count 0) |
| TP-15 | happy | The block's vocabulary | No native checkbox and no visible file input; the seed, the bank filter and the landing selector carry the editors' field styling; `.field-inline`, `.file-ghost` and `.provenance` declared once in the stylesheet; what remains grouped under an eyebrow | ✅ test · ✅ run (drive: no native checkbox in the transport and the file input is visually hidden behind its ghost label) |
| TP-16 | empty | No recording | The six transport controls are disabled and visibly so — **never enabled controls that do nothing** | ✅ run (drive with the load blocked: all six controls really `disabled`) — this is the exact pair the retired § B.3 found **false** twice, which is why it sat high on § 26 |
...
| TP-18 | alternate | The status line after a run | `AWT … · WT95 …`, or the suppression reason in its place | ✅ run for the writing (drive: `AWT … · WT95 …` on clean runs, the verbatim suppression reason in its place on refused ones) · ✅ test for the gate — `meansAreSuppressed` has one home in `frame/overlay.ts` and is read by everything |
...
| Id | Class | Scenario | Expected | Mark |
|---|---|---|---|---|
| DR-01 | happy | The playhead reaches the end | The day closes and the sheet opens, **once** — a loop does not file the same day twice | ✅ run (drive at ×900: the sheet opened once at end-of-shift and a hand reopen did not refile the day) |
| DR-02 | alternate | Open the Day report tab by hand | The shift closes there too, and the tab is not set twice by two writers fighting | ✅ run (drive: opening the tab by hand closed the shift there too, once) |
...
| DR-09 | happy | The rest of the sheet | Title, meta lines, verdict, streak, cleared banner only when a day banked the last clean shift, the four levers **verbatim**, diagnosis rows accented from their own tone, tomorrow's forecast with the growth it really applies, and the small print naming this run's dispatcher — and the contract line's banked count **clamped on display**, so a sheet on a contract `cleanRun` kept counting past never reads *2 of 1* | ✅ test (the clamp since the defect-fix lane — [§ D199](../../DECISIONS.md), `2a2cdc0`: `report.test.ts` pins *2 of 2 clean shifts banked* on a week whose raw count ran past the ask, display-only, the week's own count untouched) |
...
| DR-12 | alternate | **Take the next assignment** | Restarts the week on the next contract's building, keeping what was cleared, and clears the run | ✅ test for the transition (`week.test.ts`: resets streak, banked count, day and history; awards a contract once however many clean days follow; at the end of the list says so rather than promising a sixth scenario) · ✅ run for the button (drive, on a cleared day 1: restarted on Midtown Office, day 1, with a fresh run) |
| DR-13 | alternate | **Open the doors on tomorrow** | Advances the day, wraps the weekday, clears the award so a banner belongs to one report, and clears the recording in the **same patch** so the reader never sees yesterday's figures under today's date | ✅ test (`week.test.ts` for the transition; and since the defect-fix lane `reportPanel.test.ts` pins the **binding site** — [§ D199](../../DECISIONS.md), `b94d602`: the panel owns its buttons per the `mountTypes.ts` idiom, `main.ts`'s duplicate listeners are deleted, and the pins assert **one binding site per button and one `nextDay` application per press**, so the double-wiring class cannot ship silently again. `#report-back` carried the same double wiring, idempotent only by luck, and is pinned by the same rule) · ✅ run (re-driven: one press went Day 1 → Day 2) · *History:* ~~🔲 **driven 2026-07-30 — defect filed: one press advances TWO days.** `reportPanel.ts:400` and `main.ts:1359` both wired `#report-next-day` and each applied `nextDay`, so driving went Day 1 → Day 3 in one press and Day 1 → Day 5 in two — the even days were unplayable~~ |
| DR-14 | recovery | **Back** | Returns to the Simulation tab | ✅ run (drive: Back returned to the Simulation tab) |
| DR-15 | alternate | A building the reader drew | Graded without pretending it banks anything | ✅ test |
| DR-16 | alternate | <kbd>⌘</kbd>/<kbd>Ctrl</kbd>+<kbd>Enter</kbd> | Closes the shift | ✅ run (drive: closed the shift and opened the sheet) — but **it is documented nowhere on screen**; there is no shortcut list in the interface |
...
| SC-05 | happy | The objective line | Counts what has been banked, **only on the contract you are on**, reads *Cleared* once it has been, and never counts past what the contract asks — the banked count is **clamped on display**, so *2 of 1* cannot be printed | ✅ test (the clamp since the defect-fix lane — [§ D199](../../DECISIONS.md), `2a2cdc0`: `scenariosPanel.test.ts` pins *1 of 1 banked* on a contract `cleanRun` kept counting past; display-only in `contractLineFor` and `objectiveOf` both, the week's own count untouched) |
...
| SC-08 | alternate | Take a card | The week restarts on that contract's building and what was cleared is kept | ✅ test for the transition (`week.test.ts`) · ✅ run for the click (drive: taking a card restarted the week on that contract’s building) |
| SC-09 | alternate | The sixth *build your own* card | Opens the building editor on a blank document | ✅ run (drive: the *build your own* card opened the Building tab editing a blank *My building*) — its copy is inline in `mountScenarios` and reaches only `honesty/derive.ts`'s **static** sweep, which `derive.test.ts` states as *"a limitation, not coverage"*; `startOwn` has no test |
...
| Id | Class | Scenario | Expected | Mark |
|---|---|---|---|---|
| RR-01 | happy | The four segments | Exactly one shown and given the tabindex; arrows wrap; <kbd>Home</kbd>/<kbd>End</kbd> are the ends | ✅ test (`surfaces.test.ts`) · ✅ run (drive: the four segment clicks showed exactly one panel each; the arrows wrapped the ring; Home/End hit the ends) |
...
| RR-08 | happy | Pick an item in any of the four lists | The run changes | ✅ run (drive: a dispatcher pick, a traffic pick and a building pick each changed the run’s figures; a machines pick changes the editing target rather than the run, which is `ME-07`’s wiring fact and not a defect) — the *effect* is also covered downstream (`state.test.ts`, `authoring/authoring.test.ts`) |
| RR-09 | alternate | The four `Open … editor →` buttons | Reveal the contextual tab, open it, and move focus to it | ✅ run (drive: all four buttons revealed the tab, opened it and moved focus to it) |
...
| RR-11 | alternate | The drawer | The same rail as an overlay, with a toggle labelled by what pressing it will do — and the toggle **stacks above the overlay it closes**, because the handoff ships no close control inside the drawer, so the toggle *is* the close control | ✅ test — the label (`drawerStateFor`), and the stacking since the defect-fix lane ([§ D199](../../DECISIONS.md), `3f8dd8c`): the handoff's own close-control treatment decides the fix — its drawer labels the toggle *Close controls* while open and ships no close control inside the drawer — so the toggle takes `position: relative; z-index: 21` against the overlay's `20` in the same 1339 px block, **no chrome added that the handoff does not have**, and `surfaces.test.ts` pins both stylesheet z-indexes parsed rather than string-matched · ✅ run (re-driven at 1200 px: a real pointer hit-test pressed *Close controls* and the drawer closed — the same hit-test that timed out before the fix; <kbd>Escape</kbd> untouched) · *History:* ~~🔲 **driven 2026-07-30 — defect filed: the open drawer covers its own toggle.** Below 1340 px the overlay (`z-index: 20`, `width: min(var(--rail-right), 90vw)`) lay over the strip’s right end where `#drawer-toggle` sits, so the button labelled *Close controls* could not be pressed — a pointer-only reader could not close what they opened; <kbd>Escape</kbd> was the only way out and no close control exists inside the drawer~~ |
...
| DE-09 | happy | Name, **Copy current**, dirty flag, Save, Close | The working copy is named, seeded from the current profile, marked dirty and saved | ✅ run (drive: opened seeded from the current profile, **Copy current** re-seeded it, a weight move showed the dirty flag, the edits survived a close-and-reopen, Save cleared the flag and grew *your own* to 1, Close returned to the run) |
...
| TE-08 | happy | Name, dirty, Save, Close, error | | ✅ run (drive: a parameter move showed the dirty flag, Save cleared it with an empty error slot, Close returned to the run) · ⚠️ (the name field and a *refused* save were not exercised) |
...
| ME-06 | happy | Name, dirty, Save, Close, error | | ✅ run (drive: renamed — the *Editing* line followed — a row move showed the dirty flag, Save cleared it, Close returned to the run; a rename alone is deliberately not dirty — `machineIsDirty` compares the engineering fields) · ⚠️ (the error slot) |
| ME-07 | happy | **A machine-class change reaches the run** | Fitting a different class changes the legs | ✅ test — **closed in wave 12, lane T** ([§ D191](../../DECISIONS.md)): *"the machines editor is not decoration"* now sits beside the three sibling suites and **drives the fit path, which is the wiring fact to keep**: `resolveCar` prefers the car's own `ratedSpeedMps`/`ratedLoadLb`, so a class's rated speed and capacity reach a run **only through the building editor's fit** (the class-chip `onPick` snaps speed to the class typical and clamps load into the class range); only acceleration and jerk reach `resolveCar` directly. **Do not re-test the fit-less `specsWithClass → resolveBuilding` path — it would produce a false *inert* finding about a control that works.** The speed arm runs on Garden Apartments (3.0 m pitch, a measured 11 % round-trip change); Midtown Office is the named speed negative control. Every arm asserts non-empty legs; the unchanged-spec arm asserts bit-identity · ✅ run (drive: fitting Gearless traction on Garden Apartments through the building editor and picking the saved building in the rail moved AWT 15.0 → 8.7 s) · *History:* ~~⚠️ — **this is the one editor § D177's rule does not cover**~~ — its four assertions were round-trip, clamp, non-mutation and parseability, every one about the **spec**, none about a **run** |
...
| BE-21 | happy | The chrome | Add / remove zone, add a credential group (button **and** <kbd>Enter</kbd>), level occupancy, clear ranges, start from blank, Save, Close, dirty, error | ✅ run (drive: add zone drew a chip; a credential group added by button **and** by <kbd>Enter</kbd>; *start from blank* opened *My building*; Save validated and cleared the dirty flag; the coverage matrix drew) · ⚠️ (*level occupancy* and *clear ranges* are conditional chips this pass never surfaced; the error slot) |
...
| BE-23 | alternate | The document editor | Kept whole beneath the elevation, in `<details id="building-document">` — the only surface that can express a floor range, per-floor ids and the access zoning the block above now also edits | ✅ run for the **route** (drive: the `<details>` opened beneath the elevation, a shipped building opened into it, an occupancy edit read *edited · 1 undo step*, Undo read *unchanged*, and **Run this building** handed a changed run back — the figures moved); the rows themselves are the retired § 4's `ED-01`…`ED-25`, whose marks were established against the old shell and **have not been re-established here**. `dev/editor.ts` resolves its own thirty-two ids, which is why `elementMap.test.ts`'s *"ids the viewer never resolves"* list is not empty and is asserted exactly |
| BE-24 | alternate | **Open the machine editor →** from the spec column | Reveals and opens the Machines tab | ✅ run (drive: the button revealed and opened the Machines tab and moved focus to it) |
...
| IS-04 | happy | Compare's controls, progress, cancel and error slot | | ✅ run (drive: a 6-replication 900 s batch ran to a report with common random numbers held and the two arms different) · ⚠️ (cancel and the error slot; `mountBatchPanel` stays excluded from the honesty search as DOM-bound) |
| IS-05 | happy | Campaign | Seven stages in order, one per measured scenario; goals a **subset and a superset** of the measured table, so none can be added or dropped by hand; briefs free of probability words at load time; fail states in R4's order; the editable weight vector runs a profile `data/` does not contain; the holdout half is carried, including the stage that **does not survive** it | ✅ test (`campaign/campaign.test.ts` with sixteen negative controls; the edited vector and its holdout half are `campaign/stageTwoEdited.test.ts` since #356 split the file) |
| IS-06 | happy | Campaign's panel and weights bar | | ✅ run (drive: the brief drew on load and a 50-replication stage ran to an honest *stage not cleared* verdict naming its seed) · ⚠️ (the weights bar) |
...
| IS-09 | happy | The Parameters form's mount | | ✅ run (drive: 11 sources offered, 94 controls drawn from the schemas, the status line counting the dimensions) |
...
| Id | Class | Scenario | Expected | Mark |
|---|---|---|---|---|
| MD-01 | happy | The header select | Two options, the handoff's labels over `mode/`'s values | ✅ run (drive: exactly `basic`→*Casual* and `advanced`→*Engineer*, values the model's and labels the design's) |
| MD-02 | alternate | The remembered mode | Persisted to `localStorage`, **unless the link named one** — a deep link is somebody sending a finding, and a remembered preference that overrode it would show the recipient a different page without either of them knowing | ✅ run (drive: engineer survived a reload via `localStorage`, and a fresh tab’s `?mode=advanced` won over a remembered *Casual*) |
| MD-03 | edge | `?mode=advanced` | The **select follows the state**, so the page is not in engineer mode with its own control reading *Casual* | ✅ run (drive: on `?mode=advanced` the select read *Engineer* and the nameplate was up) — the bug this guard exists for is exactly the kind that has shipped here before |
...
| Id | Behaviour | Mark |
|---|---|---|
| KX-01 | ⛔ Every control reachable by <kbd>Tab</kbd> in visual order; a skip link is the first stop | ✅ run for the first stop (drive: the first <kbd>Tab</kbd> on a fresh page landed on *Skip to the building*) · ⚠️ (the full visual-order walk was not exhaustively driven) |
| KX-02 | ⛔ Focus ring always visible, never removed without a replacement | ✅ run (drive: a keyboard-focused control showed a 2 px solid outline; the ghost file-input label carries the ring the hidden input would have drawn) |
| KX-03 | <kbd>Space</kbd> toggles play/pause | ✅ run (drive: toggled Play ↔ Pause from page focus) |
| KX-04 | ⛔ Typing in an `<input>`, a `<textarea>` **or a `<select>`** never triggers a shortcut, **and <kbd>Space</kbd> over a focused control that activates on it is that control's** | ✅ run (drive: <kbd>Space</kbd> typed into `#seed` left playback alone) — the typing guard is three `instanceof` checks at the top of the window handler · ✅ run for the activation half — **added 2026-08-05 ([§ D234](../../DECISIONS.md), issue #69)**: the <kbd>Space</kbd> arm called `preventDefault()` unconditionally, which **cancels a focused `<button>`'s own activation**, so <kbd>Space</kbd> over `#step-back` toggled playback and did not step a frame. `spaceBelongsToFocus` keys on `(tagName, role)` rather than on a constructor — a `<div role="button">` owns the key and an `<a>` does not — and `role="slider"` is deliberately excluded, so the focused timeline still gives play/pause (drive: `keyboard.browser.test.ts`, the playhead moved and the transport did not) |
| KX-05 | <kbd>,</kbd>/<kbd>.</kbd> step one display frame, pausing first | ✅ run (drive: one display frame each way, pausing first) · **re-driven 2026-08-05 against issue #69's claim that it does nothing, and the row stands**: `,` moved the playhead `5.15 % → 5.10 %`, the same distance `#step-back`'s own click moves it ([§ D234](../../DECISIONS.md)). What the issue found is that **nothing on the page could show it** — one display frame at the shipped ×60 is one simulated second, under half a pixel of timeline, against an `hh:mm` readout. The timeline slider's `aria-valuetext` is `hh:mm:ss` now, so the step the tooltip advertises is observable where the promise is made |
| KX-06 | <kbd>[</kbd>/<kbd>]</kbd> walk the speed ladder, clamped at both ends | ✅ run (drive: walked ×60 → ×240 and clamped at ×900 and at ×1) |
| KX-07 | <kbd>⌘</kbd>/<kbd>Ctrl</kbd>+<kbd>Enter</kbd> closes the shift | ✅ run (drive: closed the shift) — and still undiscoverable; see `DR-16` |
| KX-08 | <kbd>←</kbd>/<kbd>→</kbd>/<kbd>Home</kbd>/<kbd>End</kbd> walk the tab strip and the rail segments, over the **visible** ring | ✅ test for both rings (`surfaces.test.ts`) · ✅ run for the two listeners (drive: both rings walked, wrapped and answered Home/End) |
| KX-09 | <kbd>←</kbd>/<kbd>→</kbd> on the focused timeline (`role="slider" tabindex="0"`) seek | ✅ run (drive: the focused timeline’s arrows sought) |
| KX-10 | <kbd>←</kbd>/<kbd>→</kbd> seek ∓5 s and ∓60 s with <kbd>Shift</kbd>; <kbd>Home</kbd>/<kbd>End</kbd> jump to the run's ends | ✅ test — **built in wave 12** ([§ D188](../../DECISIONS.md)): a pure `seekActionForKey(key, shiftKey)` — ∓5 s, ∓60 s with <kbd>Shift</kbd>, <kbd>Home</kbd>/<kbd>End</kbd> to the run's ends — **deliberately distinct from `KX-09`'s speed-relative frame step**, with an `event.defaultPrevented` guard so the timeline's arrows, the tab strip and the rail segments do not double-fire · ✅ run (drive: <kbd>End</kbd> → 06:30, <kbd>Home</kbd> → 06:00, <kbd>→</kbd> +5 s, <kbd>Shift</kbd>+<kbd>→</kbd> +60 s, with no double-fire from the strip or the timeline) · *History:* ~~🔲 **not built in this shell.**~~ The retired `KB-04`/`KB-05` had no successor: no global arrow handler and no <kbd>Home</kbd>/<kbd>End</kbd> handler |
| KX-11 | <kbd>Escape</kbd> dismisses the drawer | ✅ test — **built in wave 12** ([§ D188](../../DECISIONS.md)); see `SH-12` for the mechanism and the deliberate column-mode inertness · ✅ run (drive: dismissed the reader-opened drawer with focus returned to the toggle; inert in column mode) · *History:* ~~🔲 **not built**~~ |
| KX-12 | ⛔ After an error, focus moves to the error message so a screen reader announces it | ✅ run for the load-failure site (drive: focus landed on the `role="alert"` region naming the failing path) · ⚠️ — the `failRun` and recording-load focus moves were not probed, and the editors' own error slots are separate elements |
| KX-13 | ⛔ Modal dialogs trap focus and restore it on close | ✅ run (drive: opening another building over a dirty document raised the dialog, three <kbd>Tab</kbd>s stayed inside it, and Cancel kept the edits with focus restored to the page) — a native `<dialog>.showModal()`, so the trap is the platform’s |
...
| KX-17 | The roving tabindex never lands on a hidden tab | ✅ test · ✅ run (drive: the roving 0 never sat on a hidden tab) |
...
|---|---|---|---|
| RX-01 | ≥ 1340 px | Three columns: left rail, stage, right rail | ✅ run (drive at 1500×950: `296px 840px 364px`) |
| RX-02 | < 1340 px | Two columns; the right rail becomes an absolutely positioned overlay drawer, **closed**, and `#drawer-toggle` appears | ✅ test — `surfaces.test.ts` reads `index.html` and asserts the `@media` rule matches the constant, which is what stops the two drifting · ✅ run for the layout (drive at 1339: two columns, the rail an absolute closed overlay, the toggle shown) |
| RX-03 | < 768 px | Controls stack; the canvas keeps at least 60 % of the height | ✅ test — the 767 px block is pinned by `surfaces.test.ts` (flex column, stage ordered first, `.stage-wrap` at a definite 60vh) · ✅ run (**driven, fixed and re-driven 2026-07-30, drive phase, commit `5d4b782`**: at 375×667 the old layout held a 236 px left rail and gave the canvas **0 %** of the height; after the fix 375×667, 414×896 and 767×700 all stack stage-first with the canvas at 60 % of the viewport height and no horizontal scroll, and the grid returns untouched at 768) · *History:* ~~🔲 **not built.** The 768 px query restyled three things and `.body` kept a **236 px left rail at every width**; found by reading, confirmed by driving~~ |
| RX-04 | < 1180 px | The building's **spec line** and the (unwritten) banner step aside — the name outranks the spec. The **phase pill** and the **mode select** do not: [§ D236](../../DECISIONS.md) took both out of `[data-hide-narrow]`, the pill because `FILLING`/`PEAK`/`EASING`/`DRAIN` is the only statement of what the building is doing at the playhead, and the select because `display: none` gives a zero-size box — it left the **tab order**, and no other surface in the product changes Casual/Engineer, so below 1180 px a reader was locked into whatever `localStorage` last held (issue #72) | ✅ test (`shellChrome.test.ts` asserts the attribute on the spec line and its absence on the pill, the select and the select's label) · ✅ run (drive at 1100 × 800: the select is 201 px and takes focus, the pill is 49 px, the spec line is gone, nothing past the right edge) |
| RX-04b | < 768 px | The header **wraps** rather than clipping: the brand, the building and the state block take a row each | ✅ test (the 767 px block's two `flex-basis: 100%` rules, and `flex-wrap: wrap` on `.topbar` and `.topbar-right`) · ✅ run (drive at 375 × 812: `scrollWidth === clientWidth === 375`, zero descendants past the edge, against a **516 px** `scrollWidth` inside a 375 px clip before — 141 px of header, including the clock and the day, that no gesture could reach; issue #74) |
...
| RX-07 | More shafts than fit | *Showing N of M*, never a silent truncation, and a list with no room at all collapses to one line naming what it holds. Since [§ D236](../../DECISIONS.md) the notice also **names the remedy the reader has** — the `bank` select — rather than *"widen the window"*, which is false on the flagship building at every desktop size, and it is the longest of three forms that fits rather than one form truncated | ✅ test · ✅ run (drive: `showing 27 of 35 shafts — pick one bank below, or widen the window` at a 1232 px canvas, `showing 10 of 35 … pick a bank below` at 504, `7 of 35 shafts — pick a bank` at 360 — where it had been a bare `…`) |
| RX-12 | Any width | The plot keeps **45 %** of the canvas: a caller's gutter request yields to it, the overlay first, then the right gutter, then the left, each to its own floor | ✅ test (`layout.test.ts` — at 360 px the shipped request is 16 px *wider than the canvas*, and two `Math.max(1, …)` guards turned that into a **one-shaft** picture of every building on every phone; issue #73) · ✅ run (drive at 375: two of two shafts on Garden Apartments, six of six on Chancery House) |
| RX-08 | Resize during playback | Relayout without pausing or jumping the playhead — the layout is rebuilt per frame | ✅ run (drive: a resize mid-playback kept playing and the playhead advanced without a jump) |
| RX-09 | Crossing 1340 px | The drawer's state changes at the same instant the layout does — a `matchMedia('change')` listener, because `resize` alone left the rail as a stale overlay needing **two** presses of the toggle | ✅ run (drive: crossing 1339/1340 in both directions kept the drawer state coherent with the layout — one press opened, the remembered choice survived the round trip, and the two-press symptom is gone) |
| RX-10 | `devicePixelRatio` 2–3 | Crisp, capped at ×2 | ✅ run (drive at `deviceScaleFactor` 3: the bitmap was 2× the CSS size — capped) |
| RX-11 | Print / screenshot | The frame exports as PNG carrying its seed, its clock **and its reporting window** | ✅ test for the header's content · ✅ run for the export (drive: a real PNG downloaded, named by building and seed) |

---

## 25. Ledger — where the 220 rows stand
...
| ✅ **test** — the whole row asserted, the assertion located and read, and the drive did not reach it | **107** | `BE` 21 · `LR` 17 · `DR` 10 · `DE` 9 · `SG` 8 · `TE` 7 · `IS` 6 · `MD` 6 · `RR` 6 · `SC` 6 · `ME` 5 · `KX` 2 · `RX` 2 · `SH` 1 · `TP` 1 |
...
**220 rows. 87 driven green, 107 fully asserted by test, 19 half established, 7 unverified, 0 driven red.**

> **Re-derived 2026-07-30 (wave 12, defect-fix lane), after § D199's fixes landed.** The table read
> *81 / 106 / 20 / 8 / 4* until this pass. All four driven-red rows closed — `TP-08`, `DR-13` and
> `RR-11` to `✅ test` + `✅ run`, each fixed, pinned and re-driven in Chromium; `TP-10` to
> `✅ test`, the writer moved to the reader, with the browser adopt through the file input still
> `TP-11`'s `⚠️`. `SH-09` lost its residual `⚠️` (boot writes `?seed=…` the moment it completes),
> `SH-19` moved `⚠️` → `✅ run` with its false markup-literal claim corrected in place, and one row
> was **added** — `SH-21`, the favicon — because [§ D198](../../DECISIONS.md) recorded that *no
> ledger row covered the favicon; the absence is the finding*, so 219 rows became 220. Counted
> mechanically from §§ 9–24's own mark cells on 2026-07-30 (struck-through history spans excluded;
> a row whose clauses are all `✅` with at least one `✅ run` counts as run; any `⚠️` beside a `✅`
> counts as half; `🔲` outranks everything).

> **Re-derived 2026-07-30 (drive phase), after § 26 was executed in a real browser.** The table
> read *0 / 118 / 47 / 53 / 1* at the start of the drive. Chromium via Playwright against the dev
> server drove § 26 items 1–21 top to bottom plus the primary flows end-to-end; every `✅ run`
> below carries its one-line evidence in the row. `RX-03` — the last of the seven wave-open `🔲`
> ids — was reproduced at 375×667 (236 px rail, canvas at 0 % height), fixed in `5d4b782`, and
> re-driven green. **The four `🔲` ids above are new, found by this drive and filed rather than
> fixed** — which is the fourth consecutive pass in which driving found defects that reading and
> the suite had both passed: two listeners on one button (`DR-13`), a save/load pair that refuses
> its own artifact (`TP-10`), a provenance field that silently rewrites the seed (`TP-08`), and an
> overlay that covers its own close control (`RR-11`). Counted mechanically from §§ 9–24's own
> mark cells (struck-through history spans excluded; a row whose clauses are all `✅` with at least
> one `✅ run` counts as run; any `⚠️` beside a `✅` counts as half; `🔲` outranks everything).

> **Re-derived 2026-07-30 (wave 12), after the six 🔲 fixes landed.** The table read *117 / 40 / 55
> / 7* until this date; the seven-id 🔲 list it carried — `SH-09` · `SH-12` and `KX-11` · `SG-15` ·
> `TP-13` · `KX-10` · `RX-03` — lost six to wave 12's fixes ([§ D187](../../DECISIONS.md)–[§ D190](../../DECISIONS.md)),
> each now `✅ test` on its mechanism with the listener/browser half still `⚠️` and the § 26 drive
> owed, and `ME-07`/`CO-02` moved on [§ D191](../../DECISIONS.md). Counted mechanically from
> §§ 9–24's own mark cells (struck-through history spans excluded) on 2026-07-30.

The five bucket counts above were **derived from this file's own tables**
...
2. **`dev/main.ts` holds all the event wiring and `main.test.ts` covers one exported function.**
   ...
   `⚠️` column. *(Wave 12: `applyDeepLink` now has its first tests, via the pure `deepLinkStateOf`
   it was refactored around — [§ D189](../../DECISIONS.md); the rest of the sentence stands.)*
3. **Seven `🔲` rows, and three of them were found by reading in this pass** — `SG-15`, `SH-09`,
   ...
   of a handler (`KX-10`). *(Wave 12: `SG-15` was then confirmed inert **by driving** before being
   fixed — the reading was right — and six of the seven are now built and tested with the browser
   re-drive owed; `RX-03` is the remaining `🔲`, deferred to the drive phase.)*
...
> **Executed 2026-07-30 (drive phase), Chromium via Playwright, top to bottom.** Every item below
> carries its outcome at the end of its last cell. Four defects were found and filed rather than
> fixed (`TP-08`, `TP-10`, `DR-13`, `RR-11` — §§ 13, 12, 14 and 16 hold the rows); `RX-03` was
> reproduced, fixed in `5d4b782` and re-driven. The full evidence per row is in §§ 9–24. *(All four
> filed defects were then fixed and re-driven the same day in the defect-fix lane —
> [§ D199](../../DECISIONS.md); the "is now 🔲" outcomes below are the drive's record, kept as
> written, and the rows above carry the closures.)*

| # | Drive this | Rows | Why it is here |
|---|---|---|---|
| 1 | **The bank filter.** Pick a bank on `vertical-city` (35 shafts, 4 banks) and watch the stage | `SG-15` | Suspected **inert** from reading — **confirmed inert by driving, then fixed in wave 12 ([§ D187](../../DECISIONS.md)) — re-drive to confirm.** The retired `RS-05` is re-marked · **Driven — PASS.** Paused, sha-256 over the bitmap: all eight banks distinct, `(all)` byte-identical on return |
| 2 | **The URL.** Change building, seed, tab and rail; read the address bar; reload | `SH-09`, `SH-08` | Suspected **not built** — confirmed, then **fixed in wave 12 ([§ D189](../../DECISIONS.md)) — re-drive to confirm.** `applyDeepLink` now has tests; the browser round trip (write, copy, paste, reload) is still the half only a browser can check · **Driven — PASS, one defect.** The round trip held and the reload was bit-identical; the opening seed never reaches the bar until the first interaction (filed on `SH-09`) |
| 3 | **The machines editor's effect on a run.** Fit a different class, run, compare the legs | `ME-07`, `ME-06` | The only one of the four editors with no *"the control changes the run"* test — **test landed in wave 12 ([§ D191](../../DECISIONS.md))**; `ME-06`'s chrome still needs the browser. § D177 found three inert or wrong controls in the other three · **Driven — PASS.** The fit path reached a run in the browser: Gearless traction on Garden Apartments moved AWT 15.0 → 8.7 s via the rail pick |
| 4 | **The narrow layouts.** 1339, 1179, 899 and 767 px, plus a crossing in each direction | `RX-03`, `RX-01`, `RX-04`, `RX-09`, `SH-10` | `RX-03` is suspected **not built**. `RX-09` is a defect the code documents having fixed and nothing asserts — and the symptom was a rail needing *two* presses to close · **Driven — RX-03 reproduced (375×667: 236 px rail, canvas 0 %), fixed in `5d4b782`, re-driven green at 375/414/767.** Crossings clean both ways; found instead: the open drawer covers its own toggle (filed on `RR-11`) |
| 5 | **The out-of-service badge.** Click one under a shaft; confirm the car leaves and the shift re-runs | `SG-06`, `SG-07` | The only pointer target on the canvas. The hit rectangle is asserted; the handler that consumes it is not, and it re-runs the whole shift · **Driven — PASS** in both directions, with the state named in the text alternative |
| 6 | **The coach ribbon's two untested selects.** Change the pattern, then the shift length; compare the legs both times | `CO-02`, `CO-03`, `CO-01`, `CO-04` | The controls that decide what runs, held to a lower standard than any editor control. ~~`shiftRunConfigOf` asserts the *negative* half only~~ — `CO-02`'s positive half landed in wave 12 ([§ D191](../../DECISIONS.md)); the `change` listeners are still the browser's to check · **Driven — PASS.** Both selects re-ran the run; returning to the building’s own demand reproduced it bit-identically |
| 7 | **The transport's empty and error states.** Reload with `data/` moved aside; then a run that throws; then an editor hand-over after a failed run | `TP-16`, `TP-17`, `SH-15`, `SH-16`, `SH-19` | This exact pair was found **false twice** in the retired ledger — enabled controls with no listeners, and a disabled transport after an editor hand-over · **Driven — PASS** via blocked fetches (path named, focus moved, transport really disabled, campaign failure isolated). A run that *throws* could not be constructed through the UI — the editors validate first — so `TP-17` stays undriven |
| 8 | **Retry.** Restore the moved file and press Retry | `SH-17` | `RV-21` was false when first driven and killed the page silently. `bootstrap.test.ts` now covers the state machine; the button and the focus move are still unverified · **Driven — PASS.** Refetched with no reload, started exactly once |
| 9 | **The seed field.** Type a non-numeric seed | `TP-08` | `BigInt(raw.replace(/\D/g,'') \|\| '0')` silently yields **seed 0**. A provenance control that quietly changes the run · **Driven — the hazard is real and filed.** `banana` silently became seed 0 (`TP-08` is now 🔲) |
| 10 | **`copy run`.** Copy on a non-default pattern and on day 3; paste into the CLI | `TP-13` | ~~Known unmet~~ — **fixed in wave 12 ([§ D190](../../DECISIONS.md)) — re-drive to confirm**: the line should now carry `--traffic`/`--template`, and day 3 should **refuse with a named reason** rather than copy · **Driven — PASS on both halves**: the flags line on a non-default pattern, the named refusal on a later day with the clipboard untouched |
| 11 | **The right rail's picks and its four `Open … editor →` buttons** | `RR-08`, `RR-09`, `SH-05`, `BE-24` | The handoff's **only** route to the four contextual editors. If `openTab` misfires, four surfaces are unreachable · **Driven — PASS.** Three of the four picks change the run; the machines pick changes the editing target, which is `ME-07`’s wiring fact |
| 12 | **The tab strip.** Click each; arrow through it before and after revealing an editor; reload on a deep-linked tab | `SH-02`, `SH-03`, `SH-04`, `KX-08`, `KX-17` | The decision is well tested and the writer is not; a roving tabindex is a rule with an off-by-one in it · **Driven — PASS**, including the ring at rest skipping unrevealed editors and the deep-linked reload |
| 13 | **The day-close paths.** Let a shift end; open the sheet by hand; **Take the next assignment**, **Open the doors on tomorrow**, **Back**, and <kbd>⌘</kbd>+<kbd>Enter</kbd> | `DR-01`, `DR-02`, `DR-12`, `DR-13`, `DR-14`, `DR-16` | Two entry points into `closeShift`, a `filedRunId` guard, and three buttons that patch the week · **Driven — one defect, the rest PASS.** `#report-next-day` has two listeners and advances two days per press (`DR-13` is now 🔲) |
| 14 | **The rest of the transport.** Play, pause, both step buttons, click-to-scrub, the loop chip, each speed chip, Verify, Save, Load, Export | `TP-01`–`TP-07`, `TP-09`–`TP-12`, `TP-18` | Ten controls; every underlying module asserted, every button unverified · **Driven — PASS except the round trip**: Save writes a document Load refuses (`TP-10` is now 🔲); Verify, scrub, steps, loop, speeds, PNG export and the three load refusals all held |
| 15 | **Keyboard and focus.** <kbd>Space</kbd>, <kbd>,</kbd>/<kbd>.</kbd>, <kbd>[</kbd>/<kbd>]</kbd>, typing in a field, the timeline's arrows, focus after an error, the confirm dialog, <kbd>Tab</kbd> order, <kbd>Escape</kbd> | `KX-01`–`KX-07`, `KX-09`–`KX-13` | Four ⛔ rows are in this block. ~~`KX-10` and `KX-11` are expected to confirm as **not built**~~ — both **built in wave 12 ([§ D188](../../DECISIONS.md)) — drive to confirm**, including <kbd>Escape</kbd>'s deliberate inertness in column mode · **Driven — PASS** (first Tab, Space, the guard, `,`/`.`, `[`/`]` clamped, `KX-10`’s fixed-second seeks, Escape both modes, the confirm dialog’s trap); `KX-01`’s full order walk and two of `KX-12`’s three sites stay ⚠️ |
| 16 | **The mode toggle.** Switch, reload, then open `?mode=advanced` in a fresh tab | `MD-01`, `MD-02`, `MD-03`, `MD-07` | The remembered-mode-versus-deep-link rule and the select-follows-state fix are both unasserted · **Driven — PASS**: remembered mode survives reload, the deep link wins, the select follows the state |
| 17 | **The honesty disclosure and the decision log**, on a suppressed run and a clean one | `LR-15`, `LR-22`, `SG-11` | The one control in the left rail, and the log the prototype's own rule made inert · **Driven — PASS** on a clean run and on a run refused on the abandonment-horizon ground, quoted verbatim with no digit anywhere |
| 18 | **The four editors' chrome.** Name, dirty, Save, Close, error, and the building editor's zone / group / occupancy buttons | `DE-09`, `DE-11`, `TE-08`, `ME-06`, `BE-21`, `SC-09` | Every editor's **model** is asserted; none of their save paths is · **Driven — PASS** for name/dirty/save/close on all four plus the building’s zone, group (button and Enter) and blank paths; no editor’s *error* slot could be provoked |
| 19 | **The three retained instruments.** Run a Compare, run a campaign stage, open Parameters | `IS-04`, `IS-06`, `IS-09` | Three mounts excluded from the honesty search by construction · **Driven — PASS**: a 6-replication Compare batch with CRN held, a 50-replication campaign stage to an honest verdict, Parameters mounted with 94 controls |
| 20 | **The document editor, re-established.** Open Building → the `<details>` block and walk the retired § 4 | `BE-23`, and the retired `ED-01`…`ED-25` | Twenty-five rows whose marks belong to a shell that no longer exists, describing a surface that still ships · **Driven — the route and the chrome PASS** (open, edit, undo, discard-confirm, *Run this building* changes the run); the full `ED-01`…`ED-25` walk is not re-established and those marks stay retired |
| 21 | **The degraded and empty states.** A building with no demand; a campaign file that will not load; a canvas with no 2D context | `SG-17`, `SG-18`, `SH-19`, `SH-20`, `CO-05`–`CO-07`, `SC-07` | `SG-17` lost its text fallback in the rewrite; `SH-19` is nearly unreachable because the shell runs on load · **Partially driven**: the campaign-file failure is isolated to its own slot (`SH-20` ✅ run); `SG-17` (no 2D context) and a no-demand building are not constructible in this harness and stay ⚠️ |
...
| PB-09 | alternate | Window selection then loop | Only the selected span repeats | ✅ test — **built, and not yet driven in a browser**, which is why this is one tick and not two. `Playback#setLoop` takes a `LoopWindow` on a live transport and `#advance` wraps at the window's `toS` rather than at `endedAt`; `playback.test.ts` § *the loop window* drives sixty frames of a quarter-second each round a two-second window and asserts the playhead never leaves it. `#loop-window`, the `A–B` chip beside `#loop`, is the selection — three presses, `aria-pressed` `false` → `mixed` → `true`, over `playback/loopMark.ts#nextLoopMark`. **The row's old note was wrong about the thing it was describing**: it said the transport is *"rebuilt at the current instant"*, and `adopt` passes no `startAtS`, so pressing `loop` at 07:30 rebuilt it at **06:00** — the playhead, the landing selection and the day-filing gate all went with it. That is fixed by the same change and pinned by `main.test.ts` § `PB-09` |
...
| ED-23 | recovery | Leaving with unsaved edits | Warned before navigation | 🔲 **not built** for the browser's own leave prompt · ✅ run for the in-app half: opening another building, starting from blank or importing over unsaved edits each ask first. **The ⚠️ was wrong, and in the opposite direction from the one anybody was looking in.** *Unverified* says *built, reachable, neither driven nor tested*; `grep -rn beforeunload packages/*/src` returns **nothing**, and neither does the same grep over the whole tree outside this file — so there is no handler to have driven. `ISSUE_VERIFICATION_FINDINGS.md` had found the same absence from the other end while reading issue #113 (*"There is no `beforeunload` handler anywhere in `packages/viz/src/dev`"*) and the two sentences never met. **Whether to build one is an open product question and not a decision this row has taken**: the page opens on Everyday Mode ([§ D335](../../DECISIONS.md)), so a page-wide unload prompt driven by the Engineer editor's dirty state reaches past the editor, and `ED-19` makes *unsaved* the ordinary state of anyone who has edited without downloading |
...
|---|---|---|---|
| RS-01 | ≥ 1340 px wide — **corrected from ≥ 1280 px**, see the note | Full layout: **both rails are columns and nothing is displaced.** The contents clause is unchanged and moved into the note, because it does not distinguish this band | ✅ w1, for the contents clause · **Corrected 2026-08-25 — the ≥ 1280 px boundary is withdrawn**, which is the annotation `RS-02`'s note asked for and deliberately left standing as outside issue #256's scope. It was the same deleted rule: `@media (max-width: 1279px) { .wide-only { display: none; } }`, gone at `22a1021` with the class it hid. The real edge above which nothing is displaced is **1340 px** — `@media (max-width: 1339px)` turns `.rail-r` into an overlay drawer, and `dev/surfaces.ts#DRAWER_BREAKPOINT_PX` is `1340`; the two are pinned in step by `packages/viz/src/render/viewportClaims.test.ts`, which asserts `DRAWER_BREAKPOINT_PX - 1` is a breakpoint `index.html` declares. **The two rows had to be settled together, and that is why #256 left them**: renumbering `RS-02`'s upper edge to 1339 while this row still said ≥ 1280 would have overlapped the two bands across sixty pixels. **What this row no longer claims is the half worth reading.** All shafts, floor labels, landing counts and per-car status are drawn on the **canvas**, which no width rule touches, so that clause is equally true inside `RS-02`'s band and never separated the two. Layout is what separates them. **Settled 2026-08-29 under [§ D405](../../DECISIONS.md): the withdrawal itself is [§ D352](../../DECISIONS.md)**, which rules the correction and its refusal to name a replacement width; this row is that decision applied to a run mark, and takes none of its own |
| RS-02 | 768–1339 px — **upper edge corrected with `RS-01`; the band is not one regime**, see the note | ~~Per-car status list collapses~~; shafts and landings stay | ✅ run at 1024 px — shafts, landings and the metrics panel stay · **Annotated 2026-08-24 (issue #256): the collapse clause in this run mark is withdrawn.** It read *"the bank filter, landing selector and PNG export collapse below 1280 px"*, and that was **true when it was measured and is not true now** — the *stated mechanism goes stale* class, not a mark that was ever wrong. `index.html` carried `@media (max-width: 1279px) { .wide-only { display: none; } }` and all three controls carried `.wide-only`, until the wave 10 rebuild to the design handoff deleted the rule and the class together and moved the three into `.provenance`, **which no width rule reaches**. So the three controls no longer collapse at any width, and this row's upper bound is design intent with nothing behind it. The page's real width breakpoints are **720, 767, 899, 1179 and 1339 px** (plus `dev/surfaces.ts#DRAWER_BREAKPOINT_PX` = 1340), pinned in both directions by `packages/viz/src/render/viewportClaims.test.ts` so that the next rule deleted takes a test with it. What governs the landing selector instead is **which tab is open** and **whether a landing has been picked** — the argument is in `render/canvas.ts`'s `unansweredCallFloorIds`. **RS-01's ≥ 1280 px inherits the same withdrawn boundary and is deliberately left standing**: it is outside issue #256's scope, and it needs this same annotation. **The withdrawal is [§ D352](../../DECISIONS.md); this row needs no number of its own ([§ D405](../../DECISIONS.md))** · **Settled 2026-08-25, with `RS-01` and not before it.** The upper edge is **1339 px**, the drawer rule's own number, and `RS-01` moved to ≥ 1340 in the same edit — either alone would have left the two bands overlapping or gapped, which is why #256 could not close this half. **Two further things this row was claiming are withdrawn with the edge.** First, the band is **not one regime**: it contains two real internal edges — 1339 (the right rail becomes an overlay drawer) and 1179 (`[data-hide-narrow]` drops the spec line and the banner) — so a single ✅ over 768–1339 is one measurement standing for three layouts, and the 1024 px run sits in the lowest of them. Second, **the *collapse* half of the expectation is struck rather than annotated**: no width rule in `index.html` collapses a per-car status list at any width, and the five that exist do something else entirely (720 `.overnight-row`, 767 stack, 899 left rail, 1179 `data-hide-narrow`, 1339 drawer). The 1024 px run recorded only what **stays**, so the collapse half was never measured — it is design intent, like the upper edge was, and struck on the same ground. **Neither row is derived, and that is a gap rather than a decision:** `viewportClaims.test.ts` derives the breakpoint set from `index.html` and sweeps `.ts` files under `packages/viz/src` for the withdrawn claim, but it does not read this file, and its `WITHDRAWN_CLAIM` is the string `below 1280 px`, which is not how either row spells it. Extending that sweep to `UX.md` is what would make these two rows fail on the next deletion instead of drifting again |
...
| RS-05 | More shafts than fit | Horizontal scroll **or** a bank filter — never silently truncated | ✅ run — `vertical-city` at 1280 px drew *showing 30 of 35 shafts — widen the window*, and the bank filter narrows to a bank. The metrics panel's two lists obey the same rule, and a list with no room at all collapses to one line naming what it holds rather than to "showing 0 of N" · **Annotated 2026-07-30 (wave 12): this run mark was half false when written.** The scroll/caption half was true; the bank-filter clause — *"narrows to a bank"* — was **false of the shipped viewer** in every pass until wave 12, when `SG-15` drove the control inert (byte-identical canvas hash across three banks, playback paused) and [§ D187](../../DECISIONS.md) wired it. **Needs re-drive** (§ 26 item 1) |
...
| ✅ **test** — asserted, and the assertion proved to bite | 15 | `RV-14` · `PB-09` (built by issue #417, **not driven**) · `ED-03 07 08 09 12 13 14 15 16 17` · `ED-17a` (added by `T44`) · `KB-15b` · `RS-04` |
| ✅ + 🔲 — one clause each way | 1 | `ED-23` (in-app half run, the browser's leave prompt **not built**) |
...
| 🔲 **not built** | 0 | — (`PB-09` built by issue #417; `ED-23`'s second clause is in the split row above, where it has a clause that *is* run) |
...
shipped building that produces it, and `ED-23`'s `beforeunload` half was recorded here as *still
unverified*.

> **That last clause was wrong for as long as it stood, and it is worth leaving in place with its
> correction rather than deleting.** `T39`'s whole job was the four rows that were *built and never
> exercised*, and two of them turned out to be **false**. `ED-23`'s second half was neither: there
> is no `beforeunload` handler in this tree and there never was, so it was never a row `T39` could
> have driven. A ⚠️ that should have been a 🔲 does not merely overstate the product — it puts the
> row in the bucket labelled *somebody go and press this*, which is why three waves passed over it.
> Corrected in the row itself and in the ledger above (GitHub issue #417).
