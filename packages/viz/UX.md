...
| TP-08 | invalid | Type into the seed field | A blank draws one and **shows the drawn seed**; a whole number re-runs at that seed; anything else is **refused by name** rather than coerced, and only the literal `0` ever runs seed 0 | ✅ test — **fixed in the defect-fix lane** ([§ D199](../../DECISIONS.md), `815e314`): the parse is the exported `seedEntryOf`, holding the deep-link reader's own rule (`/^\d+$/` or a refusal): a whole number runs as itself, a blank asks for a fresh draw and the drawn seed is written back into the field (a blank field over a drawn seed is the same hazard one keystroke earlier), and anything else refuses on `#status` **naming what was typed** with the field restored to the seed still running — `main.test.ts` pins all three arms, including that only the literal `0` may run seed 0 · ✅ run (re-driven: `banana` refused by name with the run untouched; a blank drew and showed the drawn seed) · *History:* ~~🔲 **driven 2026-07-30 and the hazard is real — defect filed.** Typing `banana` silently re-ran at **seed 0**: the field still read `banana` while the footer read *seed 0*, so a mistyped seed reproduced a different run with no refusal anywhere. The parse was `BigInt(raw.replace(/\D/g, '') \|\| '0')`, so a non-numeric entry became **seed 0** rather than being refused~~ |
