...
| KX-04 | ⛔ Typing in an `<input>`, a `<textarea>` **or a `<select>`** never triggers a shortcut | ✅ run (drive: <kbd>Space</kbd> typed into `#seed` left playback alone) — the guard is three `instanceof` checks at the top of the window handler |
| KX-05 | <kbd>,</kbd>/<kbd>.</kbd> step one display frame, pausing first | ✅ run (drive: one display frame each way, pausing first) |
