...
| SH-12 | recovery | <kbd>Escape</kbd> with the drawer open | The drawer closes | ✅ test — **built in wave 12** ([§ D188](../../DECISIONS.md)): a pure `escapeClosesDrawer(viewportPx, openedByReader)` in `dev/surfaces.ts`, wired into `wireKeyboard`, focus returned to `#drawer-toggle`; **deliberately inert in column mode** — at ≥ 1340 px the rail is a remembered layout choice, not a modal · ⚠️ (the `keydown` listener; **drive owed, § 26 item 15**) · *History:* ~~🔲 **not built — a real gap, found by driving in an earlier pass and confirmed here by reading.**~~ There was no `Escape` handler in `packages/viz/src` or in `index.html`; the toggle was the only way out |
...
| TP-13 | failure | Press `copy run` | One control copies `building, dispatcher, traffic, seed, duration` in a form the CLI accepts | ✅ test — **met in wave 12** ([§ D190](../../DECISIONS.md), `dbfc22e`): the line now emits `--traffic`/`--template`, and the viewer route was **driven bit-identical to the CLI route at 10 of 10 cells** (sha256 over `legs`/`boardedLegs`/`waiting`) rather than assumed equivalent, with one cell re-pinned leg-for-leg as the tripwire; what no flag can express — a saved pattern, any day but the first, held cars, moved group levers, saved buildings or dispatchers — **refuses with named reasons** on `#status` and copies nothing · ⚠️ (the click and the clipboard; **re-drive owed, § 26 item 10**) · *History:* ~~🔲 **unmet, and carried forward verbatim from the retired `RV-T7` and from [`GAPS.md`](../../GAPS.md).**~~ The line read `--building … --dispatcher … --seed … --duration …` and named **no traffic and no day**, so on any non-default pattern, or any day past the first, it reproduced a **different** run — a provenance claim the CLI would honour and the reader could not check |
...
| KX-10 | <kbd>←</kbd>/<kbd>→</kbd> seek ∓5 s and ∓60 s with <kbd>Shift</kbd>; <kbd>Home</kbd>/<kbd>End</kbd> jump to the run's ends | ✅ test — **built in wave 12** ([§ D188](../../DECISIONS.md)): a pure `seekActionForKey(key, shiftKey)` — ∓5 s, ∓60 s with <kbd>Shift</kbd>, <kbd>Home</kbd>/<kbd>End</kbd> to the run's ends — **deliberately distinct from `KX-09`'s speed-relative frame step**, with an `event.defaultPrevented` guard so the timeline's arrows, the tab strip and the rail segments do not double-fire · ⚠️ (the listener; **drive owed, § 26 item 15**) · *History:* ~~🔲 **not built in this shell.**~~ The retired `KB-04`/`KB-05` had no successor: no global arrow handler and no <kbd>Home</kbd>/<kbd>End</kbd> handler |
| KX-11 | <kbd>Escape</kbd> dismisses the drawer | ✅ test — **built in wave 12** ([§ D188](../../DECISIONS.md)); see `SH-12` for the mechanism and the deliberate column-mode inertness · ⚠️ (**drive owed, § 26 item 15**) · *History:* ~~🔲 **not built**~~ |
...
| ✅ **test** + ⚠️ — one clause each way, almost always *the decision is asserted, the DOM writing or the listener is not* | **47** | `SH` 13 · `TP` 9 · `SG` 6 · `CO` 4 · `KX` 4 · `RR` 3 · `RX` 3 · `DR` 2 · `SC` 2 · `LR` 1 |
| ⚠️ **unverified** — built and reachable, neither driven nor covered | **53** | `KX` 10 · `TP` 7 · `CO` 4 · `RX` 5 · `DR` 4 · `MD` 4 · `SH` 3 · `BE` 3 · `IS` 3 · `SG` 2 · `RR` 2 · `DE` 2 · `ME` 1 · `LR` 1 · `SC` 1 · `TE` 1 |
| 🔲 **not built, inert, or failing its own condition** | **1** | `RX-03` (no stacked layout below 768 px) |

**219 rows. 118 fully asserted, 47 half asserted, 53 unverified, 1 not met.**
...
|---|---|---|---|
| 1 | **The bank filter.** Pick a bank on `vertical-city` (35 shafts, 4 banks) and watch the stage | `SG-15` | Suspected **inert** from reading — **confirmed inert by driving, then fixed in wave 12 ([§ D187](../../DECISIONS.md)) — re-drive to confirm.** The retired `RS-05` is re-marked |
| 2 | **The URL.** Change building, seed, tab and rail; read the address bar; reload | `SH-09`, `SH-08` | Suspected **not built** — confirmed, then **fixed in wave 12 ([§ D189](../../DECISIONS.md)) — re-drive to confirm.** `applyDeepLink` now has tests; the browser round trip (write, copy, paste, reload) is still the half only a browser can check |
| 3 | **The machines editor's effect on a run.** Fit a different class, run, compare the legs | `ME-07`, `ME-06` | The only one of the four editors with no *"the control changes the run"* test — **test landed in wave 12 ([§ D191](../../DECISIONS.md))**; `ME-06`'s chrome still needs the browser. § D177 found three inert or wrong controls in the other three |
...
| 10 | **`copy run`.** Copy on a non-default pattern and on day 3; paste into the CLI | `TP-13` | ~~Known unmet~~ — **fixed in wave 12 ([§ D190](../../DECISIONS.md)) — re-drive to confirm**: the line should now carry `--traffic`/`--template`, and day 3 should **refuse with a named reason** rather than copy |
