...
| TP-08 | invalid | Type into the seed field | A blank draws one and shows it; anything else re-runs at that seed | 🔲 **driven 2026-07-30 and the hazard is real — defect filed.** Typing `banana` silently re-ran at **seed 0**: the field still read `banana` while the footer read *seed 0*, so a mistyped seed reproduces a different run with no refusal anywhere. The parse is `BigInt(raw.replace(/\D/g, '') \|\| '0')`, so a non-numeric entry becomes **seed 0** rather than being refused, and a mistyped seed silently reproduces a different run. Unverified, not asserted anywhere |
...
| TP-10 | alternate | Save recording | A JSON document carrying the recording and its serialized frames, named by building and seed | 🔲 **driven 2026-07-30 — defect filed.** Save downloads `{recording, frames}` with no top-level `schemaVersion`, and the shipped Load refuses exactly that document — *“the document has no numeric schemaVersion, so it is not a viz recording”* — so the product cannot reload the file it saved. `record/document.ts` has a `readRecordingDocument` and no writer beside it |
| TP-11 | failure | Load a recording | The schema version is checked **first**; a newer one is refused by name; an **older** one is refused too, because a v2 recording has no `legs` and the overlay would silently report an empty window on it; a truncated file reports its parse position and the previous run keeps playing | ✅ test (`record/document.test.ts`) · ✅ run for the refusals (drive: schema 999 refused by name against the shipped 8; a truncated file named its parse position; the previous run kept playing through both) · ⚠️ (the happy adopt through the file input — no shipped artifact loads; see `TP-10`) |
...
| DR-13 | alternate | **Open the doors on tomorrow** | Advances the day, wraps the weekday, clears the award so a banner belongs to one report, and clears the recording in the **same patch** so the reader never sees yesterday's figures under today's date | ✅ test (`week.test.ts`) · 🔲 **driven 2026-07-30 — defect filed: one press advances TWO days.** `reportPanel.ts:400` and `main.ts:1359` both wire `#report-next-day` and each applies `nextDay`, so driving went Day 1 → Day 3 in one press and Day 1 → Day 5 in two — the even days are unplayable |
...
| RR-11 | alternate | The drawer | The same rail as an overlay, with a toggle labelled by what pressing it will do | ✅ test for the label · 🔲 **driven 2026-07-30 — defect filed: the open drawer covers its own toggle.** Below 1340 px the overlay (`z-index: 20`, `width: min(var(--rail-right), 90vw)`) lies over the strip’s right end where `#drawer-toggle` sits, so the button labelled *Close controls* cannot be pressed — a pointer-only reader cannot close what they opened; <kbd>Escape</kbd> is the only way out and no close control exists inside the drawer |
...
| ✅ **test** — the whole row asserted, the assertion located and read, and the drive did not reach it | **106** | `BE` 21 · `LR` 17 · `DR` 10 · `DE` 9 · `SG` 8 · `TE` 7 · `SC` 6 · `RR` 6 · `IS` 6 · `MD` 6 · `ME` 5 · `KX` 2 · `RX` 2 · `SH` 1 |
...
| 🔲 **not built, inert, or failing its own condition** | **4** | `TP-08` (a mistyped seed silently becomes seed 0) · `TP-10` (Save writes a document Load refuses) · `DR-13` (one press advances two days) · `RR-11` (the open drawer covers its own toggle) |

**219 rows. 81 driven green, 106 fully asserted by test, 20 half established, 8 unverified, 4 driven red.**
...
> reproduced, fixed in `5d4b782` and re-driven. The full evidence per row is in §§ 9–24.
