...
| TP-02 | happy | The speed chips | ×1 / ×10 / ×60 / ×240 / ×900, exactly one pressed, each captioned *N simulated seconds per real second* | ✅ run (drive: five chips, exactly one pressed, each captioned *N simulated seconds per real second*; `[`/`]` walked and clamped the ladder) — `SPEEDS` is still read by no test; the ×900 rung exists because a 1 800 s shift at ×60 is thirty seconds of watching |
