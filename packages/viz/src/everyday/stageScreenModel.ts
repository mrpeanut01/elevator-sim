 ...
 * incident. Only (1) is a `RunInterventionConfig['change']` kind in `core` on this branch — the
 * kind union has exactly one member, which is what makes {@link STAGE_INTERVENTIONS} a table rather
 * than a list of possibilities. A concurrent lane is adding the second; when its kind lands, the
 * union widens, `live/interventions.ts#STAMP_VERBS` fails to be exhaustive until it gains a verb,
 * and this table gains **one row**. That is the whole change — the control below iterates the
 * table, the screen draws a button per row, and nothing branches on a kind anywhere.
 *
 * (3) is not a row here and will not become one: the campaign incident is answered from § 7.5's
 * dock, which needs a `ctx === 'campaign'` no route in this build produces. See
 * {@link STAGE_ABSENCES}.
 */
export interface StageInterventionRow {
  readonly kind: RunInterventionConfig['change']['kind'];
...
/** The arms this build ships. One row per `InterventionChange` kind — see the interface docstring. */
export const STAGE_INTERVENTIONS: readonly StageInterventionRow[] = Object.freeze([
  Object.freeze({
    kind: 'park-cars-lobby' as const,
