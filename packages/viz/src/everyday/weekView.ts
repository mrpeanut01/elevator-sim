...
export interface WeekDayCard {
   ...
   * the one press that does — the stage row's `Close the day`, whose § 3.3 note is literally
   * *"stops the clock and writes the report"* — is on a bar the shell does not draw over the
   * handed-off Engineer stage (`EVERYDAY_SHELL_ABSENCES` names that). So the loop's last two
   * screens were reachable in the design and unreachable in the product, which is the shape of
   * defect this repository has shipped eleven times: everything wired, nothing that can press it.
   *
...
export function weekScreenViewOf(input: WeekScreenInput): WeekScreenView {
  ...
  return {
    ...
    board: {
      ...
      refusal: UNBUILT_REASONS.board ?? 'the board screen is not built',
      rules: BOARD_RULES,
    },
  };
}
