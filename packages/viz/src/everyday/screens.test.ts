...
describe('what this build has actually built', () => {
  it('is the shell’s own menu and every registered module — the menu plus stage, § 8’s three, fixit and settings', () => {
    /*
     ...
     * `settings` the second (§ 15.1), `stage` the third (§ 7, `everyday/stageScreen.ts`) and § 8's
     * `towers`, `building` and `contract` the fourth, fifth and sixth (`everyday/campaignScreens.ts`
     * — one module with three rows, because the three are one flow). The order is
     * `EVERYDAY_SCREENS`' own, because the constant is a filter over the inventory rather than the
