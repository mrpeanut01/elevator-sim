...
describe('what this build has actually built', () => {
  it('is the shell’s two screens and every registered module — the two plus § 8’s three, fixit and settings', () => {
    /*
     ...
     * `settings` the second (§ 15.1, `everyday/settingsScreen.ts`), and § 8's `towers`, `building`
     * and `contract` the third, fourth and fifth (`everyday/campaignScreens.ts` — one module with
     * three rows, because the three are one flow). The order is `EVERYDAY_SCREENS`' own, because
     * the constant is a filter over the inventory rather than the registry's insertion order.
     */
