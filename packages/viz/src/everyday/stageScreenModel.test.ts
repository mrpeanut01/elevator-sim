...
describe('§ 7.6 — the intervention control', () => {
  it('ships one arm per InterventionChange kind this build has', () => {
    /*
     * The claim the docstring makes, checked: the table is one row per kind, so lane A3's second
     * arm is a row rather than a rewrite. If the union widens and this table does not, the count
     * disagrees and this case says so.
     */
    expect(STAGE_INTERVENTIONS.map((arm) => arm.kind)).toEqual(['park-cars-lobby']);
