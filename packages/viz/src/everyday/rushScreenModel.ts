 ...
 * screen*, because deixis is what went stale on `RUSH_BESTS` (deleted, GitHub issue #418) and a module name is a thing a
 * test can check.
 *
 ...
 *
 * ## The standings entry left on the commit that measured them — GitHub issue #418
 *
 * It read *"the standings — the five entries on the rush setup screen are the handoff’s own
 * fixtures, not runs this build measured"*, and it was the last entry here. #418 replaced those rows
 * with the house's measured runs (`everyday/rushHouse.ts`, § D547), which made the entry false, so it
...
/*
 * **The standings left this file on the commit that measured them** — GitHub issue #418, § D547.
 *
 * `RushBestView`, `RUSH_BESTS` and `RUSH_BESTS_FIXTURE_NOTE` stood here: § 9.1's five rows as the
 * handoff's own fixtures, two of them carrying handles that read as player accounts, under a marker
 * saying so (#293, § D376). The owner's ruling of 2026-09-10 replaced the rows with pinned runs of
 * the shipped dispatchers on the rush's one seed, labelled as the house, and dropped both handles.
 * The rows and their note are `everyday/rushHouse.ts#rushStandingsOf` now, read off
 * `data/rush-house-runs.json`, and `rushScreen.ts` draws that view. The marker went on the same
 * commit as the fixtures, because a label saying *not runs this build measured* over rows that are
 * runs this build measured would be the stale refusal the marker was written to prevent.
 */
