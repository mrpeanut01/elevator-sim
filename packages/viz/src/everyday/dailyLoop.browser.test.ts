...
/** Walk the § 3.3 primary from the front door to the handed-off stage. */
 ...
 *
 * **Not a shortcut around a control, and the distinction is the point.** § 3.3 gives the stage row
 * the primary `Close the day`, and the shell does not draw its bar over the handed-off Engineer
 * stage (`EVERYDAY_SHELL_ABSENCES` names that, and now names this consequence). So there is no
 * Everyday button to press for it in this build; the press this call stands in for is the Engineer
 * surface's own. What is being tested here is what closing a day does to the four screens, and the
 * gap between this line and a button is a stated absence rather than a hidden one.
 */
...
describe.skipIf(!HAS_BROWSER)('the daily loop is walkable end to end', () => {
  it('goes door → brief → stage → report → week on the § 3.3 primary alone', async () => {
    ...
    try {
      ...
      // § 6.3 / § 7: the day is the handed-off Engineer stage, uncovered beside the rail.
      await page.locator('.everyday-bar-primary').click();
      await page.waitForFunction(
        () => document.querySelector<HTMLElement>('.everyday-main')?.style.display === 'none',
        undefined,
