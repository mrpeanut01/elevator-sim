...
export const SURFACE_ADAPTERS: readonly SurfaceAdapter[] = Object.freeze([
  ...
  WITHHELD_MATRIX,
  /*
   * Appended last, per the fault-ordering rule stated at SHIFT_REPORT. The workshop re-renders
   * term rows and lever sub-lines the editor adapters already draw, and the bench re-renders the
   * suite's index words, so placing either earlier would move every editor-shaped and
   * suite-shaped fault onto these two surfaces.
   */
  EVERYDAY_WORKSHOP,
  EVERYDAY_BENCH,
]);
