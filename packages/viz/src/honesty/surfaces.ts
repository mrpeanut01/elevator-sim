...
const EVERYDAY_STAGE: SurfaceAdapter = {
  ...
  render(context) {
    ...
    for (const arm of STAGE_INTERVENTIONS) {
      seeds.push({ field: `stage.intervene.${arm.kind}.label`, text: arm.label, role: 'label' });
      seeds.push({ field: `stage.intervene.${arm.kind}.explains`, text: arm.explains, role: 'prose' });
    }
