...
import {
  ...
  contractView,
  towersView,
} from '../everyday/campaignModel.js';
import {
  applyCampaignAction,
  freshTower,
  openingCareer,
...
export const SURFACE_ADAPTERS: readonly SurfaceAdapter[] = Object.freeze([
  ...
  WITHHELD_MATRIX,
  // Appended last, per the fault-ordering rule stated at SHIFT_REPORT: GAMEPLAY § 8's three
  // campaign screens share record-shaped phrases with the week's own surfaces, so placing this
  // earlier would move every week-shaped fault onto it.
  EVERYDAY_CAMPAIGN,
  // Appended after it, same rule again: `docs/21` § 3.4's live metrics card re-renders phrases
  // `CANVAS` also draws — the refusal head and reason above all — so placing it earlier would move
  // every panel-shaped fault onto this surface.
  LIVE_METRICS,
]);
