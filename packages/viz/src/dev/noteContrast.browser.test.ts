 ...
 * ceiling), the one the rules editor's mount inserts (slice 4c — its scope note above the rows),
 * and the plain-lever echo `docs/19` defect 5 added (`dispatcherEditor.ts`'s `plainEcho`, an
 * `.advice` built at mount and hidden until a lever moves; hidden nodes are in the sweep, which
 * is right — a note's ink must clear AA before the press that reveals it). Pinned rather than
 * bounded below, because the count is the number `noteContrast.test.ts` derives from the sources
 * independently — the echo is a child of the mount's own node, which that tier's recorder cannot
 * resolve and its own prose names as this tier's to reach — and the two agreeing on the shared
 * sites is the evidence that neither is looking at half the page.
 *
...
beforeAll(async () => {
   ...
   */
  site = await startShippedSite({ preview: { port: 5293, strictPort: false } });
  origin = site.origin;
