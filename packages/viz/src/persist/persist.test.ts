...
const NOT_PERSISTED: Readonly<Record<string, string>> = Object.freeze({
  ...
  'viewer.campaignFitOut':
    'the kit § 8’s campaign tower has had fitted. Derived rather than authored — `campaign/fitOut.ts#fitOutOf` folds it out of the tower’s own bookings — and the record it is folded from is written to this device since GitHub issue #375, but under its **own** slot and its own version (`campaign/careerPersist.ts`) rather than in this snapshot. Persisting the derivation here would put a second copy of a career fact in a record that does not own it, and restore a fabric belonging to a contract the career may have moved past, which is `viewer.commissioning`’s objection with the pair already broken',
  'viewer.campaignEventId':
    'the event § 8’s campaign day runs under (GitHub issues #171 and #169 item 1, § D507). Out for `viewer.campaignFitOut`’s reason exactly: it is chosen from the tower’s contract day and the run’s seed by `campaign/incidents.ts#campaignEventFor` on the press that runs the day, its source is the career, which persists under its own slot rather than in this snapshot since GitHub issue #375, and a restored event beside a week that is not that day would put a breakdown on a run nobody pressed',
