...
export const DESTINATION_CASES: readonly BenchmarkCase[] = Object.freeze([
  ...
  Object.freeze({
    ...
    rationale:
      'The access-control half: the same pattern on the building with five access zones. THE NUMBER SURVIVES AND ITS REASON DOES NOT, which is the whole of why this rationale is rewritten rather than edited — DECISIONS.md § D254, § D256 (withdrawal item 4) and § D261. It read: "the point of it is that the CONVENTIONAL arms cannot serve it … an access-restricted pickup carries no credential under up-down-buttons, so every car returns accessDenied and the call is permanently unassignable … there is no budget at which a conventional arm has a quotable AWT here." Every clause of that is now false. The pickup check was a modelling error — a credential governs where you may go, not where you may be collected — and it is deleted, along with the accessDenied reason itself. Re-censused over 300 replications at BENCHMARK_SEED on 2026-08-06, EVERY arm — nearest-car, eta, collective and every credentialled one alike — first loses its AWT at replication index 3, and the ground is CENSORING rather than the old bank-wide refusal: § D265\'s credential gap turns away 4.1 % of arrivals, which crosses the 5 % limit on a 50-to-75-person reporting window. THAT NUMBER IS NOT INHERITED EITHER. § D261 recorded these arms "clean across the whole census", which was true when it was written and was measured before § D265 landed; § D279 re-measured rather than re-cited, and the correction is that the arms fail together at 3 instead of failing at all or not at all. Failing at the same index on every arm is the evidence it is the traffic and not the dispatcher — under common random numbers the turned-away riders are the same people in every cell. admissibleReplications stays 0 because the field means "the largest budget at which EVERY arm including the baseline still has a valid AWT", and one arm still loses it earlier still, at index 0: destination-entry-bare, the bare kiosk, which discloses a destination and carries no credential so an access-restricted DESTINATION is refused by every car — and its ground is SATURATION, a queue rising 0.8 to 1.8 persons per minute, which is a different failure in kind and not only in degree. That is authorization of a destination, which is the only access question a lift is asked, and it is untouched by § D254 — it got cleaner, since every undelivered leg is now a credential refusal and nothing else. So the ceiling is real and the arm that binds it has changed. H-ACCESS-1 is reported as counts rather than as an interval for that narrower reason, and its own verdict is REFUTED (§ D256); whether this cell should now carry an interval table over the credentialled arms alone is a re-design of the arm list, and § D256 requires a criterion written before the numbers.',
  }),
]);
