...
describe('every parameter core declares is accounted for', () => {
  ...
  it('carries every other declared row a dispatcher profile can hold, and only those', async () => {
    ...
    //
    // **And one more from § D265**: `traffic.credentialGap.wrongZoneShare`, the share of
    // in-building journeys made by somebody whose badge does not open where they are going. It is
    // `default: null` for the same reason the twelve before it are — the number lives in
