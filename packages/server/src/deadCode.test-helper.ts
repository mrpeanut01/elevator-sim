/**
 * **The sixth audit's scanner** — copied verbatim from `packages/viz/src/deadCode.test-helper.ts`,
 * which itself inlined `packages/experiments/src/tuning/callers.test-helper.ts`.
 *
 * ## Why a copy rather than an import, for the fourth time
 *
 * The same three constraints viz's header states, and this package hits all three:
 *
 * 1. **The scanner is on no surface this package may import.** It is not re-exported by
 *    `experiments`' `index.ts` and cannot be, because it reads `node:fs` and the browser barrel is
 *    the environment-free entry point (§ D121).
 * 2. **This package depends on `core` and nothing else.** `package.json` lists one dependency and
 *    `tsconfig.json` references one project. Adding `experiments` — or, worse, `viz` — to a
 *    server so that a *test* can reach a scanner would put a rendering package in a socket's
 *    dependency graph to satisfy an audit.
 * 3. **A cross-package relative import breaks the build.** `rootDir: "src"`, so a file reaching
 *    `../../viz/src/…` is outside its own root and `tsc -b` refuses it under project references.
 *
 * **This is now the fourth copy, and that is the finding rather than a footnote.** Four sites is
 * past the point where duplication is cheaper than consolidation, and the fix is a real one: a
 * tiny `@elevator-sim/dev-audit` workspace package that every audit devDepends on. It is not done
 * here because this branch is already carrying a new package, and adding a second one to hold a
 * scanner would be scope this change did not ask for. Recorded so a consolidation knows all four
 * sites: `experiments/src/tuning/`, `core/src/dispatch/`, `viz/src/`, and here.
 *
 * ## Divergences from `experiments`' original, all three deliberate
 *
 * 1. **`PACKAGES_DIR` is two levels up, not three** — this file sits at `server/src/`, one level
 *    shallower than `experiments/src/tuning/` and at the same depth as `viz/src/`.
