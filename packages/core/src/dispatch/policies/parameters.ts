 ...
 *
 * ## Why the reserve carries no `activeWhen`, though it is conditional
 *
 * `auction.reserveMarginalDelayS` is genuinely inert while `auction.rounds` is 1, and saying so in
 * an `activeWhen` would be the obvious thing to do. It would also be **unevaluable**, and that is
 * worse than absent.
 *
 * `DispatchParameterSpec.activeWhen` maps a parameter id to *the values that make this knob live*,
 * and `dispatch/parameters.test.ts` states the contract every row in `DISPATCH_PARAMETERS` obeys:
 * the gate must exist, `gate.type` must be `'categorical'`, and each listed value must appear in
 * `gate.values`. Every existing gate names a categorical — `dispatch.assignmentTiming`,
 * `dispatch.reassignmentPolicy`, `idle.parkingStrategy`. `auction.rounds` is an **integer with a
 * range and no `values`**, so a gate on it — `{ 'auction.rounds': ['2', … , '8'] }` — satisfies the
 * shape and none of the semantics: an optimizer implementing the contract evaluates
 * `gate.values.includes('2')` against `undefined` and either throws or treats the reserve as
 * permanently inactive, and one comparing its own sampled `3` against the string `'3'` never
 * activates it. A gate whose evaluation rule differs from every other gate in the schema is exactly
 * the elevator-specific knowledge CLAUDE.md invariant 8 exists to remove.
 *
 * So the condition is stated where an optimizer can only ignore it rather than misread it — in the
 * `description` — and the inertness is asserted **behaviourally** instead: `policies.test.ts` runs
 * the same reserve at `rounds: 1` and at `rounds: 2` and requires no withdrawal in the first case
 * and one in the second. That is the real content of the gate, and it cannot rot into a string
 * literal that no longer means anything.
 *
 * The follow-up this owes, in a file this module does not own: give
 * `DispatchParameterSpec.activeWhen` a numeric-threshold form — `{ id, atLeast }` alongside the
 * value list — and update both suites, so there is **one** evaluation rule and the gate can come
 * back. Until then a conditional knob declared unconditionally costs an optimizer a wasted
 * dimension at one end of another knob's range, which is a cost that can be measured; a gate
 * nothing can evaluate costs it the knob entirely, silently.
 */
