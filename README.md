...
| [Audio direction](docs/29-audio-direction.md) | The audio decision, and it is a **recommended cut** rather than a brief — awaiting the product owner, who owns this call. The argument is measured rather than aesthetic: the stage's slowest shipped speed is already **8× compressed**, so a door cycle of 9.8 simulated seconds is 0.33 s of wall time at the default and 16 ms at `30×` — door chimes and motor whine cannot exist as discrete cues at any speed the transport offers. Carries the strongest argument against its own recommendation in full, exactly what the cut changes across five sites, and the three things that would reopen it |
