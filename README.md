...
| [Static site deployment](docs/16-static-site-deployment.md) | Hosting the viewer on Azure, and why the platform question is short rather than close: the app has no server half, so every compute-bearing option pays for an idle process to serve files a CDN serves for nothing. Carries the priced comparison, the deployment path that leaves **no secret in this repository** — a federated identity, a token fetched per run, a custom role with two actions on one resource — and the build-only bug it found on the way: a boot-critical manifest that a dev-server middleware was the sole producer of. Unarmed until a repository variable is set |
