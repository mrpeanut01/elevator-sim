...
wait, they board, they leave. **Passenger patience, lift-lobby crowding and stair-taking are built and
opt-in, off by default** ([`docs/14`](docs/14-building-behaviour-contract.md) — steps 0–6). With
them off, a rider will queue for twenty minutes without ever glancing at the stairwell, which no
real person does; with them on, riders abandon and some take the stairs.
...
| [Audiences and the core loop](docs/23-audiences-and-core-loop.md) | Who the two audiences are, what each must get in the first session with an observable condition that can fail, and the core loop every mode declares which half it serves. Written against premises that did **not** survive verification — `packages/viz/UX.md` already defined five roles and the design-canonical gameplay handoff already carried a per-mode loop table with lengths and lose-conditions — so it reconciles those rather than writing a second inventory beside them, which is the drift this repository exists to prevent. Carries the honest register of modes that do not currently serve the loop |
| [Competitive teardown](docs/24-competitive-teardown.md) | Nine adjacent games read against one template, each with an adopt-or-refuse verdict tied to a named pillar or non-goal. Extends [the experience-layer contract](docs/10-experience-layer-contract.md) § 3's cited prior-art survey rather than restating it — two accounts of the same four titles would drift — and adds the five that appear nowhere in the tree, Elevator Saga first, since it is the closest comparator and the one this product is most likely to be mistaken for. Opens with the correction note that both of the issue's premises failed verification |
| [Telemetry and privacy](docs/26-telemetry-and-privacy.md) | The posture first, then the schema, because data collected without a posture cannot be un-collected: four KPIs and nothing else, each tied to the charter criterion it serves and each estimator chosen so event loss makes the gate harder rather than easier. Its load-bearing choice is the **run pointer rather than the payload** — invariant 5 makes a run reconstructible from seed and config, so an event names the run instead of carrying derived figures, which answers more later while saying less about a person now. Carries what is deliberately not collected (including a refusal itself, whose cost is published beside every figure rather than fixed by collecting more), and states plainly that two criteria are not funnel-measurable at all and get no proxy. **Part B (2026-09-09) is the product-wide privacy posture**, added under three owner rulings made that day: player-typed text is allowed and P-4 is rewritten rather than excepted, the product may read its own collected data back, and the whole of Part B **ships marked as requiring professional legal review before it goes live**. It itemises every category of data the product touches with why each is held, gives retention and a deletion path per item, drafts what a player is told and where, and puts the lawful basis and the position on children **as options with their consequences rather than as answers** — nineteen questions flagged for the reviewer in one checklist. Its own measurement found that *Clear saved progress* clears two of the six slots this origin holds while its copy says nothing kept survives |
| [Flow maps](docs/27-flow-maps.md) | Every player-facing flow end to end, and each one in **five states** rather than one: happy path, empty, restored after a reload, server-unavailable, and recovery. A flow mapped only on its happy path is not mapped — the two worst defects this project has produced were both found by playing rather than by the suite, and both lived in the restored and recovery columns. The graph is derived from `everyday/actionBar.ts`'s rows and the screen registry rather than asserted, so the next reader can re-derive it; dead ends are listed and split into unreachable-by-defect and unreachable-by-design |
| [Audio direction](docs/29-audio-direction.md) | The audio decision, and it is a **recommended cut** that was **overruled** — [§ D344](DECISIONS.md) ruled on 2026-08-24 that audio ships, speed-tiered, and #258 is the lane that builds it. The argument is measured rather than aesthetic, and the measurement is what survived the ruling: a door cycle of 9.8 simulated seconds is 0.33 s of wall time at the stage's default speed and 16 ms at its fastest, so door chimes and motor whine cannot exist as discrete cues **up there**. What did not survive is the sentence that made it a cut — the slowest rung was 8× compressed when this was written and is `1×` — **real time**, the same cycle in 9.8 s of it — since [§ D354](DECISIONS.md), which is why the owner read the same arithmetic as a defect report about the speed ladder. Carries the strongest argument against its own recommendation in full, exactly what the cut changes across five sites, and the three things that would reopen it — one of which has since been delivered |
| [Support matrix](docs/31-support-matrix.md) | Which browsers and platforms are supported, in four tiers, with the reason each is in the tier it is — and the gap between what is *tested* and what would be *supported* as the document's main content, since today that is **Chromium only, on two desktop OSes, and no touch device at all**. Touch is committed as best effort and split into two clauses that can each fail: small-screen layout at 360 px and above is in scope for launch, touch interaction is not. Carries a three-part performance budget for `charter S9` written to be enforceable on a CI runner that is not a mid-range laptop, and the costed answer to what testing the matrix would take — because a matrix nobody can afford to test quietly becomes a lie |
| [Playtest programme](docs/30-playtest-programme.md) | Recruitment, cadence and protocol — the document M2 cannot exit without, since *ten testers complete the slice* and *six of ten can say what went wrong* are unexecutable without one. It is also the only instrument for the two success criteria telemetry can never measure. Its governing rule is the one this project learned the expensive way: **a playtest finding is a claim, not a defect** — findings route through verification before they are scheduled, and a refuted one is recorded with its refutation rather than dropped |
| [Art direction](docs/28-art-direction.md) | The visual language, stated rather than invented — the vendored handoff is canonical for the interface, so this names what exists, records where the build diverges and why, and specifies only what is genuinely unowned. That is chiefly the stage, where the crowd, doors and queues were all already drawn and the real defect was that a shut car was painted entirely over by its own door leaves — **fixed**, and the fix is asserted on painted pixels rather than described. Carries the ceiling on pillar P3: the stage may make a thing visible **as it happens** and may not publish the run's summary of it |
| [Game design](docs/32-game-design.md) | What the player does, what progression and the economy mean, and what losing looks like in each mode — the questions twenty-three engineering contracts specify around without ever answering. It references those contracts rather than restating them, since two accounts of one rule can drift apart, and carries a register of every contract it relies on with confirmation that none is weakened |
| [Difficulty curve](docs/33-difficulty-curve.md) | What the player is expected to understand at each campaign stage, each week and each fix case, what pressure produces it, and what has to go wrong for the lesson to land. Bounded by [§ D345](DECISIONS.md) and written against the world in which that decision's fix has landed — one bar per stage whatever tier is selected — because a curve specified against the current code would bake in the thing the decision forbids. Its measurements refuted the brief's own central figure: **three** of ten campaign stages clear from the dispatcher dropdown alone, not four, and three more stages satisfy that rule only because they admit no dispatcher but their own. Specifies the sweep that re-derives all of it, since the published count has now gone stale three times with nothing checking it |
| [Usability audit of the rules](docs/34-usability-audit-of-the-rules.md) | Asks of this repository's own decisions and invariants whether any of them is costing a player something, and separates a rule that is wrong from a rule that is right and has been read one step too far. Six findings, every claim about what a player meets taken by driving the built page at 1280 x 720 rather than read off a docstring — two of the findings are docstrings that were wrong. **No rule was weakened and none is proposed for weakening**: one was right and unenforceable and is now enforced (fourteen dead controls carrying no reason, taken to zero), two are right and are escalated because a lane may not relax them, one issue's premise is refuted as already fixed, and the audit's own opening hypothesis — that the suppression rules leave a player with a bare `withheld` — is refuted against the code |
| [A problem per mode](docs/35-problem-per-mode.md) | What problem each of the four modes poses, how the player **sees** it before any figure, what they change, and how the verdict stays honest. It exists because `docs/33` says every stage names *what **visibly** goes wrong* and then discharges that word with a goal miss — a goal is a figure, and a figure is not a sight. Its central finding is that **no shipped mode satisfies the see-it-first constraint, including Fix a building**, which has no stage at all and already throws away a full recording of the failing building on every case open. Measured here rather than assumed: at the shipped day-one configuration the tutorial building's landings are **empty about 91 % of the hour**, and on **16 of 20 seeds no instant exists** at which anybody has been waiting sixty seconds |
| [Accessibility standard](docs/36-accessibility-standard.md) | The standard the game holds itself to, seventeen clauses, each naming the instrument that checks it or saying plainly that there is none and who does it instead — because `docs/31` § 5's rule is that a row no instrument checks is a tier-3 claim and must be labelled one. Applying that honestly produces the finding: **no clause is wholly tier 1**, and every clause that has an instrument has it on the Engineer shell, which is the one a player meets second. The conformance target is **not** chosen here; three options are priced for the product owner, and the surprise is that WCAG 2.2 AA costs 2.1 AA one unmeasured criterion and one small check. Measures the eight shaft tints for the first time, in both themes: not one of twenty-eight pairs reaches 2:1, and four of the eight are one greyscale value in dark mode |
| [Content plan](docs/37-content-plan.md) | What ships, what beta must ship, how each content type is produced and validated, and the rule that new content may not require a new engine capability unless that capability is separately scheduled. Every current count is derived from `data/` rather than quoted, which is how it found that the issue asking for the plan was wrong about one of its own five figures: there are **eight** contracts, not five. Sets the target from arithmetic rather than ambition — the daily rotation rules fix the building count, the design handoff's own catalogue fixes the fix-case count, and seven independently derived targets land inside a play-hours band derived from a different document. Resolves the proof-case building substitution by authoring the two towers, and finds the same substitution running unrecorded through **thirteen of the eighteen** fix cases |
| [What the game is](docs/38-what-the-game-is.md) | **Adopted 2026-09-06.** One page that says what is being built, written to sit before the charter: the charter says how the game is built and this says why. Names the three modes the front door carries — Scenario first, then Career and Rush — in one sentence each, and what each absorbs from the shipped four: *Fix a building* and *Today's tower* become Scenario content beside the campaign stages and the six Engineer challenges, under one schema with a budget and one price ladder; the Campaign becomes a Career that persists; the rush is built to [§ D477](DECISIONS.md) and its rounds are postable. Records the owner's rulings that no fix is proposed to the player, that difficulty is the number of affordable configurations that survive, pre-simulated, and that every mode plays live on the stage, and that one currency earned by completing turns and spent on a mode's modifiers runs across all three — nothing resetting on time, no purchase shipping, and the ledger built so an add from outside is invisible to the play surface. Its three decisions, [§ D525](DECISIONS.md), [§ D526](DECISIONS.md) and [§ D527](DECISIONS.md), are decided by the product owner; its § 2.4's two open items were closed the same day — [§ D530](DECISIONS.md) names the currency **chimes** and [§ D531](DECISIONS.md) ships the sign-in bonus — so nothing on the page is open |
| [Decisions in force](docs/39-decisions-in-force.md) | The index to read before `DECISIONS.md` if you are touching the game layer: which rulings govern the game's shape now, which older entries they supersede or amend — each of those carries a **Status** line under its heading, and none is deleted or renumbered — and the rename map from the four-tile names to the three modes. Says what was and was not re-read, and the one marker vocabulary a future superseding decision uses |
| [Incident runbook](docs/40-incident-runbook.md) | What is watched, what a fault line means, and what to do when the deployed page is dead — GitHub issue #242. Three severity levels graded by **what a player can still do** rather than by how alarming the cause looks, which is what makes an API that is entirely down an S2: the page is a static bundle on a CDN and the game plays without it. Opens with the list of what is armed and what is not, in `docs/16` § 9's voice, because two of its five instruments do not exist yet and finding that out mid-incident would be worse than having no runbook. Points at [`docs/16`](docs/16-static-site-deployment.md) § 11 for the rollback rather than restating it, and carries that section's own account of what has been run: its **git half was rehearsed on 2026-09-15** by `scripts/rehearse-revert.mjs` and found two failure modes the reasoning had not named, while the half that touches the live site — the upload, the branch policy, the duration, the API — **has still been run by nothing** ([§ D587](DECISIONS.md)). Says why client errors are not transmitted anywhere — not the content policy, which permits a first-party route, but the lawful basis [`docs/26`](docs/26-telemetry-and-privacy.md) § 14.2 leaves open — and states what that costs |
| [Launch checklist](docs/41-launch-checklist.md) | What to run before a launch, each item naming its apparatus, what a pass looks like and who holds the credential to do it — GitHub issue #243. One of them is a run rather than a paragraph: `packages/viz/src/persist/migrationMatrix.test.ts` opens an envelope shaped as **every** version each save slot has ever written, requires the migration to fill the value the absence determined, and then requires the current build to take the result back — save it, read it, play a day on it. The row set is derived from the version constant the writer stamps, so a bump adds a red row on the same commit rather than needing a list somebody remembers to edit, and four positive controls are recorded with the output each turns red. § 9 keeps what has been run apart from what has only been reasoned about, in [`docs/16`](docs/16-static-site-deployment.md) § 9's voice, and § 10 says per criterion which of #243's five are open and what each needs from a human |
| [Launch record](docs/42-launch-record.md) | The answers, which deliberately do not live in the checklist: every item of [`docs/41`](docs/41-launch-checklist.md) that a checkout with no credentials could reach, executed on 2026-09-15 against `fb15704`, and every item it could not reach recorded as **not executable here** with the reason — measured wherever a measurement was available, as with the § 8 step 2 row that fails on a **shallow checkout** rather than on the code. **It is a dress rehearsal and not a launch**, and it says so in its own first line: an answer is a fact about a day, and of its thirty-two rows **twelve** are ones a checkout cannot reach at all — the browsers, the deployed page, the API and putting a previous build back — with five more that no apparatus exists for. What running it found is its § 3, including a revert range form that aborts part-way and leaves a partial revert staged |
| [What winning looks like](docs/43-what-winning-looks-like.md) | **The termination condition for autonomous work**, and a correction to the version that preceded it. The first panel convened to judge this product was given five axes — depth, correctness, reachability, legibility, honesty — and **not one of them would notice a game that is correct, honest, fully reachable and boring**, which is the failure this project is actually exposed to. So four axes are promoted above them and judged by **playing rather than reading**: game-play (is the second-to-second activity any good), usability (can a person operate it untaught), understandability (can the player predict a change before making it, which is a stronger test than explaining one afterwards) and entertainment (is there a specific pull back tomorrow, named rather than assumed). Each carries a protocol that can be run rather than a definition that can be argued — a decision census with the *longest gap* reported rather than the average, a prediction test where **both** tails fail, a blind diagnosis against ground truth, and the tomorrow question answered unprompted. The old five become a floor that gates rather than wins, with honesty the one that may never be traded. § 5 answers the obvious worry with a measurement rather than a reassurance: fifteen genre levers were tested against the charter's non-goals and none required a rule relaxed — the engine is the differentiator and not the tax |
| [Vertical slice](docs/25-vertical-slice.md) | The one representative ten minutes taken to shipping quality: what is in it, what is deliberately out and why, the quality bar itemised so a reviewer can refuse work against it, and exit criteria that can fail — each naming the instrument that measures it and whether that instrument exists yet. Written against the verified state of the thirteen M2 issues rather than their filed state, which matters: #209 is already fixed, #212 is largely refuted, and #208 is a slot decision rather than a data one |
| [Architecture](docs/01-architecture.md) | Three-layer design, core interfaces, DES kernel, determinism strategy |
| [Elevator Reference](docs/02-elevator-reference.md) | Elevator classes, speeds (up and down — descent is capped by air pressure above 300 m of travel), capacities, door and motion timings |
| [Traffic & Statistics](docs/03-traffic-and-statistics.md) | Demand profiles, RTT math, replication methodology, CRN, stopping rules |
| [Test Buildings](docs/04-test-buildings.md) | Seventeen reference buildings from low-rise to supertall |
