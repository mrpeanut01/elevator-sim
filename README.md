...
| [Audio direction](docs/29-audio-direction.md) | The audio decision, and it is a **recommended cut** that was **overruled** — [§ D344](DECISIONS.md) ruled on 2026-08-24 that audio ships, speed-tiered, and #258 is the lane that builds it. The argument is measured rather than aesthetic, and the measurement is what survived the ruling: a door cycle of 9.8 simulated seconds is 0.33 s of wall time at the stage's default speed and 16 ms at its fastest, so door chimes and motor whine cannot exist as discrete cues **up there**. What did not survive is the sentence that made it a cut — the slowest rung was 8× compressed when this was written and is **1:1 real time** since [§ D354](DECISIONS.md), which is why the owner read the same arithmetic as a defect report about the speed ladder. Carries the strongest argument against its own recommendation in full, exactly what the cut changes across five sites, and the three things that would reopen it — one of which has since been delivered |
