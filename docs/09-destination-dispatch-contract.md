...
| Some shipped profile is a destination dispatcher | **FALSE.** All ten profiles in `data/dispatcher-profiles.json` author no `dispatch.callType`, so all ten run at `up-down-buttons`, and **none weights `rideTime`** — the one term whose `activeWhen` names `destination-entry`. There is no destination dispatcher in `data/` today |
...
| **C. Level 0 disclosure** | `eta` + `rideTime`, `destination-entry`, `immediate` | Information moved earlier, model unchanged. Comparable on all 19 metrics |
| **D. Level 1 dispatch** | as C + per-passenger assignment + `sim.assignedWalkS` | The model change. Comparable on 10 metrics |
