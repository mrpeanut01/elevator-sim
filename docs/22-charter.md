...
|---|---|---|---|
| S1 | **Yes — and unevaluated** | 2026-09-10 | **The instrument landed on 2026-09-10** (GitHub issue #340), and this cell is the one this table has had to reverse. It read *"there is no funnel, no event chain and no session record anywhere in the tree"*, which was true for as long as `docs/26-telemetry-and-privacy.md` was a specification with no code under it. There is now a funnel — `packages/viz/src/telemetry/` composes the events `docs/26` § 7 declares, `packages/viz/src/everyday/` emits them from the player's own screens, `POST /api/telemetry` receives them and `telemetry_events` holds them under a ninety-day sweep. **Instrumented is not met**, which is S8's own distinction: no cohort has been measured, every figure is on the consenting subset only (§ 4.2), and this criterion stays **unevaluated** until the M4 gate's recruited cohort exists. The retired sentence's own lesson survives in `documentation.test.ts`: a word grep matches the word used to deny the thing, so the assertion behind this cell asks the code |
| S2 | **Partly — the chain cannot close today** | 2026-09-10 | Same instrument, and this cell says less than its neighbours on purpose. `docs/26 K2`’s five-beat chain is emitted from four shipped screens, and **two** beats have no event, not one. *Diagnose* happens in the player’s head and § 6.2 refuses to assert an event about it — that one is a decision. **Beat 4, `rerun_same_crowd`, is a gap**: it is in `telemetry/schema.ts#UNEMITTED_EVENTS`, so K2 reads 0 % by construction until it is emitted. This cell read *“the one beat with no event is *diagnose*”* for one commit, which is the instrument claiming a completeness its own register denied. A criterion whose KPI cannot reach a non-zero value is not instrumented, however much of the chain is wired |
| S3 | **Yes — and unevaluated** | 2026-09-10 | Same instrument. `docs/26 K3` reads the last event of a session and never a heartbeat, so it under-reports by construction — which is the safe direction for a gate |
| S4 | **Yes — and unevaluated** | 2026-09-10 | Same instrument. `docs/26 K4` is a browser-profile cohort rather than a person cohort, so it under-reports cross-device return; `packages/server/src/` still carries no analytics of any kind, and a telemetry row still carries no account id |
...
| S9 | **No** | 2026-08-29 | `.github/workflows/` carries `blocked-by.yml`, `ci.yml`, `closing-keywords.yml`, `deep-tiers.yml`, `deploy-viz.yml`, `review.yml`, `seed-boards.yml` and **no load budget** — `blocked-by.yml` (2026-09-06, [§ D485](../DECISIONS.md)) is a daily read of the issue backlog and touches no page, and `seed-boards.yml` (2026-09-06, [§ D522](../DECISIONS.md)) is a daily call of the API's seed route and touches no page either. `closing-keywords.yml` (2026-09-08, GitHub issue #400) reads a pull request's own text for a negated closing keyword and neither builds nor loads anything. `validation/perfScaling.test.ts` and `perfSweep.test.ts` measure simulation throughput, not page load — and `deep-tiers.yml` is where both of them now run (GitHub issue #163), which moves nothing in this column: a scheduled sweep of simulation throughput is still not a cold-load gate |
...
**Three of ten criteria cannot currently be evaluated at all, two only in part, four are
instrumented and unevaluated, and one is both instrumented and passing.** That is
