...
> `EVERYDAY_SHELL_ABSENCES`, kept beside the code rather than listed here, so they cannot go stale
> the way a prose row in a plan did. **It no longer lives in `shell.ts` and is no longer drawn on the
> menu**: GitHub issue #207 moved the array to `everyday/buildNotes.ts` and the six registers to a
> build-information panel in Settings, leaving the menu one sentence pointing at it.
>
...
| deep (`ELEVATOR_SIM_HONESTY=deep`) | 60 | **474 029** | 4 710 | **37** | **green** — 24/24, exit 0 |

Both tiers green on the same tree in one sitting. The deep tier's surface count is **37 against
the always-on tier's 36**, measured by its own run rather than inherited — the two differ by
construction (`campaign/judge.ts#judgeStage` is silent in the always-on tier, where
`STANDARD_SPACE` sets `stageProbability: 0`, and loud in the deep one), and this repository's own
§ D307 lesson is that a count carried across tiers is a count nobody measured.
...
- **Slice 10a, the honesty half — LANDED.** ENGINE_CONTRACT §12.2's withheld matrix, **enumerated
  ...
  **watched** player's share as the spectator's own *today, so far*, because `drawShift` read the
  ...
  the **35 statically swept DOM entry points**, which are still not driven — a figure this line
