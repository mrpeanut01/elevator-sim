...
| **Empty** | The setup screen is its own empty state, and **the register that describes it is not on it** — `RUSH_ABSENCES` names **one** missing seam — the standings — and is drawn on the Settings build-information panel (`everyday/buildNotes.ts`), where issue #207 put every register. **Corrected 2026-08-27 (issue #293),** and it is the same shape as the two rows above: this row said *on the screen*, and `rushScreen.ts` has not imported the register since #207. It also quoted the standings entry as *"the five entries **below**"*, a deictic that pointed at five rows on a screen the register had left — the entry now names the screen the rows are on. What **is** drawn beside the standings is `RUSH_BESTS_FIXTURE_NOTE`, § 20.11's required marker over five authored fixtures, two of which carry handles that read as accounts |
