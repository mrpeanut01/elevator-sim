...
| **T30** | F4 | browser | **A refusal is a designed state.** The rush tile opens, the setup draws, the primary is disabled **and relabelled**, its reason is drawn in the § 3.3 **bar's note** beside it, and the register names four seams. Asserts the refusal is *present*, which is the opposite polarity from every other row here. **Corrected 2026-08-25 (issue #262): this row specified the reason as being *on the control*, and it is not.** The row is unbuilt, so nothing passed on a false premise — but a browser test written from it would have asserted a `title` the `inert` branch never writes, and the honest reading is worse than a red: at 1280 × 720 the sentence it was meant to find was 186 px below the fold, so a row asserting *presence* on the screen would have found it and called the refusal designed. **Presence is the wrong predicate here; above the fold is the right one**, which is what moving the sentence into the bar makes true by construction. `RUSH_ABSENCES`' four seams are unchanged and re-counted |
