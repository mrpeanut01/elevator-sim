...
| `rush` | primary *Start the rush — not built yet* | — | **inert** and relabelled; `RUSH_PRIMARY_REFUSAL` is the § 3.3 bar's **note**, beside the button and not on it — see § 4.4's row (issue #262) |
...
| | |
|---|---|
| **Happy** | Main menu: heading, lede, **three** mode tiles — Scenario, Career, Rush — and the register *What this build does not do yet*. All three open (`UNBUILT_REASONS` is empty). **Corrected 2026-09-10:** this row said *four* after GitHub issue #364 retired the *Today's tower* and *Fix a building* tiles into Scenario ([§ D525](../DECISIONS.md)), which is the shape [§ D525](../DECISIONS.md) clause 1 names. The bar's left button reads `⌂ Modes` and is inert; the primary follows the selected card |
| **Empty** | There is no empty state. The menu is the same on the first load and the thousandth — no name, no streak, no history is required to draw it. The rail's `PLAYING AS` card falls back to `you` on sun and states *no days saved yet — close a day and it lands here*. **That wording changed with #214** and the change is the point: the card used to say *“this build keeps no career”*, which had stopped being true of the product and was, worse, the **only** line it could draw — it read a `profile` field no producer ever wrote. It now reads the week, so the empty state is a real empty state rather than the only state. **And it is not the only *no-week* state**: for the length of `dev/main.ts`'s async boot the shell holds a host slot and no host, and the card reads *reading your saved days…* rather than claiming a career it has not been handed — the front door drew the empty state's sentence over a **restored** week until § D402, because a `'menu'` route redraws no rail |
...
| **Restored** | **The career survives** — GitHub issue #375, § D525. `campaign/careerPersist.ts` is the versioned envelope, `everyday/careerStore.ts` the storage, and `everyday/host.ts` restores on open and saves through one writer. Towers, slots, purse, works, wear and contracts all come back. A version this build does not read is **refused, not patched**, the bytes are **kept** rather than cleared, and `EverydayHost.careerNotice()` carries the sentence the player meets. The week is **not** reconciled with: separate keys, separate versions, neither evidence about the other, asserted both ways in `careerPersist.test.ts`. |
...
| | |
|---|---|
| **Happy** | There is none, and that is recorded rather than implied. The tile opens § 9.1's setup screen — the bands drawn off the ramp, the standings, the driving line — and the § 3.3 primary is drawn **disabled** and relabelled *Start the rush — not built yet*, with `RUSH_PRIMARY_REFUSAL` beside it as the **bar's note**: *"the climbing stream is not built — this screen is the setup, and there is nothing behind it to start yet"*. **Corrected 2026-08-25 (issue #262).** This row said the refusal was *on the control*, and nothing has ever carried it there: `shell.ts#drawBar` writes `title` on its **refusal route** only — the branch for a screen key with no module — and `rush` is registered, so it takes the `inert` branch instead: `disabled`, `opacity:.6`, no `title`, no `aria-label`, no `aria-describedby`. Measured on the deployed build at `000852a`, the sentence sat **186 px below the fold** at 1280 × 720 and 2 776 px below it at 375 × 667, so a player met an amber button, pressed it, and the reason was on the screen behind them. The bar is the one element § 3.1 pins above the fold, which is why the fix is a move and not a margin |
| **Empty** | The setup screen is its own empty state, and **the register that describes it is not on it** — `RUSH_ABSENCES` names **no** missing seam — it named one, the standings, until GitHub issue #418 measured them — and is drawn on the Settings build-information panel (`everyday/buildNotes.ts`), where issue #207 put every register. **Corrected 2026-08-27 (issue #293),** and it is the same shape as the two rows above: this row said *on the screen*, and `rushScreen.ts` has not imported the register since #207. It also quoted the standings entry as *"the five entries **below**"*, a deictic that pointed at five rows on a screen the register had left — the entry now names the screen the rows are on. **Corrected 2026-09-11 (issue #418, § D547).** The standings are the house's measured runs on the standing building now, one per shipped dispatcher, and what is drawn above them is the house note, a field of `rushHouse.ts#rushStandingsOf`'s view; the fixture marker this row used to name is deleted with the fixtures |
| **Restored** | Nothing to restore; nothing is produced |
| **Unavailable** | Not applicable — the missing thing is an **engine**, not a server. `modes.ts` states the rule this follows: *where the screen exists and the thing behind it does not, the refusal belongs on the control, not on the door* — **which survives issue #262 unchanged, and is not the claim the two rows above it lost.** This one is about *which* refusal site owns the sentence — the control rather than the tile — and `modes.ts` still says it. What was false was the separate claim about how it is *rendered* once it is the control's: as an attribute on the button. It is the bar's note, beside it |
...
| **Empty** | With nothing stored: `you`, on sun, and *no days saved yet — close a day and it lands here* (#214 — see § 3's note; the old wording claimed the build kept no career, and was unconditional). Before the host lands, *reading your saved days…* — a different state, not a softer wording of this one (§ D402) |
...
| **T30** | F4 | browser | **A refusal is a designed state.** The rush tile opens, the setup draws, the primary is disabled **and relabelled**, its reason is drawn in the § 3.3 **bar's note** beside it, and the register names its seams. Asserts the refusal is *present*, which is the opposite polarity from every other row here. **Superseded 2026-09-10 (issue #423): the rush primary is live.** GitHub issue #220 built the engine (§ D515), `rushBarModel` became `return base`, and the relabelled cell `RUSH_SCREEN_COPY.primaryInertLabel` is deleted — it had no reader from that commit and what it said (*not built yet*) was false. This row is now a record of a state the product left rather than a row to build; the *above the fold* lesson below is the half worth keeping. **Corrected 2026-08-25 (issue #262): this row specified the reason as being *on the control*, and it is not.** The row is unbuilt, so nothing passed on a false premise — but a browser test written from it would have asserted a `title` the `inert` branch never writes, and the honest reading is worse than a red: at 1280 × 720 the sentence it was meant to find was 186 px below the fold, so a row asserting *presence* on the screen would have found it and called the refusal designed. **Presence is the wrong predicate here; above the fold is the right one**, which is what moving the sentence into the bar makes true by construction. `RUSH_ABSENCES` held four seams when this row was written and holds **one** today — the three about the missing engine left when #220 built it, § D227's rule |
