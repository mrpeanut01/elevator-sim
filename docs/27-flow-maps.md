...
| **Empty** | There is no empty state. The menu is the same on the first load and the thousandth — no name, no streak, no history is required to draw it. The rail's `PLAYING AS` card falls back to `you` on sun and states *no days saved yet — this build keeps no career* |
...
| **Empty** | With nothing stored: `you`, on sun, and *no days saved yet — this build keeps no career* |
