...
|---|---|---|---|---|
| 1 | `PM-FB1` | ~~Mount a stage over `session.asBuilt.recording` on the fix-case screen, above the four figures~~ — **built 2026-09-06**, GitHub issue #348: the module (`everyday/asBuiltStage.ts`, renamed `everyday/caseStage.ts` on 2026-09-19, see row 13) plays the recording on the stage's own painter (`everyday/cutaway.ts`, moved out of the stage for it), skippable, and the figures are stated after | `everyday/fixitScreen.ts`, `everyday/caseStage.ts` | **Small.** The recording already exists on the session; this is a renderer mount and a transport, not a simulation |
| 2 | `PM-TT1` | Move the *How hard this looks* plate off the pre-run position, or reword it as configuration | `everyday/today.ts` and its caller | **Small**, and it is a copy-and-ordering change rather than a deletion — **built 2026-09-06 as the second option** ([§ D514](../DECISIONS.md)): the plate reads people per working car and says the day shows whether that is comfortable |
...
| 8 | `PM-CA1`, `PM-CA3` | ~~The day opens on the building with today's event visible in it; a works-night purchase changes the picture as well as the purse~~ — **built, 2026-09-06** ([§ D504](../DECISIONS.md), GitHub issue #353): the tower screen draws the elevation above every word with the works car as a dashed well, and a works night holds one car out of service on the run, pinned on the legs. The event's own hole is on the stage the moment the run lands; the tower screen cannot know it before the press and says so | `campaign/works.ts`, `everyday/elevation.ts`, `everyday/campaignScreens.ts` | **Medium**, and #181 was the precondition |
...
| 10 | `PM-RU2` | A climbing demand template, authored in `data/traffic-profiles.json` with its own uncited-assumption note where the rate leaves the profile's cited band | `data/traffic-profiles.json` | **Medium**, and it is `CLAUDE.md` invariant 7 work rather than engine work — **conditional on § 11's unverified item 3** — **built 2026-09-06** ([§ D515](../DECISIONS.md)): `endless-rush`, with the rate as the override § D478 governs and the disclosure drawn on the setup screen and the result |
| 11 | `PM-RU1`, `PM-RU3` | The rush's held-time stage, the `Call it` press, and a result screen that publishes the divergence point from `summary.saturation` — never from a second definition — with its across-seed spread measured first | `everyday/rushScreen*.ts`, a measurement | **Large.** This is GitHub issue #220's whole scope — **built 2026-09-06** ([§ D515](../DECISIONS.md)) as the handoff's own `End the rush` rather than `Call it`: the result quotes the trend test and draws no *how far off* figure, because that figure's spread is unmeasured and the waves are one seed by design |
...
| 15 | `PM-FB1` (second half) | ~~The run the **player's own change** produced is played too — `primary`'s `onDone` kept `before` and let `after` go out of scope after `measuredOf` read it, so the payoff of the turn was a table~~ — **built 2026-09-19** ([§ D644](../DECISIONS.md)): `caseStage.ts` takes one pane or two, and a press mounts the pair above the outcome card at one playhead — as it stands, with your change — which is common random numbers made visible rather than asserted in a basis line. The verdict is **not** withheld behind it, for `docs/20` defect 16's reason | `everyday/caseStage.ts`, `everyday/fixitScreen.ts`, `everyday/fixitScreenModel.ts` | **Small.** Both recordings already exist on the press; this is one kept object and a second mount of a block that existed |
...
### 13.3 The fifteen specified code changes, reconciled against the open backlog
...
| 15 | `PM-FB1` (second half) | **built —** [§ D644](../DECISIONS.md) | The first half (row 1) mounted the as-built run; this is the run the player's own change produced, which `primary`'s `onDone` had kept only long enough for `measuredOf` to read its figures. Verified: `everyday/caseStage.ts` takes one pane or two and `everyday/fixitScreen.ts` mounts the pair at one playhead above the outcome card. **Numbered fifteenth rather than folded into row 1**, because row 1 is built, and a built row that grows a second clause stops being checkable against what it claimed |
...
| 13 | `PM-TT2` | **built** — [§ D512](../DECISIONS.md), GitHub issue #354 | `packages/viz/src/shift/legibility.ts#legibilityOf` measures the longest contiguous stretch a landing holds somebody in the third wait band, by exact interval union over the legs (§ 9.3's method); `legibility.sweep.test.ts` runs every contract's day 1 over `docs/33` § 4.6's cell — **650 days since 2026-09-15**, thirteen contracts × fifty seeds (it was 500 over ten from 2026-09-14) — and the table is published beside the window constant, with a ten-seed slice pinned by `legibility.test.ts`. **The finding:** Garden Apartments is legible on **0 of 50** seeds, Chancery House on 2 and St Jude's on 1, Midtown Office on 50 — which is #208's per-building eligibility measurement, built once here rather than twice. **And the limit this rule names now has a shipped instance** (GitHub issue #501): Ashgate is legible on **10 of 50**, below the eligible threshold, while its authored problem — a car-park arrival rides twice — is real and lives in time to destination rather than in a held landing. A held-landing measure cannot see it, which is why `PM-TT2` is a clause about *seeing* rather than a second verdict. **And the opposite limit now has three instances** (GitHub issues #425, #424 and #430): the three reference towers are legible on **50 of 50** at medians of 1 221 s, 729 s and 459 s, which is not a compliment to them — on a tower of four to eight thousand people a landing held past the band is the fabric rather than a problem a session can see and solve, and this arm should be read beside `docs/33` § 4.7k's miss rates rather than on its own. |
