...
**No issue was created, closed, labelled or commented on to produce this table** — #342 asks for the
mapping and forbids the creation. A row reading *new issue* is a title and a body offered to whoever
opens it.
...
|---|---|---|---|
| 1 | `PM-FB1` | **new issue —** *Fix a building: play the as-built run before the four figures* | Searched: nothing open covers it. #233 is case authoring and content scale; #177 item 7 is § 10.3's fuller **editor**. Neither is a stage mount. **Body:** the fix-case screen already runs the as-built configuration when a case opens — `packages/viz/src/everyday/fixitScreen.ts:315` `measureAsBuilt`, called at `:533` — and a full recording lands on `sessionOf(entry).asBuilt`, from which `fixit/run.ts#figureValuesOf` reads the four opening figures. Mount a stage over that recording above the figures, per `PM-FB1`. Constraint 2 for this mode costs a renderer mount and a transport, not a simulation |
...
11 sit on issues; rows 1, 4, 5, 6, 7, 8 and 13 have no issue and no blocker. That is the honest
summary of *"a specification whose items map to nothing is a plan nobody is executing"*: after this
table, seven items map to a title and a body that nobody has opened yet, which is a smaller failure
than mapping to nothing and is not zero.
