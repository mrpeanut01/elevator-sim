...
| **5 — patience, lobby crowding, stairs (§ 3)** | **built, and the gaps are part of the verdict** — abandonment with a fifth `awtIsValid` ground above censoring, a crowding term that **destabilises four of nine measured cells** (a finding, not a bug), and stairs with both asymmetries. **§ 3.3's condition 2 is withdrawn**, below. **Criterion 4 is met at the renderer too, and the refusal that said otherwise left on the commit that made it false** ([§ D227](../DECISIONS.md), GitHub issue #456). This row read *"met inside `core` and **not** at the renderer … bridged by a run-record disclaimer and not closed — that is the clause to distrust first"*, and it was right for as long as it stood: `shift/goals.ts` graded `minutePct`, a share whose denominator is the legs that **boarded**, so a day could clear the bar by leaving riders on the landing and no surface said so. `goals.ts#gaveUpBesideOf` is the closure — the second of the two the issue named, which is what § D106's wording asks for — and it is drawn wherever the bar is graded: the Engineer rail, the Everyday stage strip, the Day report's goal block and the campaign desk. **Pinned by a pair of days rather than by a sentence** (`shift/goalsBeside.test.ts`): forty arrivals both times, forty carried on one and twenty left standing on the other, `minutePct` **100 %** and the `minute` reading byte-identical on both — and the screens differ. Which quantities the count may stand beside is a `Record` over `GOAL_OBSERVATION_IDS`, so a new gradeable quantity fails to compile until it is classified, and which *modules* draw it is recovered from disk in three directions rather than transcribed. |
...
1. **`drawTransport` has a shipped caller and no DOM driver.** `dev/main.ts` is the only caller of
   ...
   **40 statically swept DOM entry points**, of which this mount is one — matched exactly by the
