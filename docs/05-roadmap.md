...
**Status: half green. The first criterion is MET; the second FAILS, and the measured effect is
exactly zero.** The gate lives in `packages/experiments/src/benchmark/`, whose `index.ts` is the
written report; every number below is regenerated by `formatBenchmark(await runBenchmark())`.

| criterion | verdict |
|---|---|
| each dispatcher beats `nearest-car` with a paired-t interval excluding zero on at least one building | **MET** — 8 of 8 arms, on all 3 buildings, on all 4 metrics. Nothing is WORSE in any of the 96 cells |
| pre-positioning shows measurable AWT improvement on Garden Apartments | **NOT MET** — `predicted-demand` vs `stay` is `0.00 [0.00, 0.00]`, 500 of 500 paired differences precisely `0` |

Three qualifications the headline hides, all of them measurements rather than opinions:

- **Three of the eight arms are bit-identical to `eta`** (`rho = 1`, interval `[0, 0]`, every
  metric, every replication). `zoned-uppeak` everywhere, `fairness-first` on both up-peak
  buildings. `zoneAffinity` and `predictedDemand` evaluate to `0` for every car in a real run, so
  what those arms beat the baseline with is a wait-time term and not their own mechanism. That is
  `IDENTICAL`, not `INDISTINGUISHABLE` — no budget changes it.
- **The pre-positioning mechanism works; the forecast never reaches it.** `lobby` parking moves
  AWT by **+1.97 s (12.0 %)** on Garden — far above any resolution limit here, and in the *wrong*
  direction, because on a sparse residential building demand originates upstairs. So the roadmap's
  second clause (*"where parking policy dominates"*) is true and the stage runs. `Simulation.#park`
  simply supplies no `demandForecast`, and `predicted-demand` with none answers `no-forecast`,
  which is observationally identical to `stay`.
- **`nearest-car` is too weak a baseline to separate anything.** It loses by 27–30 % on the
  up-peak buildings and is the only profile in the library that saturates anywhere in the study,
  which caps Midtown Office at **n = 287 permanently** (its first invalid replication). The tail
  terms only earn their weights one load step *above* where the baseline stops being quotable: at
  2 % with two entrances contending, `fairness-first` − `eta` is **−0.23 s AWT [−0.41, −0.05]**,
  **−1.58 s WT95** and **−1.94 s WT99** — the effect growing as you move into the tail, which is
  the signature of a genuine fairness term. There is no load above 2 % at which every arm is
  simultaneously quotable.

Four pieces of wiring are what stand between this and a fully green phase, none of them in a
Phase 5 module — they are enumerated with their one-line fixes as gaps 2 to 5 in
`packages/core/src/dispatch/policies/index.ts`: no policy hook on `SimulationConfig` (so
`AuctionDispatchPolicy` cannot be injected and multi-round contract-net has a measured 9.2 %
divergence rate and no wait-time result), no `reconsider` call site (so stage-5 capacity migration
never fires), no forecast in `#park`, and no `groupContext` in `#dispatchBank`. **Phase 7's
optimizer will search those dimensions and measure exactly zero until they close; they are worth
more than any weight vector.**
