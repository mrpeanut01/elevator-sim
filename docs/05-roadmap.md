...
| clause | finding |
|---|---|
| **1 — the honesty property under *search*** | **MET, and met by a run — and the figures below are the 2026-08-08 re-measurement, because the ones this row carried had gone stale in three of five places (§ D307).** Always-on: **49 cases, 246 875 strings, 30 surfaces, 0 violations.** Deep: **60 cases, 312 104 strings, 4 650 simulations, 30 surfaces, 10 violations in one case** — `honesty-9100031`/`suppressed-mean`, a cue-rule coincidence rather than a false claim on screen. The superseded figures were **271 985 strings, 23 surfaces, 0 violations** (`ELEVATOR_SIM_HONESTY=deep`, `packages/viz/src/honesty/campaign.ts`). It **found two violations first** ([§ D186](../DECISIONS.md)) — one real, fixed at the cause, and one a false positive whose check was *accepting the other branch for the wrong reason*. **[§ D172](../DECISIONS.md) had asserted this tier clean on "the refinement relation, not a run"; running it said otherwise**, which is what the label on that claim was for |
