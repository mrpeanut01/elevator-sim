...
> **A second building landed on 2026-09-15, and the Level-0 arm did not reproduce on it**
> ([§ D595](../DECISIONS.md), GitHub issue [#428](https://github.com/mrpeanut01/elevator-sim/issues/428)).
> `one-wtc-class-reference` is a 104-floor tower with six banks and a transfer on the way up,
> measured on § D100's own apparatus — `eta` baseline, n = 200 under common random numbers, TTD the
> gate — at *that tower's* highest quotable rate, 2 %, censused at the budget the run spends.
> `destination-eta` reads ΔTTD **−0.092 [−0.804, +0.619]**, **INDISTINGUISHABLE**, with
> `requiredReplications` of **11 704** against a budget of 200; `destination-panel` reads
> **−1.293 [−2.069, −0.517]**, **BETTER**, resolvable at n = 1. So the arm that carries this
> phase's accepted result does nothing there and the Level-1 panel does — the reverse of the shape
> Midtown shows. **The verdict above is not moved**: Midtown's interval is still Midtown's, and what
> the pair establishes is that the effect is **building-dependent**, which is what a second building
> is for and what a second seed could never have shown.
> `packages/experiments/src/benchmark/destinationSecondBuilding.test.ts` carries the run.
...
| Analytical cross-validation, every building the apparatus can reconcile | ✅ built | `experiments/src/oracle/fiveBuildings.test.ts`, `bankCensus.test.ts`, `reconcile.ts` — and `remainingBuildings.test.ts`, which takes the shipped set from five reconciled buildings to **eleven** (`chancery-house`, then `harbour-point` and `ashgate`'s `main`, GitHub issues #500 and #501, then one bank each of the three reference towers, GitHub issues #425, #424 and #430) and records why two of the rest cannot be reconciled at all, why `ashgate/carpark` throws, why nine further banks of the three reference towers throw on three separate grounds — one of them new: the apparatus cannot drain the crowd it offers a twenty-to-thirty-car bank — and why `burj-class-reference` is still owed (GitHub issues #232, #376) |
...
| clause | finding |
|---|---|
| **1 — the honesty property under *search*** | **MET, and met by a run — and the figures below are the 2026-08-09 re-measurement of both tiers, because the ones this row carried had gone stale for the second time.** Always-on: **49 cases, 287 083 strings, 30 surfaces, 606 simulations, 0 failing cases.** Deep: **60 cases, 362 317 strings, 31 surfaces, 4 710 simulations, 1 failing case**. **Both tiers pass**, which means every violation the search finds is held by `honesty.test.ts`'s `OUTSTANDING` register. A headline count of violations is deliberately not published: a finding is reported once per property that sees it rather than once per string, and two attempts to extract a total from the run's output disagreed. The register names *what* is outstanding, which is the thing a reader can ask a run to reproduce (`ELEVATOR_SIM_HONESTY=deep`, `packages/viz/src/honesty/campaign.ts`). **Both moved, and the first move was the same change**: issue #127 put the Day report's run-to-run delta block into the corpus — `honesty/surfaces.ts` had rendered `reportViewOf(shaped)` with no `previous`, so the caption, both note arms and the § D311 comparability refusal were swept by nothing on a block § D310 draws on two surfaces — which adds +1 220 always-on strings, +1 552 deep, and one simulation per case (a *drawn* comparison needs two runs). **Its first sweep found a real R13 gap**, 24 of 49 always-on and 28 of 60 deep: `AVERAGE WAIT was 17.8 s → 23.4 s`, a mean with no count in its own box, recorded rather than fixed on § D307's precedent and owed its own issue — **which is issue #137, and it is now closed in the product**: each paired mean carries the count it was taken over, one per side because the two values are means of two different runs, drawn by both renderers from one decision in the view, and a refused mean carries none. That took `estimate-without-n` from **48 reported violations to 0** always-on and **104 to 0** deep, took the always-on tier from 24 failing cases to none, moved the strings a third time (285 954 → 287 083 and 360 945 → 362 317, with cases, simulations and surfaces unmoved), and **deleted both `OUTSTANDING` entries on the commit that made the finding stop reproducing** — a registered finding that has been fixed must stop being registered. The deep tier's **remaining** failure is `honesty-9100031`/`suppressed-mean`, a cue-rule coincidence rather than a false claim on screen, published as outstanding since the temporal axis landed **and held by no register until this wave** — which is why the deep tier was red on `integration/issue-wave-15` before any of this. Measured on that base first: always-on **259 956 strings, 0 violations**; deep **327 805 strings, 31 surfaces, 10 violations**. The superseded figures were **246 875 / 312 104 strings, 30 surfaces** (§ D307), and before those **271 985 strings, 23 surfaces, 0 violations**. It **found two violations first** ([§ D186](../DECISIONS.md)) — one real, fixed at the cause, and one a false positive whose check was *accepting the other branch for the wrong reason*. **[§ D172](../DECISIONS.md) had asserted this tier clean on "the refinement relation, not a run"; running it said otherwise**, which is what the label on that claim was for |
...
tightens automatically when it does. The **42 statically swept DOM entry points** are **not
driven**, so a sentence assembled at runtime there is invisible to the search — **20** mounts and
**21** screen-registry rows, whose pure halves *are* driven, so what goes unswept in both groups is
...
**A limit that was invisible until a null result exposed it, and it is now closed**
([§ D437](../DECISIONS.md)). Every case this corpus had ever run was a tower **as built**: § D427
made a § 8 purchase reach the run and predicted these figures would move, they did not move at all,
...
alone**. It does not say the honesty property holds; it says it held over the generated strings
...
| **The game layer, wave AF (a press that decides the day)** | **Not a phase verdict — a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Four lanes against the wave AE re-score, which scored the game **P1 4, P2 6, P3 6, P4 5** against a bar of 8 and answered the termination question **no**. Its diagnosis was not repetition — wave AE had fixed that — but that **nothing the player does while watching changes the verdict**, measured three ways: 13 of 13 arms miss Today's scenario, 5 of 5 clear career day 1, and 302 s of a career day at the shipped speed held no prompt and nobody standing. **That is now false of one contract** ([§ D871](../DECISIONS.md), GitHub issue [#576](https://github.com/mrpeanut01/elevator-sim/issues/576)). A fourth authorable fact on the contract ladder takes a car out of passenger service mid-shift — a **rung** rather than a wrinkle, because `shift/events.ts#eventFor` is pure in `(day, dayIdx)` so a wrinkle on day 1 books a car out on all sixteen towers and re-derives every figure pinned there. On Crown Hotel, one pinned day: as-built **Shift missed** at 303 s against a 230 s bar, *park the cars in the lobby* **Shift missed** at 311 s, *spread the cars* **Shift cleared** at 215 s — **270 of 355 legs differ and 0 of them boarded before the press**, so the prefix is bit-identical and the press is a decision taken at a moment rather than a different day. A second pinned day has **two** presses clear, so neither verb is the answer. Over fifty seeds the dominance check is answered rather than assumed: as-built misses 22, a `predictive-balanced` handover 15, and *park in the lobby* **23 — worse than pressing nothing**, which is this contract's own lesson (a hotel has no dominant direction). **No goal bar moved**; `GOAL_BARS` is byte-identical. The decision census on that day goes from the assessor's 282 s with nobody standing to **80 s**, and from eighteen days to the first consequential choice to **112.5 s**. **The Workshop's two inert levers reach the run** ([§ D886](../DECISIONS.md), issue [#575](https://github.com/mrpeanut01/elevator-sim/issues/575)) — the standing requirement failing in § D227's first polarity, on the screen where a player is taught to tune, and the cause was **traced rather than inferred**: `dev/state.ts#drivingProfileOf` read `specFromProfile(base, base.name)`, the base profile round-tripped through the editor's shape and straight back out, so the player's edits were read by nothing that builds a run. The rival explanation was **refuted on the same sitting** by putting the same weight on the profile through `savedDispatchers`, where it does move the legs. That lane's first wiring then broke `dispatcherId`, which `scope/scope.test.ts` reported as an inert control **within the minute** — the repository's own instrument catching a lane's own regression — and it **declined to ship a refusal it could not establish**, because *no weight can make this term bite under `noDirectionReversal`* is a claim about `argmin` over survivors that eligibility does not settle. **The career keeps the player** ([§ D899](../DECISIONS.md), issue [#577](https://github.com/mrpeanut01/elevator-sim/issues/577)): the onward press landed on Today's tower with the career's crumbs gone, and an assessor played **thirteen days** before noticing — and the instructive half is that **nobody's module was wrong**, three correct pieces whose *composition* swapped the mode, which is why every unit passed and why the case that pins it must drive the shipped bundle. **The day report gains a causal line that says what kind of claim it is** ([§ D900](../DECISIONS.md), issue [#581](https://github.com/mrpeanut01/elevator-sim/issues/581)), and route 1 was **rejected on this repository's own statistical rule rather than on cost**: a with/without pair on one seed is one replication, and a difference is not declared without a paired-t interval excluding zero at 50–200. The sentence says so to the player — *"the day was not run again without it, so nothing here measures the change"* — and a test fails if *moved*, *caused*, *because* or seven other causal verbs appear in that row. **The chimes loop closes** ([§ D911](../DECISIONS.md), issue [#579](https://github.com/mrpeanut01/elevator-sim/issues/579)), for the first time and with **no `apiOrigin` and no account**: clear a fix case, earn six, buy a +6 unit budget rung on another, and **129 of 200 boarding legs differ**. Both figures are **chosen, not measured**, and the data file says so. **The corpus moved exactly 128.00 strings a case in both tiers**, attributed to the string across five producers, with **all four forecasts right** — one exact term by term, and one that published a **floor** rather than a figure for two whole Settings cases and was right by six strings. **Every lane also reported something it could not fix, and two are worth more than the fixes**: issue [#583](https://github.com/mrpeanut01/elevator-sim/issues/583), the 80 kJ/ride bar was derived over 1 800 s days and is applied to Today's authored 36 000 s day, where the same building under the same dispatcher reads **35.6 kJ/leg over the peak half-hour against 151.9 over the whole day** — a bar and a run that are not the same measurement, which is why 12 of 13 arms fail it and the one that passes is the worst on waits; and issue [#584](https://github.com/mrpeanut01/elevator-sim/issues/584), `legibility.test.ts` pairs each contract's building with `week.contractId` left at `c1`, so `rungFor` returns `undefined` and the sweep measures **every tower as built rather than as its contract hands it over** — and `ELIGIBLE_FIRST_CONTRACT_IDS`, which decides what a first-time player is offered, is derived from it. **What this wave did not do is re-score the game**; that is a run, not an argument |
| **The game layer, wave AG (a tower the player picks, and a press with its counterfactual beside it)** | **Not a phase verdict — a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Four build lanes and two fix lanes against the post-AF assessor, whose most valuable finding was that the day whose verdict turns on a press was **reachable only through a 37-in-365 date draw, once per device, with no player control**. **A player now chooses the contract their week runs** ([§ D912](../DECISIONS.md)): one row per shipped contract on the front door, moved through `shift/week.ts#switchWeek` with the `resume` arrival, and each row says before it is pressed whether the press parks or resumes. The mechanism had existed since issue #107; what was missing was a control in the daily loop, which is this repository's signature defect one layer up from a dead seam. Proved by pressing it on the shipped bundle rather than by a unit test. **Seven contracts carry a day whose verdict turns on a press** ([§ D914](../DECISIONS.md)), four clearing on *park in the lobby* and three on *spread the cars*, so the right verb is a property of the building. On `c6` and `c9` the arm that **clears** has the **higher** worst wait, because the goal deciding the day is the landing queue, which is why the verdicts are quoted off `dayReportOf` rather than derived from a figure. The brief asked for eight and measurement allowed seven: `c1` never misses at all over twenty seeds with a car booked out, and the count of both refused sets is pinned so an eighth fails a case. `GOAL_BARS` is byte-identical. **The day report shows the same seed run without the press** ([§ D931](../DECISIONS.md)), and it cost no simulation: `runShift` already re-runs the whole day on every mid-run press, so the recording it replaces **is** the day without that press, and it had been dropped. On the measured seed the press is the arm with **more** long waits, and the copy says so. § D900's refusal is **replaced rather than joined**, because two of its clauses stop being true beside a figure off the unpressed run. **Session lengths follow the day the tile opens** ([§ D946](../DECISIONS.md)): Career was sold at ~4 min against 15 actual, Today's scenario at 8–15 min against 150. The briefing said five office contracts run a 36 000 s day and the census through the shipped derivation says **thirteen of sixteen**. **Two measurements that were wrong about what they measured are corrected.** The energy bar was **one constant grading two window spans** ([§ D962](../DECISIONS.md), issue [#583](https://github.com/mrpeanut01/elevator-sim/issues/583)) — `peak-5min` on seven of eight contracts against a whole authored day — which on the flagship day meant **0 clean days of 325 across all thirteen dispatchers**. The whole-day bar is **350 kJ**, the tighter of two roundings, because the two-thirds point falls inside one tower's cluster where a looser figure would have been better conditioned; § D106's perverse ranking was re-checked at the new horizon and holds on the two contracts measured, with nothing claimed about the other eleven. The legibility sweep **measured sixteen towers as built** ([§ D961](../DECISIONS.md), issue [#584](https://github.com/mrpeanut01/elevator-sim/issues/584)), pairing every contract's building with a `c1` week, and in correcting it refuted a published reading: § D572 said Harbour Point holds a landing *even let at three fifths*, and the measurement had never applied the three fifths. **Two lanes built the same wrong thing and the difference between them was a run** ([§ D913](../DECISIONS.md), [§ D949](../DECISIONS.md)). Both derived that a parking press is inert under `zoned-uppeak` because the strategy it would set is already in force; one swept the predicate over thirty-six cells and found **two where the press moves the legs**, and the other withdrew its shipped refusal on that measurement. A refusal on a live control is § D177's inert-control class with its polarity reversed, and nothing reached the screen. **Integration is the finding worth keeping. Four lanes, each green on its own branch, produced nine failures together.** Six were registries and ratchets each lane had updated for its own new files only. Three were a genuine collision: one lane pinned an 800-day legibility sweep and another moved six of the rungs that sweep runs over, and neither was wrong. Re-measured on the merged tree ([§ D963](../DECISIONS.md)), **ten of sixteen rows reproduce to the second and six move, exactly the six whose rung moved**, with `c7` an unarranged control. **The first-session set grew from eleven towers to fourteen** with § D512's threshold untouched, and `FIRST_SESSION_LINE` moved on its own because the count is derived. That is § D454's lesson at its sharpest and the reason § D343 puts the corpus measurement after integration rather than on a branch. The fix lanes also caught a browser test **certifying an artifact nobody receives** (issue #281's defect reopening), cut a bound **1 414× its own job** rather than registering it, and found that the two figures in `vitest.config.ts`'s census prose that had drifted were **exactly the two the claim loop did not assert**. **The corpus moved exactly 104.00 strings a case and one surface in both tiers, and all four forecasts summed to it to the string**, with both fix lanes contributing zero. **The integrator opened this wave's decision block at D912 in error** — wave AF had already registered D912–D925 as holes — and a lane had to resolve the contradiction mid-build. The block should have opened at D926, and `CHARTER_PROGRAMME.md` now carries the rule: a wave opens above both the highest heading and the highest registered hole. **What this wave did not do is re-score the game**; that is a run, not an argument |
| **The game layer, wave AH (the week survives other modes, press days on days people play, a report that says what a press did, and a forty-minute day)** | **Not a phase verdict, a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Five build lanes against the post-AG panel, which answered the termination question **no** four times out of four. **Two decisions about what the game does were ruled by three-agent swarms**, which the owner made standing on 2026-09-24 and `CHARTER_PROGRAMME.md` now records: leaving the tutorial files nothing ([§ D993](../DECISIONS.md), 3–0 on the outcome and 2–1 that § D529 is kept, the dissent recorded under it) and tenant-floors is offered as authored cohorts ([§ D1001](../DECISIONS.md), 2–1). **A day 3 that could never finish** ([§ D966](../DECISIONS.md), issue [#593](https://github.com/mrpeanut01/elevator-sim/issues/593)): the viewer asked two wrong questions about whether a template varies its mix, and 1 157 day configurations on the thirteen whole-day towers were ones `core` refuses, plus 182 on the period path; a sweep of every contract, day and weekday now finds none. **The week survives other modes** ([§ D964](../DECISIONS.md), [§ D965](../DECISIONS.md), issue [#594](https://github.com/mrpeanut01/elevator-sim/issues/594)): a career day stands on a week of its own, and mode weeks are never saved as the live one. **Press days are chosen from the front door** ([§ D973](../DECISIONS.md), [§ D974](../DECISIONS.md), issue [#595](https://github.com/mrpeanut01/elevator-sim/issues/595)) rather than written onto date seeds, gated on the horizon the press will run; five office pins were re-measured on the whole day and pressed in the lunch act, applied at integration from a patch whose SHA-256 was checked against the lane's. **The report says what a press did** ([§ D982](../DECISIONS.md)–[§ D984](../DECISIONS.md), issues [#596](https://github.com/mrpeanut01/elevator-sim/issues/596), [#597](https://github.com/mrpeanut01/elevator-sim/issues/597)), and the brief asks the goals its own press grades, held by a new declared pair. **A whole day plays in about forty minutes** ([§ D991](../DECISIONS.md), issue [#592](https://github.com/mrpeanut01/elevator-sim/issues/592)); legibility re-measured on the day as played moves the first-session set from fourteen towers to fifteen, the third slice-against-whole-day mismatch in two waves, and `LEGIBILITY_SWEEP` rows now carry their horizon. **Fix a building's editor writes all eighteen answers** ([§ D1000](../DECISIONS.md)); the menu is not retired, because § D706 § 7's playtest comes first. **Integration found four defects no lane could see**: a new Scenario press that did not release a standing career day, a rush press that did release one against [§ D548](../DECISIONS.md) clause 5, a test asserting the mechanism #594 removed, and an allowlist entry a lane's new caller made stale. Two new test bounds were lowered to the project ceiling rather than registered: they had stood at 64× and 105× their measured jobs, and at the 300 000 ms ceiling the headroom is 21× and 53×. **The corpus moved 1 909.73 strings a case always-on and 2 043.10 deep, with one surface in each**, and a probe tallying every always-on case by producer on both trees reproduced the corpus exactly and found three forecasts exact and every floor held. **What this wave did not do is re-score the game**; that is a run, not an argument |
| **The game layer, wave AI (a press stays on its own day, a fix-it verdict true of the run it names, a press that is a call, and a newcomer's first day a measured one)** | **Not a phase verdict, a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Six build lanes against the post-AH panel (P1 5, P2 5, P3 5.5, P4 5, honesty 6.5; termination **no** four times out of four). **Three questions were ruled by three-agent swarms before any lane started**: the fix-it judge, the press moment and the first day. **A press belongs to the day it was made on** ([§ D1002](../DECISIONS.md)–[§ D1004](../DECISIONS.md)): presses no longer leak into later days and replays, the address bar stops describing a mode's run, and a banked today opens tomorrow from the door. **A fix-it verdict prints the case's diagnosis only over the run it was written about** ([§ D1011](../DECISIONS.md)), and an eleventh honesty property, `unbacked-attribution`, holds every string that credits an act to a run. **A fix is judged on fifty mornings** ([§ D1020](../DECISIONS.md)): the swarm split three ways on the count, and `CLAUDE.md`'s own rule (*ten is not enough*) settled it at k = 50. Fourteen cases pass, three are held from the hub, one was re-authored, and the repair menu retired. The wait was measured on the built bundle (median 6.3 s, p90 18.4 s, 34 to 46 s on Vertical City's every-deck case) and is the next panel's to judge. **The press is a call** ([§ D1029](../DECISIONS.md)): the stage stops once at an instant it draws, and a pin is admitted only if it holds over a window of at least 120 simulated seconds from that instant. Six pins were admitted (three re-pinned) and one refused with its reason. **Sentences true of the run** ([§ D1038](../DECISIONS.md)–[§ D1041](../DECISIONS.md)): a wrinkle no longer takes a booked car, the brief prints booked times, and the report's destination card stops stating a mechanism. **A newcomer's first scored day is a pinned press day** ([§ D1047](../DECISIONS.md)), drawn off the date and stored nowhere. **Integration found three defects no lane could see**: a call card whose inline `display` outranked its `hidden` attribute, two telemetry control keys the retired menu left with no press, and eight browser cases still driving the tree before this wave. The container was also reclaimed three times while unattended runs were in flight, which is recorded so the next integrator keeps a tracked job alive. **The corpus moved 304.02 strings a case always-on, with three surfaces**, attributed exactly by a producer probe. **What this wave did not do is re-score the game**; that is a run, not an argument |
| **The game layer, wave AJ (a call on every scored day, a week a player can win, a campaign a player can play, and a fix that is a search)** | **Not a phase verdict, a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Eleven lanes against the post-AI panel (P1 5, P2 5, P3 6, P4 4.5, honesty 6; termination **no** four times out of four), and four questions ruled by three-agent swarms first. **The call is the loop** ([§ D1138](../DECISIONS.md)): up to three calls on a scored day, admitted where the answers measurably change who waits; 75 % of unpinned days call and every legible whole day does; decisions per real minute on a Midtown whole day go from 0.025 to 0.098, and the longest quiet stretch from 40.5 to 17.4 minutes, short of the 2.5 the player lens wanted because the peaks are hours apart. The first closed attempt banks and a retake is practice. **Fixing a building is a search** ([§ D1120](../DECISIONS.md)): the diagnosis waits for a free press, the check counts its mornings with the controls live (median 4.2 s, worst 17.0 s), the interval drops the letter's morning, and a fix may stop early only to say it is not fixed. **The campaign is playable** ([§ D1129](../DECISIONS.md)): one admission check for the Lab and the census, stages played from the hub, a first clear pays and unlocks nothing. **A week can be won** ([§ D1066](../DECISIONS.md)–[§ D1067](../DECISIONS.md)): growth is data, DC-10 makes winnable a measured claim, and the bars never move. Also: wrinkles spliced as episodes ([§ D1057](../DECISIONS.md)), the whole dispatcher adopted mid-day ([§ D1048](../DECISIONS.md)), the day on screen one record ([§ D1093](../DECISIONS.md)–[§ D1099](../DECISIONS.md)), sentences true of the run with four declared pairs ([§ D1102](../DECISIONS.md)–[§ D1107](../DECISIONS.md)), the call in view and the phone laid out ([§ D1111](../DECISIONS.md)), four filed issues ([§ D1075](../DECISIONS.md)–[§ D1078](../DECISIONS.md)), and the queue bar pinned ([§ D1085](../DECISIONS.md)). **Three rules already in force settled three of the swarm's splits**: the hint is free and a clear unlocks nothing (`docs/38`), and there is no early *fixed* (`CLAUDE.md`'s replication budget). **The corpus moved 534.08 strings a case always-on and 535.90 deep, with six surfaces each, all forecast by name**, and wave AI's owed deep reading was taken. **What this wave did not do is re-score the game**; that is a run, not an argument |
| **The game layer, wave AK (a day that keeps asking, a week with a stake and an ending, and the campaign played through)** | **Not a phase verdict, a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Seven lanes against the post-AJ panel (P1 5, P2 6, P3 6, P4 4.5, honesty 7; termination **no** four times out of four), whose shared finding was that a scored day is still mostly watching. Three defect lanes came first, then two three-agent swarms ruled and four lanes built the rulings. **A day keeps asking** ([§ D1166](../DECISIONS.md)–[§ D1169](../DECISIONS.md)): a scored day runs at 30× while nobody has waited a minute; where park or spread would change nothing the stage may ask who drives the rest of the day, through the adopt-dispatcher switch and § D1138's admission rule; up to six calls; no call once the day is lost, and an End the day control. On a Midtown day 1 decisions per real minute go from 0.098 to 0.184 and the day from 40.5 to 27 minutes; the longest gap only falls from 17.4 to 15.3 minutes, because it is the stretch between peaks. **A week has a stake and an ending** ([§ D1176](../DECISIONS.md)–[§ D1180](../DECISIONS.md)): only census-admitted days count, the target is those days minus one, and Midtown reads 4 of 5 after Tuesday's and Friday's wrinkles moved out from under the lunch peak; the week closes on a sheet beside the house and rolls; a tower whose week counts no day has its offer held with a reason that separates measured from unmeasured days. **The campaign plays through** ([§ D1183](../DECISIONS.md)–[§ D1185](../DECISIONS.md)): the survivor census presses the stage page's own choices, stages 2, 6, 7 and 8 open, and fix-it shows a par. Also: one record of the day ([§ D1139](../DECISIONS.md)–[§ D1143](../DECISIONS.md)), the report from one measurement ([§ D1148](../DECISIONS.md)–[§ D1154](../DECISIONS.md)), and fix-it and campaign defects ([§ D1157](../DECISIONS.md)–[§ D1160](../DECISIONS.md)). **The deep corpus caught what no lane could**: naming goals in words broke R12's exemption for the kinds it never judges, 22 cases in 60, fixed by carrying the kind rather than reading the words. **The corpus moved 129.20 strings a case always-on and 131.02 deep, with one surface each, forecast by name.** Queued, not done: stages whose budgets bind nothing, two stages above their bands, Secure Tower's and Harbour Point's unmeasured days, and a report sentence that reads like an unlock. **What this wave did not do is re-score the game**; that is a run, not an argument |
| **The game layer, wave AL (a day half as long that asks all day, one attempt per day, a week worth coming back to, and answers not given away)** | **Not a phase verdict, a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Seven lanes and two integration fix lanes against the post-AK panel (P1 6, P2 6, P3 6, P4 5, honesty 7.75; termination **no** four times out of four), whose shared finding was that a scored day ran 25 to 30 minutes of mostly watching. A diagnosis found the newcomer's silent Monday was not load but § D1138's rule that a pinned press day raises no ordinary calls, made every newcomer's day by § D1178. Two defect lanes came first ([§ D1186](../DECISIONS.md)–[§ D1197](../DECISIONS.md)), then swarm DN ruled and five lanes built it. **The day asks all day** ([§ D1204](../DECISIONS.md)–[§ D1206](../DECISIONS.md)): the pinned day opens its ordinary calls after its own, at most two calls a peak, no question repeated within ten minutes, the driver question back after a handover, and a present-facts line on the card with no forecast; over Midtown's five weekdays decisions per real minute go from 0.152 to 0.225 and rows with a call in every peak from 11 to 32 of 42. **Skip what nobody decides** ([§ D1212](../DECISIONS.md)): between peaks, while nobody has waited a minute, the stage seeks to the next moment that matters and says what it skipped; a Midtown weekday goes from 28.8 to 14.2 real minutes, decisions per minute from 0.157 to 0.344 and the longest gap from 18.2 to 8.3 minutes, with the legs identical. **One attempt per scored day** ([§ D1218](../DECISIONS.md)–[§ D1219](../DECISIONS.md)), resumed rather than rewound, and each call's row on the stage once its window is observable. **A week worth coming back to** ([§ D1226](../DECISIONS.md)–[§ D1231](../DECISIONS.md)): the target marked on the day, a Sunday screen against the house, a continue tile, new crowds each week, a record that buys nothing, and a counted Scenario day that pays its chimes. **Answers not given away** ([§ D1233](../DECISIONS.md)–[§ D1235](../DECISIONS.md)): the hub shows totals until a clear, sixteen fix-it notes stop naming the fault, stages 7 and 8 re-authored against a criterion written first, and par marks that pay nothing. **Integration found the lanes' seams**: a snapshot that would have resumed a day without its call memory, an attempt and a close reading different crowds, two length tables measured under different rules, and a gated sweep missing from its registry. **The corpus moved 142.24 strings a case always-on and 143.87 deep, with one surface each, forecast by name.** **What this wave did not do is re-score the game**; that is a run, not an argument |
| **The game layer, wave AE (the score the wave exists to move)** | **Not a phase verdict — a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Four build lanes against the playability assessor's ranked list, plus a fix lane and two decision agents. The assessor had scored the game **P1 3, P2 6, P3 4, P4 3** against a bar of 8 with the engine floor at 8–9, and its judgement was the brief: *"a superb ninety seconds is followed by a mode that repeats one unchanging day."* **That day is gone.** GitHub issues **#563** and **#564**: `ViewerState.seed` had **no writer in the Everyday product at all** — born once at boot from the UTC date ([§ D729](../DECISIONS.md)) and never rewritten — and the tenant growth the report announced was true of `week.day`, a counter `campaign/career.ts#fileDay` never advanced while it incremented `CampaignTower.day`. So `TENANTS 120 → 135` was literally `week.day 1 → 2` and a contract re-ran day 1 for a month. Measured on the legs through the shipped press, ten consecutive days now read 60, 67, 40, 56, 50, 73, 58, 51, 76 and 59 legs against **four byte-identical 44-leg days before**, with the tower growing 120 → 240 tenants and a calendared `move-in` — an event `campaignEventFor` could not return at all — firing on day 4 ([§ D840](../DECISIONS.md), [§ D841](../DECISIONS.md)). **And the second half of that issue was refused on measurement rather than built**: the incident draw was **already** correctly per-day seeded and fires a breakdown on day 17 at this seed, so ten silent days was the **96 % outcome** of `FRESH_ODDS_PCT = 0.4`. That constant is § 8.3's own figure and is **deliberately unmoved**; the fix is a repertoire and a calendar, priced at 18.9 %/day with **98.5 %** of contracts carrying at least one event over twenty days, measured 197/200. The lane also **wrote a dead seam and deleted it inside the same commit** — a technician row no weekday template declares — which is this repository's oldest rule applied to a lane's own work rather than caught later. **The rush stopped misleading** ([§ D856](../DECISIONS.md)–[§ D859](../DECISIONS.md), issue #565): the house board's advertised figure was **reachable all along** and no screen inside the rush offered the route, so a route was built rather than a disclaimer — four arms on Harbour Point hold at 2 766 s set before the run, 2 480 s untouched and 2 364 s handed over at 0:00, and arm D proves a handover's whole effect is the weight substitution, **bit-identical on the legs**, a mechanism measured rather than quoted. **Fix-it stopped printing its own answer** ([§ D869](../DECISIONS.md), [§ D870](../DECISIONS.md), issues #566 and #568): the tell was on **all eighteen** diagnosed repairs rather than the seven the issue named, and underneath it the answer was **row one on 18 of 18** — now drawn in a deterministic hash order, landing first on five cases and fourth on six. Each of the eighteen is demonstrated solvable **by a test that solves it**, and the census corrects § D706 § 1 from *1 of 18* to **12 of 18** solvable from the editor alone, published as a floor. #568's hypothesis was **refuted**: the cause is the air-pressure descent cap biting a bank that travels 307.5 m against a 300 m threshold, so lifting the cap alone is 9 → 9 and lifting it *with* the half metre is 9 → 1 — `CLAUDE.md`'s `symmetric-speed` exemption arriving in gameplay, produced by a patch. **Two goal sets were made legible rather than reconciled** (issue #567): the issue said only one was graded and **both are**, deciding different things, so each now says what it decides and a new test **measures** the divergence over four tiers × twenty days so the copy obligations relax on the commit that ever reconciles them. **Two defects on the first screen a player meets** ([§ D844](../DECISIONS.md)): the landing page rebuilt its whole subtree when its simulated morning landed, taking focus off a keyboard reader's only button a third of a second after they reached it; and the front door painted three mode tiles ~240 ms before it could answer who was arriving, so a *Scenario* press in that window opened *"the simulation host has not finished booting"* — § D227's first direction on the first screen anybody sees. **The paused stage was settled by citation rather than judgement** ([§ D842](../DECISIONS.md)): four handoff documents rule it, all quantifying universally, one recording it as added in answer to the client's review notes — and the prototype, which autoplays on ten of eleven transitions and holds no `Start` control at all, loses under the handoff's own precedence table. **The corpus moved by exactly 27.00 strings a case in both tiers and four published forecasts sum to it precisely**, which has not happened before; the row was measured twice, on the head after a late fix, rather than resting on a structural argument. **What this wave did not do is re-score the game** — no assessment has run since the changes landed, and the termination condition is a run rather than an argument. Three findings are filed rather than fixed: **#570** (the honesty corpus never reads `data/fixit-cases.json`, so 22 player-facing sentences are unswept and #233 doubles the area), **#571** (ten of twelve dispatchers change no leg on a full `garden-apartments` day, so the opening tower is too small for a player's first choice to matter) and **#572**/**#573** (two copy questions the lanes declined to answer alone) |
| **The game layer, wave AD (the mandate changed)** | **Not a phase verdict — a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Six build lanes, a parity panel and a playability assessor who drove roughly twenty sessions. **The headline is not a feature, it is that the finish line moved.** The panel convened to decide when autonomous work stops had been given five axes — depth, correctness, reachability, legibility, honesty — and every one of them is an engineering axis, so **not one would notice a game that is correct, honest, fully reachable and boring**. [`docs/43-what-winning-looks-like.md`](43-what-winning-looks-like.md) promotes four player axes above them (game-play, usability, understandability, entertainment), each with a runnable protocol, and demotes the old five to a floor that gates rather than wins. Two protocol details carry the weight: the decision census reports the **longest gap** rather than the average, because an average hides five minutes of watching; and the prediction test fails at **both** tails, because near-chance means the game is noise and near-perfect means it is arithmetic. **The engine's answer to that is § D802: a run can be stopped and resumed, byte-identical across seven chunkings and four configurations.** `Simulation.run()` had been the only method that advances time, so every dispatch decision was taken before the first frame was drawn and `docs/43` P1's central moment — the tower developing a problem in front of you — was *structurally unavailable* rather than unbuilt. The capability existed one layer down (`kernel.ts` already implements `run(until)`) and `Simulation` did not expose it; fork-at-`t` was assessed and needs nothing new. What else a player meets: GitHub issue **#557** closes, and it is the first dead seam in this repository's history a player **pays for** — a priced top-up with nowhere for a granted unit to land ([§ D738](../DECISIONS.md)); five session-shape figures on the last screen before a player commits their evening stop quoting a speed no rung ships, composed from the rung so the next rung move carries them ([§ D753](../DECISIONS.md), issue **#559**); three more Parameters-tab schemas reach the run and the lobby-crowding loop becomes reachable whole ([§ D761](../DECISIONS.md)–[§ D765](../DECISIONS.md)) — **and that lane's fifth number is a finding it could not fix**, that nineteen declared rows draw no control at all, filed as **#562** rather than rounded up; Scenario stops offering a purchase nothing sells and a stage row opens its own stage through a provided port ([§ D786](../DECISIONS.md), [§ D787](../DECISIONS.md)) — the second half is what wave AC-2's row promised *as rows* and could not deliver. **Four audits had their scope derived from disk rather than named**, and the first thing the derivation found was the directory the core audit was named after ([§ D836](../DECISIONS.md)–[§ D838](../DECISIONS.md)); the `experiments` audit found **83 uncalled exports in four directories nobody had listed**. **One proposed fix was refuted by the lane asked to make it** ([§ D839](../DECISIONS.md)): the barrel disposition offered for `dispatch/isParameterActive` does not close it, because two of the three names are the ones that move. **The corpus moved by exactly 4.00 strings a case in both tiers, all of it on one surface, and three forecasts scored right** — second time that has happened. The move is attributed by a probe that rendered one case on each tree and tallied by producer, not by dividing 196 by 49, which is the distinction [§ D256](../DECISIONS.md) exists to protect. **Three corrections to this repository's own record**, the sharpest being a directory count retired on its **fourth** wrong value (19 → 27 → 28 → 32) after three of its corrections had been made *inside the paragraph arguing that the figure should be derived rather than quoted* — at which point the paragraph was exhibiting the defect rather than describing it. **What the wave did not do is move the game's score.** The playability assessor scored P1 3, P2 6, P3 4, P4 3 against a bar of 8, with the engine floor at 8–9, and its closing judgement is the brief for wave AE: *"This is not a product that cannot entertain; it is one where a superb ninety seconds is followed by a mode that repeats one unchanging day."* Issues **#563**–**#569** carry the ranked list |
| **The game layer, wave AC-2 (playability)** | **Not a phase verdict — a wave, and the figures are in [`CLAUDE.md`](../CLAUDE.md)'s Phase 9 row.** Ten build lanes and six decision agents, dispatched against a diagnosis three research agents reached independently: the engine is years ahead of the game layer, and the sixty commits before this wave widened the gap. What landed that a player meets: the stage opens at `4×` rather than `30×` ([§ D641](../DECISIONS.md), discharging § D525 clause 4, which had been ruled and unimplemented for thirteen days) and draws people rather than 4.5 px capsules; a fix case plays the run **the player's own change produced** beside the as-built one at one playhead ([§ D644](../DECISIONS.md)) — common random numbers made visible rather than asserted in a basis line; the ten campaign stages reach the Scenario hub as rows, each drawing its own measured survivor count ([§ D649](../DECISIONS.md)) — **read *as rows* precisely**, because the same assessor found the three live buttons all call `enterEngineer()` with no argument, so pressing a stage row lands the player on the engineering surface with no stage selected. The rows arrived; a row opening its own stage did not, and wave AD is building it; the report leads with a figure the run produced when the mean is refused, the refusal untouched ([§ D666](../DECISIONS.md)); the chimes loop gains an earn acknowledgement and a spend ([§ D671](../DECISIONS.md)–[§ D673](../DECISIONS.md)) — **and this row said *closes at both ends*, which was false and is corrected here rather than quietly reworded.** `data/chime-ledger.json` carries three sinks: two purse top-ups, **both non-functional at the time of that claim** (the career one is GitHub issue **#557**, a priced purchase with nowhere for a granted unit to go, and the rush one has a separate cause its own `$comment` names), and `rush-prefit`, which is offered in the **third** mode and is account-gated and server-gated besides. A bundle built without an `apiOrigin` has no ledger at all (`dev/main.ts:1250` returns no client), and the deployed bundle is built that way. So the earn end closed and the spend end closed **in one mode, for a signed-in player, against a server** — which is a real thing to have built and is not what the sentence said. *Amended 2026-09-22, GitHub issue **#579**, [§ D911](../DECISIONS.md): the clause *"a bundle built without an `apiOrigin` has no ledger at all"* was exactly true of that wave and stopped being true on that commit. A device ledger ships (`packages/viz/src/everyday/deviceChimes.ts`), `dev/main.ts#bankCompletion` is defined on every build, and a **scenario's budget rung** — priced by the scenario in `data/fixit-cases.json`, so it needs no sink and no route — is bought from it. Measured: the diagnosed repair plus the costly one at `zoning-starves-the-top`, refused at its 12 u base and admitted at the 18 u rung, **129 of 200 boarding legs differ** (`packages/viz/src/fixit/budgetRungReachesTheRun.test.ts`). **Stated at exactly that width**, because this row is where the last over-claim was corrected: the loop closes **for one sink in one mode, with no account and no server**. The three ledger sinks still need an account, no rush rebuild travels, the ten campaign stages sell no rung, and `docs/43` P4's tomorrow question is not declared answered — #579 makes an assessor who plays the acceptance test and it has not been re-run.* Found by the parity panel's adversarial assessor; a run is shareable without publishing an input ([§ D685](../DECISIONS.md)); the day's crowd is the date's own digits, so *the same for everybody* became true rather than being reworded ([§ D729](../DECISIONS.md)–[§ D733](../DECISIONS.md)). **And one measurement is worth more than any of them**: a dial witness exists at stage 1 — three of 200 draws, all buying `idle-parking`, the dial that stage's own brief teaches ([§ D691](../DECISIONS.md)). *Moving a dial does win, on shipped content, by the intended route.* It was found by searching rather than rebalancing, so **no shipped difficulty moved**. What it does not do is make the ladder generous: 3 of 200 is 1.5 % against a band floor of 0.25, stage 2 is a measured null at 0 of 60, and seven of ten stages still have no way through — GitHub issue **#558**, filed with the traps named so the next lane does not pay for them twice |
