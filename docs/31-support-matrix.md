...
|---|---|---|
| Linux, x86-64 (`ubuntu-latest`) | **Chromium** headless shell, from `playwright-core` | 69 `*.browser.test.ts` files, driven through a real Vite dev server against the built `core` |
...
then; the tier holds 69** and the timing has not been re-measured, so read the percentage as the
...
|---|---|---|---|
| **Firefox** on the existing Linux leg | Tier 3's largest claim becomes a fact. Gecko is where the canvas and `@container` assertions are most likely to differ | ~157 s per leg when the tier held 25 files and the tier holds 69 now, so somewhat more, plus one more Playwright browser download (size unmeasured — `playwright-core install firefox` reports it), and a real risk of an initial burst of engine-specific failures that are the product's, not the tier's | **Recommended, and the highest-value single addition.** Run it on the **Linux leg only** — the engine is the variable, not the host OS |
...
7. **One of the two counts in this document that were read off the tree drifted, exactly as this
   ...
   the two documents disagreed with each other as well as with the tree. The tier holds **69** now
