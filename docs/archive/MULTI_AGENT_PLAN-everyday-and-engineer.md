> **CLOSED RECORD — the Everyday-and-Engineer wave, as it was planned.** This is the body that
> `MULTI_AGENT_PLAN.md` carried until 2026-08-24, moved here unchanged when that path was rewritten
> for the charter programme. It is kept because `WAVE5_PLAN.md` § 1, `WAVE6_PLAN.md` and
> `packages/viz/UX.md` all cite `MULTI_AGENT_PLAN.md` by path for material that now lives here, and
> a citation a reader cannot follow is the defect `validation/citations.test.ts` exists to catch.
>
> **Nothing in this file is current.** The wave it plans landed in full; what it left behind is in
> `AGENT_STATUS.md`, and what supersedes it is `MULTI_AGENT_PLAN.md` and `CHARTER_PROGRAMME.md`.
> Do not schedule from this document.
