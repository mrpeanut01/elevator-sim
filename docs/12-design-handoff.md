...
| `#loop` | Restarts at the start of the loop when the transport reaches its end | `PB-06`, `PB-T7`. **Whether** the transport repeats; `#loop-window` beside it is **which part** |
| `#loop-window` | The `A–B` repeat — three presses mark a span's start at the playhead, then its end, then clear it | `PB-09`, built by GitHub issue #417 and no longer the ledger's one 🔲. `aria-pressed` takes all three of its legal values here (`false`, `mixed`, `true`), the marked span is drawn over `#timeline` as `#loop-band`, and the cycle is `playback/loopMark.ts#nextLoopMark` so the mount's listener decides nothing. The handoff has no requirement row for it — its prototype transport has no loop at all — so this is the same *obligation this simulator has and the prototype does not* as the provenance block below |
