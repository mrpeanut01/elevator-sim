...
built · the double-deck closed-form RTT · a **sweep** behind Phase 6c's one-operating-point refusal.
...
`run` / `tune` / `watch` · the resolution limits **unmeasured on TTD** · **W4's U7 half blocked on
a `core` fix** · `stopCount` with no `activeWhen` ·
