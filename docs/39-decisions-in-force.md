...
| [§ D539](../DECISIONS.md) | The counterweight's balance ratio (0.4–0.5, default 0.5) and a regenerative drive are **per-bank equipment that move energy and never a leg**, by the owner's ruling of 2026-09-10 (GitHub issue #431). They move `energyKJ`, `workPerServedLegKJ` and the 80 kJ goal's verdict; `metrics/types.ts`'s refusal to configure the ratio is moved, its reason answered by a per-sample record of the convention and a comparability disclaimer; #431's AC3 is restated on the goal verdict with the legs compared whole. **Its fourth criterion found a null for the decision**: every measured optimum lies below 0.4, so the band's best setting is 0.4 under every traffic pattern, and whether the lever stays a dial is the owner's to reconsider. The recovery fraction and both prices are drafts awaiting the owner |
