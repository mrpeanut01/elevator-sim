...
| `docs/25` § 1.1 | The slice is one day in *Today's tower* at Chancery House | **Amended.** The slice is unchanged; its mode is *today's scenario* inside Scenario rather than a tile | § D525 |
...
| `docs/12` § 4 | The design handoff wins every disagreement about what the screen looks like | **Narrowed.** Still true; two deviations are now recorded in its own register — § 4.14 the four-mode session table, § 4.15 the authored repair lists | § D525 |
