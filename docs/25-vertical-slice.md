...
| **X6** ＋ | **The slice's day is graded as it is played**: on the pinned seed the before-run misses at least one of the day's goals and the after-run meets every one. Stated without a count deliberately: the day asked four when this row was written and asks five since [§ D468](../DECISIONS.md#d468) added the energy bar | One run pair through `shift/goals.ts` and `shift/report.ts`, asserted in a test | **No** — nothing asserts it. Measured: the goal grid files this day as **cleared on 27 of 30 seeds** | The before-run files as a clean shift, or the after-run does not clear |
