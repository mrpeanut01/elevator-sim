...
| **M30** | **The goal pass-rate table**, § 11 **W9**, pinned in `data/scenario-goals.json` and re-derived by `packages/viz/src/scenario/goalRates.test.ts`. Ten stages — § 5.4's seven and three added with the buildings that came later — `collective`, `durationS: 900`, `onTimeout: 'report'`, two **disjoint** seed sets of **50** replications each (masters `20260730` and `20260731`, disjointness checked over the derived `replicationSeed`s). `long-waits-under` at **≤ 10 %**, M18's threshold. Cells are `tuning, holdout`. See the table below. Of the 50 (goal × stage) cells with a per-run predicate: **16 batch goals, 32 configuration facts, 2 withheld** — and **0 single-run goals**, which is R12's own trichotomy applied without slack rather than a threshold anyone chose. Those four figures are **counted from `data/scenario-goals.json` by `goalRates.test.ts`**, not transcribed: they read `17, 29, 4` over a table holding `16, 32, 2` for as long as the cells beneath them were re-derived one at a time and their summary was re-derived by nothing — `RISKS.md` **R38** on the very paragraph that introduces the fix for it. The third bucket's **name** was stale with its count: it said *unjudgeable*, and no cell here has an unjudgeable rate class. Both cells in it are withheld because their two seed sets disagree about the **kind** of answer, which is the other route to that bucket and the only one that currently fires. **Re-measured for § D265**, which put `deliver-everyone` back on stages 4, 5, 6 and 10 and withheld it on 9: the credential gap turns a declared share of in-building journeys away for want of a badge, so *"everybody who arrived was carried"* is a question again on every building that declares `accessZones`, and a constant on the three that do not. Stage 5 is now the richest cell in the table — four of its five goals are variable — which is the stage the credential lesson is on, and that is not a coincidence. |
