...
| **M30** | **The goal pass-rate table**, § 11 **W9**, pinned in `data/scenario-goals.json` and re-derived by `packages/viz/src/scenario/goalRates.test.ts`. Twelve rows — § 5.4's seven stages, three added with the buildings that came later, and **two engineering briefs** (GitHub issue **#227**, `docs/21` § 4), all `durationS: 900`, `onTimeout: 'report'`, two **disjoint** seed sets of **50** replications each, disjointness checked over the derived `replicationSeed`s. The ten stages run on masters `20260730` and `20260731`, eight of them under `collective` and stages 7 and 8 under `zoned-uppeak` since wave AL re-authored them against a criterion written first ([`DECISIONS.md`](../DECISIONS.md) § D1235); the two briefs run `energy-aware` on their own masters `20260910` and `20260911`, because a brief is a different question on a different configuration and sharing a master would make *"the same fifty traces"* true across two bodies of content that were never meant to be compared. `long-waits-under` at **≤ 10 %**, M18's threshold. Cells are `tuning, holdout`. See the table below. Of the 60 (goal × stage) cells with a per-run predicate: **22 batch goals, 35 configuration facts, 3 withheld** — and **0 single-run goals**, which is R12's own trichotomy applied without slack rather than a threshold anyone chose. Those four figures are **counted from `data/scenario-goals.json` by `goalRates.test.ts`**, not transcribed: they read `17, 29, 4` over a table holding `16, 32, 2` for as long as the cells beneath them were re-derived one at a time and their summary was re-derived by nothing — `RISKS.md` **R38** on the very paragraph that introduces the fix for it. The third bucket's **name** was stale with its count: it said *unjudgeable*, and no cell here has an unjudgeable rate class. All three cells in it are withheld because their two seed sets disagree about the **kind** of answer, which is the other route to that bucket and the only one that currently fires — and the third arrived with the briefs on the same boundary as the first two, at 49/50 against 50/50, which is what says the boundary is a real one rather than the hotel's own. **Re-measured for § D265**, which put `deliver-everyone` back on stages 4, 5, 6 and 10 and withheld it on 9: the credential gap turns a declared share of in-building journeys away for want of a badge, so *"everybody who arrived was carried"* is a question again on every building that declares `accessZones`, and a constant on the three that do not. Stage 5 is now the richest cell in the table — four of its five goals are variable — which is the stage the credential lesson is on, and that is not a coincidence. |
| **M31** | **M18 is corrected by M30 at a larger `n`.** Secure Tower, `collective`, shipped demand — M18's own cell, at 50 seeds rather than 20 and on two disjoint sets. `deliver-everyone` **5/50, 11/50**; `nobody-abandoned` **50/50, 50/50** (M18: 20/20 — reproduces); `long-waits-under (≤ 10 %)` **14/50, 12/50** (M18: 11/20 — reproduces as variable); and `answer-the-demand` **5/50, 9/50**, which M18 recorded as a **constant** at 0/20. `deliver-everyone`'s row has been read three ways and each reading was a measurement: **0/50** while § D254's defect stranded riders on restricted landings, **50/50** once that was fixed, and **5/50, 11/50** since § D265 gave a share of in-building riders the badge their own floor implies. The number to distrust is the middle one, and nothing in the code said so — only the re-run did. It is a very low-rate variable, so R12 makes it a **batch goal** on that stage rather than a briefing fact. A classification taken at twenty seeds moved at fifty: § D158's *"a level validated at n = 20 can suppress at n = 50"* arriving on a goal instead of on an estimate. |
...
on both is a **batch goal**; a cell is **withheld** — neither — when its two seed sets disagree about
which of those two it is, or when any run in it is unjudgeable. The first of those two routes is the
one that fires here; the second currently withholds nothing, and saying so is the point, because
this paragraph named only the empty route for as long as the totals above it were wrong.
...
| **7 prove it** | `midtown-office`, 1.25 % ¶, `zoned-uppeak` | 50/50, 50/50 | 50/50, 50/50 | 50/50, 50/50 | **33/50, 33/50** | **43/50, 49/50** |
| **8 the headline address** | `chancery-house`, 3 %, `zoned-uppeak` | 50/50, 50/50 | 50/50, 50/50 | 50/50, 50/50 | **38/50, 40/50** | 50/50, 50/50 |
...
| **11 E5 meet the handling-capacity target** | `chancery-house`, 2.5 %, `energy-aware` | 50/50, 50/50 | 50/50, 50/50 | 50/50, 50/50 | **31/50, 32/50** | 49/50, 50/50 ‡ |
| **12 E3 diagnose the saturation** | `mixed-use-high-rise`, shipped, `energy-aware` | **4/50, 5/50** | **36/50, 35/50** | 50/50, 50/50 | **6/50, 7/50** | **14/50, 19/50** |
...
**Rows 7 and 8 start on `zoned-uppeak` since wave AL, and it is a re-authoring rather than a drift** ([`DECISIONS.md`](../DECISIONS.md) § D1235). Under `collective` every one of stage 2's five ways through cleared both stages, so neither asked for anything stage 2 had not already answered. The starting setting is the only field authored differently, and stage 7's third goal follows from it; demand, horizon and seeds are as they were, and each cell is the new setting's own count, re-measured rather than authored. Stage 7's `answer-the-demand` cell read **31/50, 26/50** under `collective` and reads **33/50, 33/50**; its `long-waits-under` cell was a constant under `collective` and is a batch goal under `zoned-uppeak` at **43/50, 49/50**, so the stage now declares it: `campaign/parse.ts` refuses a stage that leaves out a goal the table measures as live there. Stage 8's `answer-the-demand` cell read **32/50, 30/50** and reads **38/50, 40/50**.
