...
| Next free decision number | **D738** — wave AC-2, the playability wave, reserved **D641–D741** across sixteen blocks: ten build lanes and six decision agents dispatched in parallel from one base, each holding a contiguous block allocated before any of them started. **Seventy numbers came back unspent** and are registered in `documentation.test.ts#KNOWN_DECISION_HOLES`, each with the reason its lane gave rather than a bare id. The spent headings are **D641** (the stage opens at 4×), **D644** (the fix-case pair), **D649** (the ten campaign stages reach the Scenario hub), **D666** (the report's headline under a refused mean), **D671–D673** (the chimes loop), **D685** (the shareable result), **D691–D693** (the dial witness, stage 2's measured null, and one refused replication in fifty suppressing an estimate row), **D701** (§ D606's three rush questions, already answered by § D640), **D706** (the fix-case repair menu retired from the screen and not from the file), **D711** (the device chime ledger records turns, not a balance), **D716–D718** (#536's four asks and #231's AC2), **D721–D723** (AD-M4's scope, #429's figures, and seven stale counts), **D729–D733** (the daily seed is the date's own digits), **D736** (the dial sample size is an instrument's precision, not a difficulty setting), and **D737** (the product owner's ruling that Rush stays on the front door, superseding § D626's placement clause). **The hole list was derived from the headings rather than from the lanes' reports, and that caught two wrong ones**: D656–D660 and D679–D682 were reported spent and head nothing — both lanes had correctly recorded their decisions as § D405 docstrings and mis-reported the bookkeeping. **D738–D741 are free rather than holed**, on wave P's own precedent: D737 is this tree's highest heading, so nothing is written past them for them to be a gap below, and registering them would point this row's `highest + 1` at a hole — the one place § D430 forbids one. |
