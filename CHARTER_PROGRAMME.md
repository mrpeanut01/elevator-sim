...
| Next free decision number | **D622** — wave AC's reserved block, originally sized for two (**D619–D620**), is closed at **five spent of six numbers ever spent across it**, because three lanes collided on it rather than two: **D615** (#540, the revert procedure's step 2 made structurally safe), **D618** (#408, charter S9 B1's run-history store and rolling gate), **D619** (#422, fix-a-building's elevation control and its price-schedule row), **D620** (#437, destination panels priced per landing, drafted as D619 on its own branch and renumbered on merge), **D621** (#412, TWIN shafts — a shaft model, the separation constraint and the deadlock property — drafted as D620 on its own branch, colliding with #437's post-renumbering D620, and renumbered again). **D616 and D617 remain permanent holes**, from the earlier #540 stale-base collision. **#422 and #437 collided on more than the number**: `raise-a-floor`'s `covers` had claimed the whole `building.floors[]` group rather than the one field it writes (`heightM`), which would have silently blocked `landing-panels` from pricing `building.floors[].landingCallType` — narrowed to `building.floors[].heightM` on the merge commit, with `repairPrice.ts#pathsIn` and `fixit/engine.ts#editorPathsOf` updated to emit the same narrowed path so #422's own pricing lookup kept working — and both lanes' independent budget-ceiling arithmetic (412→422 alone, 412→577 alone) is replaced by the schedule's own re-derived `totalUnits: 587`. **#412 collided on the number only**: `TWIN` shafts and `raise-a-floor`/`landing-panels` touch disjoint files (`packages/core/` versus `data/price-schedule.json` and `packages/viz/src/fixit,pricing/`), so no third semantic conflict was found. Three lanes opening the same two-number reservation independently, from the same pre-merge base, unable to see each other, is the wave-Z finding recurring a third time — worth naming because the fix each time was the same: the integrator resolves on merge, never a branch guessing at its siblings. The previous wave's block was **D594–D620** and spent thirteen, with D602–D605 and D607–D610 registered as holes. |
