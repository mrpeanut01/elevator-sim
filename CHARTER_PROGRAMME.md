...
| Next free decision number | **D615** — wave AB's block **D594–D620** is closed, thirteen of twenty-seven spent, and this row is reconciled on the same commit that closes it (the step D387 records that nobody performs when nothing asks; written bare, because D387 is a registered hole, a hole heads nothing, and `citations.test.ts` resolves every section reference to a heading). Spent: **D594–D600** lane A (GitHub issues #428, #427, #426 — three reference towers — plus D600 on two authored speeds moved onto their machine class's ladder, #45), **D601** lane B (#429, shafts costing floor area), **D606** lane C (#372, the rush's per-wave purse), **D611–D614** lane D (#234, the campaign rebalance). **D602–D605 and D607–D610 are registered permanently as holes**, each sitting under a higher number a sibling lane went on to write. **D615–D620 are free rather than holes**, because nothing above them was ever written, which is why this row names D615 and not D621. The previous wave's block was **D576–D600** and spent thirteen, with D584, D585, D586, D589 and D590 registered as holes. **All four lanes opened this wave's reservation themselves, at four different ceilings, before the integrator's block reached them** — each correct from where it stood and none correct here, which is wave Z's finding arriving with a full house. |
