...
| Next free decision number | **D912** — no wave is open, so this row is `max(heading) + 1` and `documentation.test.ts` derives it rather than trusting it. When a wave opens, the integrator sets this to the block's floor and it stays there for the whole wave: the row is the integrator's input and is stale while a wave runs, which is why two lanes reading it once both computed § D336. **Take your numbers from your dispatch brief, never from this row.** Wave AF's accounting, which this supersedes as the *next free* figure but not as a record: 5 headings spent of D871–D940, fifty holes registered with each lane's reason, and D926–D940 left free rather than holed because nothing is written past D911. |
