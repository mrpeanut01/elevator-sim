...
| Next free decision number | **D641** — two independent sessions each spent numbers starting at **D623** in parallel, on branches that never shared a base after the prior wave's D622 closed, and neither could see the other's allocation. A playtest/ruling session (`main`, no wave open, `OPEN_RESERVATION` was `null`) took **D623–D630 sequentially**, one after another, ruling on seven open design questions surfaced by this week's playtest across GitHub issues [#543](https://github.com/mrpeanut01/elevator-sim/issues/543), [#544](https://github.com/mrpeanut01/elevator-sim/issues/544) and [#549](https://github.com/mrpeanut01/elevator-sim/issues/549), `docs/32-game-design.md` § 9 (Q1/Q2/Q3/Q6) and `docs/12-design-handoff.md`'s two named-but-unowned open notes: **D623** (#543, live pause-and-reassign scope), **D624** (#544, bank/zone stage legend), **D625** (#549, camera floor-jump v1), **D626** (Endless Rush demoted off the front door), **D627** (`charter` non-goal 6 amended to name stakes), **D628** (campaign failure odds should roll, scoped as follow-up), **D629** (day-goal bar is the contract's, not the tier's), **D630** (Free Play should keep posting to the week). Concurrently, the issue-list wave reserved **D623–D644** across two lane blocks dispatched disjoint on purpose — **D630–D639** for #429 stage 2, **D640–D644** for #372's `rush-prefit` sink — each spending exactly one number: **D630** (a shaft's plan area priced in four measured bands) and **D640** (`rush-prefit` authored as one fixed kit). **D630 collided**, on the number alone — a shaft's plan area and Free Play's scoring question share no file and no reasoning — and is resolved on this file's own precedent (§ D620, § D621): the later entry to reach `main` is renumbered, to **D631**, with every citation corrected on the merge that reconciled both. **D623–D629, D631 and D640 are real, spent headings; D632–D639 are permanent holes.** The issue-list wave's #429 block was deliberately set above the reservation floor rather than at it, to leave collision-proof room between two lanes dispatched in parallel from the same base, and it returned every number beyond the one it spent. **D641–D644, #372's own spare block, are free rather than holed**: D640 is this tree's highest heading, so nothing is written past them for them to be a gap below — the same distinction wave P's own row draws. The previous wave's reserved block was **D619–D621**, spent in full across two collisions (#422/#437 on D619→D620, then #412 on D620→D621); **D622** was spent separately, directly by the integrator outside any lane's block, for the bundle-budget raise. |
