...
| Next free decision number | **D840** — no wave is open, so this row is `max(heading) + 1` and `documentation.test.ts` derives it rather than trusting it. When a wave opens, the integrator sets this to the block's floor and it stays there for the whole wave: the row is the integrator's input and is stale while a wave runs, which is why two lanes reading it once both computed § D336. **Take your numbers from your dispatch brief, never from this row.** Wave AD's accounting, which this supersedes as the *next free* figure but not as a record: 17 headings spent of D738–D860, eighty-seven holes registered with each lane's reason, and D840–D860 left free rather than holed because registering them would point this row's `highest + 1` at a hole — the one place § D430 forbids one. |
