...
| Next free decision number | **D601** — wave AB's block **D594–D600** is **fully spent by lane A**, and this row is reconciled on the same commit that spent the last of it, which is the step D387 records that nobody performs when nothing asks (written bare: D387 is a registered hole, a hole heads nothing, and `citations.test.ts` resolves every section reference to a heading). Lane A spent all seven: **D594–D599** on the three reference towers (GitHub issues #428, #427, #426) and **D600** on two authored speeds moved onto their machine class's ladder (#45), a defect those towers' own commissioning check found after the fact. **No number in the block is returned, so there is no hole to register.** **This row is a floor and not a ceiling while wave AB is open**: lane D had not landed when lane A closed, so if it writes D601 upward this row moves again with it — `documentation.test.ts` now carries no open reservation, so it asserts `row = highest + 1` on every commit and will say so. The previous wave's block was **D576–D600** and spent thirteen: D576–D582 lane A (#425, #424, #430), D583 lane B (#433), D587–D588 lane C (#355, #242, #243), D591–D593 lane D (#406), with **D584, D585, D586, D589 and D590 registered permanently as holes**. Two lanes each reported their own returned numbers as free and both were wrong, because free-versus-hole depends on what lands after you — which is why that call is the integrator’s at close. |
