...
| Next free decision number | **D641** — the wave of 2026-09-16 reserved **D623–D644** across two lane blocks dispatched disjoint on purpose, so neither could collide with the other on the number: **D630–D639** for #429 stage 2, **D640–D644** for #372's `rush-prefit` sink. Each lane spent exactly one number — **D630** (#429, a shaft's plan area priced in four measured bands, closing the pricing question § D601 deliberately left the product owner) and **D640** (#372, `rush-prefit` authored as one fixed kit, applied by a new core-side module at both the viewer's press and the server's replay, with the general purse-driven rebuild still refused pending § D606's boundary decision). **D623–D629, D631–D639 and D641–D644 are all permanent holes**: the two lanes' blocks were deliberately set above the reservation floor rather than at it, to leave collision-proof room between two lanes dispatched in parallel from the same base, and each lane returned every number in its own block beyond the one it spent — one issue closed end to end is one decision, this file's usual reason. The previous wave's reserved block was **D619–D621**, spent in full across two collisions (#422/#437 on D619→D620, then #412 on D620→D621); **D622** was spent separately, directly by the integrator outside any lane's block, for the bundle-budget raise. |
