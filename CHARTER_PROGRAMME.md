...
| Next free decision number | **D964** — wave AH is open, holding **D964–D1008** (extended from D999 on 2026-09-24 for a fifth lane; D1000 is this repository's first four-digit number), so this row names that block's floor rather than `max(heading) + 1`, and it stays there for the whole wave: the row is the integrator's input and is stale while a wave runs, which is why two lanes reading it once both computed § D336. **Take your numbers from your dispatch brief, never from this row.** **Wave AG merged with its fix-lane reservation still open.** The integrator opened D963–D970 for the two fix lanes, the second spent D963, and the block was never closed before `499b959` landed on `main`, so `main` claimed for a day that a wave was running when none was. Nothing caught it, because the guard checks that the reservation and this row agree with each other, and they did. D964–D970 had nothing written above them and were therefore free rather than holed (§ D430), so wave AH opens over them. The rule this adds to wave AG's: **a wave's merge commit closes every reservation it opened, the fix block included.** Wave AG's own accounting stands as recorded: nine headings spent of D912–D980, thirty holes registered, and the block opened at D912 in error over holes wave AF had already registered. |
