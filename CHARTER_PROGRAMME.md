...
| Next free decision number | **D621** — wave AC's block **D615–D620** is closed, four of six spent, and this row is reconciled on the same commit that closes it. Spent: **D615** (#540, the revert procedure's step 2 made structurally safe), **D618** (#408, charter S9 B1's run-history store and rolling gate), **D619** (#422, fix-a-building's elevation control and its price-schedule row), **D620** (#437, destination panels priced per landing and the pinned deployment study — drafted as D619 by its own lane before the integrator found it collided with #422's D619 and renumbered it, both entries and every cross-reference between them corrected on the same commit). **D616 and D617 are registered permanently as holes** — the #540 lane was redone from a stale base, and its discarded first attempt used those two numbers before the redo consolidated onto D615 alone. **A #242 (error monitoring) lane that would have used D619–D620** was dropped before either number was reserved for it, after finding the issue already substantially resolved by merged PR #482, with only Azure-credential and legal-review work left outstanding. The previous wave's block was **D594–D620** and spent thirteen, with D602–D605 and D607–D610 registered as holes. **§§ D619 and D620 landed concurrently from two lanes that both started from the same pre-#422 base**, and collided on more than the number: `raise-a-floor`'s `covers` had claimed the whole `building.floors[]` group rather than the one field it writes, which would have silently blocked `landing-panels` from pricing `building.floors[].landingCallType` — narrowed to `building.floors[].heightM` on the same commit — and both lanes' independent budget-ceiling arithmetic (412→422 alone, 412→577 alone) is replaced by the schedule's own re-derived `totalUnits: 587` rather than either figure. |
