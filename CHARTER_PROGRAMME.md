...
| Next free decision number | **D1002** — wave AI is open, holding **D1002–D1046** in five blocks of nine, so this row names that block's floor rather than `max(heading) + 1`, and it stays there for the whole wave. **Take your numbers from your dispatch brief, never from this row**: it is the integrator's input and is stale while a wave runs, which is why two lanes reading it once both computed § D336. Wave AH held D964–D1008, spent thirteen headings, registered twenty-five holes and closed its reservation in its own merge commit; D1002–D1008 were free rather than holed (§ D430), so wave AI opens over them. A wave's merge commit closes every reservation it opened, the fix block included. |
