/*
  ...
  ## Scope, and what this is deliberately not

  This is a SEPARATE lane from `infra/azure/main.bicep`, which provisions self-hosted CI runners for
  the Linux leg of the test matrix (`docs/15-compute-offload-contract.md`). The two share a cloud
  and nothing else: different resource group, different lifecycle, different failure modes, and
  neither reads the other's outputs. Deploying or deleting either one does nothing to the other.

  ## Why a Static Web App and not a server

  `packages/viz` has no server half. `core` publishes an fs-free `browser` export condition, the
  simulation runs in the page and in a Web Worker, and the reference data is five JSON files copied
  out of `data/`. The build output IS the product. Every compute-bearing option — App Service,
  Container Apps, a VM — is therefore paying for an idle process to hand back files that a CDN
  hands back for nothing. See `docs/16` § 1 for the priced comparison.
