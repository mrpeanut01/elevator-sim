 ...
 * `.github/workflows/ci.yml` still describes the environment this repository's pins are true of,
 * and a guard that needs `npm ci` to answer that cannot run when `npm ci` is what broke.
 *
 * The repository has no `yaml` dependency and this does not add one.
 *
 * ## Restored unchanged, and that is a measurement
 *
 * This file was deleted with the rest of `infra/checks/` in `4ab5c02` and is restored here for
 * GitHub issue #414 **byte-for-byte apart from this docstring**. It was written against a two-leg
 * `ci.yml` and it parses today's five-leg one — name, both jobs, the whole `include:` sequence and
 * all nine steps — with no edit. That was checked before anything was built on top of it, because
 * a reader that half-understands a file it did not throw on is how every assertion downstream
